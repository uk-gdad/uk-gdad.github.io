...
This document is written for you: an assessor evaluating a candidate for lead analytics engineer, within the UK Government Digital and Data (GDAD) Profession Capability Framework. It supports you in assessing someone who would lead the design and deployment of data models for analysis, guide a team of analytics engineers, and sit at the junction between organisational processes, stakeholder needs, and the technical craft of data modelling, transformation, and documentation.

Psychometric assessments are structured, standardised exercises that measure the mental capabilities and judgement this role uses every day: interpreting a dense specification, spotting an inconsistency in a data model, reasoning through pipeline metrics under time pressure, or choosing the wisest course of action when a senior stakeholder and a team pull in different directions. Employers — including government departments and their recruitment partners — use these assessments because they are strong, evidence-based predictors of on-the-job performance, and because, administered consistently, they give every candidate the same fair, objective opportunity to demonstrate capability regardless of background.

For a role at this level, assessment items are rarely about abstract puzzles. They test whether a candidate can do, quickly and accurately, the kinds of thinking the role genuinely demands: leading reviews of complex data models, defining data requirements with stakeholders across the organisation, setting standards for communication and documentation, coordinating problem investigations, and communicating fluently between the technical and the non-technical.

This document is organised into four assessment sections, each mirroring a widely used assessment type:

1. **Workplace job-specific cognitive assessment** — pattern recognition, logical deduction, error checking, prioritisation, and problem solving using the artefacts this role really handles: data models, transformation pipelines, test suites, and metadata repositories.
...
4. **Workplace job-specific situational judgement assessment** — realistic workplace dilemmas involving the candidate's stakeholders, team, and the values of UK public sector delivery.

Each section explains what the assessment measures, maps it explicitly to the skills in the role profile, provides a substantial bank of worked practice questions with answers at varied difficulty levels, and closes with administration tips and common pitfalls to watch for when scoring.

How to administer this fairly: present the same items and hold every candidate assessed against this role level to the same time limit, so that results are comparable across a field. Keep the worked answers and explanations back as your answer key — do not share them with the candidate before the assessment, and hold them back until scoring and debrief are complete. Each section's practice questions carry a rough time-per-item, which is useful for planning a realistic session: the cognitive and verbal sections run at well under a minute per item, the numeric section at around a minute to ninety seconds per item, and the situational judgement section is usually untimed or generously timed. Treat the result as one input among several — never the sole basis for a hiring, promotion, or development decision — and keep your scoring notes consistent across candidates so that comparisons stay fair.
...
A workplace job-specific cognitive ability assessment is a targeted psychometric test designed to measure the exact mental processes needed for this role — not a generic IQ test, but a set of practical exercises built from the everyday tasks and data formats of analytics engineering leadership. This section presents the candidate with artefacts the role genuinely handles — dependency graphs of transformation models, star schema designs, data quality rule sets, review checklists, metadata records — and asks them to find the pattern, deduce the consequence, spot the error, or choose the priority.

The format is usually online and timed, most commonly 15 to 30 minutes, with objective scoring against a benchmark or norm group. Modern platforms often include short ungraded practice questions before the real test begins, and some adapt question difficulty based on the candidate's previous answers. You will usually receive a breakdown of speed versus accuracy rather than a single score, so note both when assessing a candidate: working quickly and carefully both matter. Questions used in real recruitment are validated to ensure they predict job success without bias — which is exactly why they focus on realistic work rather than abstract trivia.

For a lead analytics engineer, cognitive ability assessment matters because the role multiplies a person's thinking across a team. When a candidate reviews another engineer's dimensional model, they are error checking. When they trace why a downstream dashboard broke after an upstream schema change, they are performing logical deduction across a dependency graph. When they decide which of five competing requests a team tackles first, they are prioritising under constraint. A strong performance on this assessment signals that the candidate can do these things reliably and at pace.

### What it measures for this role

The assessment dimensions map directly onto the skills named in the role profile:

- **Data modelling, cleansing and enrichment** — pattern recognition and error checking within complex data models: spotting a fan trap in a star schema, a grain mismatch in a fact table, or a slowly changing dimension handled inconsistently against the team's standards.
...
**Explanation:** Follow the directed dependency graph downstream from the failure. `stg_customers` feeds `int_customer_profile`, which feeds `fct_transactions`, which feeds both the dashboard and the report — so all four are affected. `stg_payments` and `int_payments_enriched` sit on a parallel branch upstream of the failure's influence and build independently, so D over-includes. C stops too early: anything consuming `fct_transactions` inherits the failure. This tests the same deduction a strong candidate performs when triaging an overnight build failure before stakeholders start their day.

**Question 2 (easy) — Error checking a standard.**
A team's naming standard states: staging models are prefixed `stg_`, intermediate models `int_`, fact tables `fct_`, and dimension tables `dim_`; all names are lower case with underscores. During a review, the candidate sees this list of new models submitted by a team member: `stg_hmrc_receipts`, `int_ReceiptsEnriched`, `fct_receipts_daily`, `dim_taxpayer`, `stg_Case_Notes`. How many models violate the standard?
...
**Explanation:** Read the requirement precisely: uniqueness applies to *current* rows per URN, not all rows — a slowly changing dimension may legitimately hold multiple historical rows per URN, so B's blanket uniqueness test would fail correct data. The requirement also asserts retention of closed schools with a specific flag value, so a presence-plus-flag test is needed, and a not-null test on the flag guards the boundary. A is far too weak; D tests completeness, which is worthwhile but does not address the stated requirement. Translating requirements into precise test conditions — no stronger and no weaker than the specification — is exactly the Testing skill at this level, where the role reviews requirements and defines test conditions for the team.

**Question 6 (moderate) — Pattern recognition in run history.**
A nightly pipeline's total run time over ten nights (in minutes) was: 42, 43, 44, 43, 58, 44, 45, 59, 45, 60. Which statement best characterises the pattern?
...
**Explanation:** Trace the consequences of the proposed order. Turning on enforcement (v) first means every change is judged against a standard nobody has read (i and ii come later) and that the tooling does not yet help them meet (iii is last) — the team is blocked, trust in the standard is damaged, and delivery stalls. A sensible order is i, ii, iii, iv, v: publish, build understanding, make compliance easy, clean up the estate, then enforce. A states a rigid rule that is not itself the flaw; C and D are simply poor practice. This item tests ordering logic — recognising which dependencies are real — and mirrors the role's responsibility for creating standards and defining ways of working in a manner the team can actually adopt.

### Administration tips

- **Give every candidate the same artefacts, cold.** Do not let a candidate rehearse on the actual dependency graphs, review checklists, or test suites used in the item bank beforehand; a candidate who can articulate *why* a model passes or fails review, drawing on their own working experience, is demonstrating exactly the deduction pathways these tests measure.
- **Watch for the constraint, not the vibe.** At lead level, questions are built so the plausible-sounding answer and the correct answer differ by one constraint, as in Questions 9 and 12. A strong candidate visibly checks each option against every stated condition rather than settling on the first plausible one.
- **Hold the time budget like a schedule.** With 15–30 minutes and objective scoring on both speed and accuracy, brief the candidate to treat each question like a job in a schedule: park anything that exceeds its time budget and return to it later.
- **Run the ungraded practice items properly.** Give the candidate time to learn the interface — how to flag questions, how the timer displays — before scoring begins, so no cognitive capacity is wasted on mechanics during the real items.
- **Control the environment.** These assessments measure processing under time pressure; a quiet room and a large screen for reading diagrams and tables are legitimate, and should be offered consistently to every candidate.

### Common pitfalls

- **A candidate answering from experience instead of from the question.** Candidates at this level have deep knowledge of real systems, but the test rewards reasoning strictly from the information given. If the question says the standard requires X, a strong answer applies X even where the candidate's own team does Y.
- **Impressionistic error checking.** A candidate skimming a list of model names or a code snippet and judging by feel that it is fine, rather than checking rule by rule, item by item, is a weaker performance the test is calibrated to catch.
- **Anchoring on the first plausible option.** Distractors are engineered to be almost right; a strong candidate's working confirms their choice survives every stated fact before moving on.
- **Time sink questions.** A candidate spending five minutes on one hard deduction while three easy questions go unanswered will show a weaker speed-versus-accuracy profile; a stranded hard question costs more than a skipped one.
- **Ignoring diagram and table labels.** Misreading which direction a dependency arrow points, or which column a test refers to, converts a solvable question into a guess — note in your scoring where a candidate's answer suggests this happened.

## Workplace job-specific numeric reasoning assessment

### About this assessment

A workplace job-specific numeric reasoning assessment evaluates a candidate's ability to analyse, interpret, and make sound decisions using numerical data of the kind this role really handles. It is business mathematics, not abstract algebra: percentages, ratios, rates of change, cost-benefit comparisons, and capacity calculations, presented through tables, charts, budgets, and performance dashboards. Most assessments allow or provide an on-screen calculator, because the skill being measured is interpretation — extracting the right figures, applying the right operation, and sanity-checking the result — rather than mental arithmetic. Time limits are strict, typically around a minute to ninety seconds per question, which simulates the reality of making defensible numerical judgements at working pace.

For a lead analytics engineer, numbers are the fabric of the job. The role quantifies data quality (null rates, match rates, duplication percentages), pipeline performance (run times, failure rates, cost per run), team capacity (review throughput, sprint allocation), and the business case for innovation (licence costs versus engineering hours saved). When presenting to stakeholders across the organisation, a lead's credibility rests on numerical claims that survive scrutiny. When reviewing a team member's analysis of a source system, they must spot the percentage that cannot be right. This assessment measures precisely that fluency.
...
Each numeric dimension corresponds to skills named in the role profile:
...
- **Programming and build (data engineering)** — capacity and scheduling arithmetic: run durations, windows, and throughput of the kind the role uses when sequencing builds.

### Practice questions

**Question 1 (easy) — Null rate percentage.**
The candidate profiles a source table of 250,000 citizen records. The `postcode` column contains 12,500 nulls. What is the null rate, and what is the populated rate?
...
**Explanation:** Null rate = 12,500 ÷ 250,000 = 0.05 = 5%. Populated rate = 100% − 5% = 95%. A quick sanity check: 10% of 250,000 would be 25,000, so 12,500 is exactly half of that — 5%. Profiling metrics like this are the first numbers a candidate computes in source system analysis, and assessments begin with items like this to confirm the candidate converts counts to percentages quickly and without error.

**Question 2 (easy) — Percentage change in run time.**
After a team refactors a transformation model, its nightly run time falls from 48 minutes to 36 minutes. What is the percentage reduction?
...
**Explanation:** Reduction = 48 − 36 = 12 minutes. Percentage reduction = 12 ÷ 48 = 0.25 = 25%. The classic trap is D: 12 ÷ 36 = 33%, which divides by the *new* value instead of the original. Percentage change is always relative to the starting figure. When reporting optimisation gains to stakeholders, this distinction is the difference between an accurate claim and an inflated one.

**Question 3 (moderate) — Match rate across conformed sources.**
The candidate is conforming two sources into a single customer model. Source A has 180,000 records; Source B has 150,000 records. After standardising keys, 132,000 records match between the two sources. What percentage of Source B records remain unmatched?
...
**Explanation:** Unmatched in Source B = 150,000 − 132,000 = 18,000. As a percentage of Source B: 18,000 ÷ 150,000 = 0.12 = 12%. Option C (48,000 ÷ 180,000) muddles populations by using Source A's unmatched count and denominator; option D mistakes the raw count (18,000) for a percentage. Reading *which population* the question asks about is half the skill — exactly as it is when reporting match rates for a conformed model, where a lead must be precise about which source the percentage refers to.

**Question 4 (moderate) — Weighted data quality score.**
A data quality standard scores each dataset on three dimensions: completeness (weight 50%), validity (weight 30%), and timeliness (weight 20%). A dataset scores 90 for completeness, 70 for validity, and 60 for timeliness. What is its weighted quality score?
...
**Explanation:** Weighted score = (90 × 0.5) + (70 × 0.3) + (60 × 0.2) = 45 + 21 + 12 = 78. Option A is the unweighted mean (220 ÷ 3 = 73.3), the most common error when candidates ignore the weights. Weighted scoring appears constantly in this role's work — quality scorecards, prioritisation matrices, vendor comparisons — and the arithmetic habit is always the same: multiply each score by its weight, then sum.
...
A team's automated test suite results over three releases: Release 1 — 320 tests, 288 passed. Release 2 — 360 tests, 333 passed. Release 3 — 400 tests, 356 passed. Which release had the highest pass rate, and what was it?
...
**Explanation:** Compute each rate: Release 1 = 288 ÷ 320 = 90%. Release 2 = 333 ÷ 360 = 92.5%. Release 3 = 356 ÷ 400 = 89%. Release 2 is highest at 92.5%. Note the trap in the question's surface: Release 3 has the most *passing tests* (356) but the lowest *rate*. Raw counts and rates diverge whenever denominators change — a distinction the role draws whenever it analyses and reports test activities and results for a growing test suite.

**Question 7 (moderate) — Warehouse cost per run.**
A pipeline's monthly warehouse compute bill is £5,400. The pipeline runs nightly (30 runs in the month), plus 15 additional ad-hoc full refreshes that month, each costing twice as much compute as a nightly run. What does one nightly run cost?
...
**Explanation:** Currently: 5 + 3 + 2 = 10 parts across 400 hours, so one part = 40 hours — build 200, review 120, support 80. Support doubles to 160 hours, leaving 400 − 160 = 240 hours for build and review in the ratio 5 : 3 (8 parts, 30 hours per part): build = 5 × 30 = 150, review = 3 × 30 = 90. Option A is the pre-change figure; option B wrongly applies the doubling elsewhere; option D splits the remainder evenly-ish without honouring the ratio. Ratio reallocation under a fixed total is the arithmetic behind every capacity conversation this role has when defining and improving a team's ways of working.


**Question 9 (hard) — Cost-benefit of tool adoption.**
The candidate is evaluating a data transformation tool for the team. Licence cost: £18,000 per year. It is estimated to save each of 6 analytics engineers 3 hours per week, with an engineering hour costed at £45. Assuming 44 working weeks per year, what is the approximate annual net benefit of adoption?
...
**Explanation:** Q1 rate = 24 ÷ 1,200 = 2.0%. Q3 rate = 18 ÷ 1,500 = 1.2%. That is a fall of 0.8 percentage points, which is a relative reduction of 0.8 ÷ 2.0 = 40%. The stakeholder's "25%" describes the raw incident count (24 → 18), which understates the improvement because run volume grew. Option A's figures are correct but option B is the better answer for this role: presenting both the percentage-point change and the relative change is how a lead manages expectations honestly when communicating between the technical and the non-technical — a single dramatic-sounding figure without its companion framing can mislead in either direction. Option D confuses denominator growth with rate growth.
...
An interoperability standard requires each source to contribute at least 220,000 records that have BOTH a valid postcode AND a valid date of birth (assume validity of the two fields is independent, and ignore duplicates). Which sources meet the requirement?
...
Five data problems were resolved by a team last month, taking 4, 6, 3, 15, and 7 hours respectively. A stakeholder asks for the average resolution time. Which statement gives the most honest summary?
...
**Explanation:** Required coverage = 95% of 80 = 76 models. Currently documented: 68. Additional models needed = 76 − 68 = 8. Option B is the total undocumented count (80 − 68 = 12), which overshoots the question asked; option C misreads the target as 90%. The pattern — compute the required absolute number from the percentage target, then take the gap from the current position — recurs across coverage, quality, and test-pass metrics, and it is the calculation the role presents when setting a team an improvement goal with a concrete, finite backlog rather than a vague percentage aspiration.

### Administration tips

- **The core four recur.** Percentage change, weighted averages, ratios, and compound growth cover most role-relevant questions. A strong candidate's setup (what to divide by what) is fast and automatic, leaving the calculator to do the rest.
- **Watch for denominator errors.** Most wrong answers in numeric tests are right numerators over wrong denominators. A candidate whose working stops to ask "percentage *of what*?" before calculating is showing good practice.
- **Look for intermediate working.** Multi-step questions (cleansing then growth; reallocating ratios) are engineered so that skipping a step lands on a distractor. A candidate who jots each stage — the same discipline as documenting each transformation step — is easier to assess and more reliable.
- **Estimation is a legitimate strategy.** In Question 11, knowing 5% compounding needs roughly five periods to reach 25% lets a candidate pick B before doing precise arithmetic; estimation first, precision second, is faster and self-checking, and should not be marked down.
- **Time the table-reading questions.** Note how quickly a candidate locates the right cell in a profiling report or cost dashboard; speed at this is trainable and a fair thing to observe.
- **Give the candidate a calm start.** The timed format is often the only unfamiliar element for an otherwise fluent candidate. A short settling period before starting can improve accuracy on the first questions, which set the pace for the rest.

### Common pitfalls

- **Misreading scale qualifiers.** Confusing thousands with millions, monthly with quarterly figures, or minutes with hours. Every table header and axis label deserves close attention — note where a candidate's answer suggests they missed one.
- **Dividing by the new value in percentage change.** The reduction from 48 to 36 is 25%, not 33%. This single error accounts for a large share of lost marks and is worth flagging in feedback.
- **Over-calculating.** Performing long exact arithmetic when rounding would identify the correct option faster. If options are far apart, estimation is the efficient route; if close together, exact calculation is needed.
- **Ignoring stated assumptions.** When a question says "assume independence" or "ignore duplicates", it is telling the candidate which mathematical route to take. Fighting the assumption wastes time and derails the method.
- **Time sink questions.** A multi-stage cost-benefit item can consume four minutes. A candidate who is two minutes in without a clear path and does not move on is trading two easy marks for one hard one.
- **Answering the present when the question asks about the future.** As in Question 13, projections are a favourite trap: the strongest answers compute the state the question actually asks for.

## Workplace job-specific verbal reasoning assessment

### About this assessment

A workplace job-specific verbal reasoning assessment evaluates how well a candidate understands, analyses, and draws correct conclusions from written business information of the kind this role deals with daily. It mirrors the exact reading demands of analytics engineering leadership: data standards documents, transformation specifications, governance policies, stakeholder emails, review reports, and metadata definitions. Passages use real professional terminology, and the questions demand fact-based logic — the candidate must answer strictly from the text provided, deliberately setting aside their own professional knowledge, however deep it is.

The dominant question format is **True / False / Cannot Say**: given a statement about the passage, the candidate decides whether the passage confirms it (True), contradicts it (False), or provides insufficient information either way (Cannot Say). Assessments also include multiple-choice comprehension items testing whether the candidate can identify the core argument, the precise scope of a rule, or the best summary of a stance. Timing is tight — often less than a minute per question — simulating the pace at which a lead skims-then-verifies documents in a working day.

Why does this matter for a lead analytics engineer? Because the role is unusually text-critical. A lead creates standards for communication, data models, and documentation — texts other people must be able to apply unambiguously. They review specifications and define test conditions from their exact wording. They interpret the needs of technical and non-technical stakeholders from emails and meeting notes where precision varies wildly. A misreading at this level does not stay contained: it propagates through a team's models and into decisions made across the organisation. Verbal reasoning assessments measure the exact discipline — reading what the text says, no more and no less — that keeps this from happening.
...
**Explanation:** The passage says reviews "should normally be completed within two working days". "Should normally" is guidance, not an absolute requirement — contrast it with the "must" used elsewhere in the same passage. A three-day review therefore departs from the norm but does not breach a mandatory rule, so the claim of a breach is contradicted. Discriminating between "must" and "should" is a precision skill assessments test relentlessly, and it is the same distinction a lead encodes deliberately when writing standards for their own team.
...
**Explanation:** The passage says such reviews must be approved by "the lead analytics engineer *or a nominated deputy*". The word "or" makes the statement's "must approve all" too strong: a nominated deputy can approve instead. The claim is therefore contradicted as written. Notice the assessment skill here — the statement is nearly right, and the single overlooked disjunction flips the answer. As the person who nominates deputies and designs approval flows, a lead knows operationally why that "or" exists; the test asks the candidate to see it on the page.
...
**Explanation:** The final sentence says the board "may grant exemptions to *any* provision of this policy", and the before-publication entry requirement is a provision of the policy. So a granted exemption would permit exactly what the statement describes. The statement is conditional ("if the board has granted an exemption"), and the passage supports the condition's effect. Candidates sometimes answer False on the strength of the opening "must" alone — but a policy's exemption clause modifies everything before it, just as it does in the real governance frameworks this role operates within.
...
**Explanation:** The passage states that the existing platform budget covers the cost "for the pilot period", and that "funding for wider rollout would require a separate business case". The statement claims the existing budget would fund the wider rollout, which the final sentence directly contradicts — regardless of the pilot's outcome. Note the two-layer trap: the conditional framing ("if the pilot is successful") invites a Cannot Say response because the passage does not say what happens after a successful pilot; but the *funding source* claim is addressed head-on and denied. When a compound statement contains one element the passage contradicts, the statement as a whole is False. Parsing compound claims into their separable parts is precisely the skill this role uses when reviewing requirement statements that bundle several obligations into one sentence.

### Administration tips

- **Look for anchoring to quoted words.** Before selecting True or False, a strong candidate can identify the exact phrase that confirms or contradicts the statement. Where they cannot point to one, the answer is probably Cannot Say. This one habit is worth noting in scoring, as it predicts accuracy on most verbal reasoning items.
- **A candidate's modal-verb reading is diagnostic.** Must, may, should, normally, minimum, at least, or, and — how carefully a candidate weighs these words distinguishes strong from weak performance on this section.
- **The test asks candidates to suppress professional knowledge deliberately.** A candidate may well know how metadata policies and incident reviews usually work, but the test only cares what *this* passage says. A strong answer is grounded in the text, not in professional experience.
- **Statements can be as precise as the passage.** Half the traps are in the statement, not the passage — an added "all", a dropped "or", a completion claimed where only an intention was stated. Note in scoring whether a candidate spots these.
- **The section rewards a two-pass read.** Skimming the passage for structure first, answering retrieval questions fast, and reserving deeper re-reading for Cannot Say candidates is the efficient approach at well under a minute per question.
- **Decomposing compound statements is a marker of strength.** When a statement bundles two claims (a condition and a funding source, an action and its completion), the strongest candidates test each part separately against the passage. If the passage contradicts any part, the whole statement is False; if it confirms every part, True; if it confirms some and is silent on the rest, Cannot Say.

### Common pitfalls

- **Bringing in outside knowledge.** The most common failure for experienced candidates: answering from how things are in their own organisation rather than from the passage. The test is calibrated to punish exactly this.
...
- **Treating silence as denial.** If the passage does not address a claim, the correct answer is Cannot Say — not False. False requires contradiction; silence contradicts nothing.
...
- **Missing exception clauses.** Absolute-sounding opening sentences are routinely qualified later ("emergency process", "exemptions"). A candidate who reads to the end of the passage before answering anything about "always" or "every" is showing good practice.
- **Poor time allocation.** Reading every passage exhaustively before looking at the questions burns the clock. Skimming for structure, then verifying against the question, is the more efficient approach, and worth noting when a candidate consistently does the opposite.

## Workplace job-specific situational judgement assessment

### About this assessment

A workplace job-specific situational judgement assessment evaluates a candidate's decision-making, interpersonal skill, and professional conduct through realistic workplace dilemmas. Unlike the cognitive, numeric, and verbal assessments, there is no single objectively provable answer computed from data — instead, each scenario offers several plausible responses, and the candidate's task is to judge which are more and less effective given the role, the organisation's values, and the practical realities of delivery. Typical response formats ask the candidate to select the **most effective** and **least effective** actions from a set, or to rank or rate every option from best to worst. Timing is usually generous or untimed, because the assessment measures natural judgement rather than speed.

For a lead analytics engineer in UK government, the scenarios draw on the real working world of the role: a team of analytics engineers whose work a lead reviews and whose growth they support; stakeholders ranging from operational caseworkers to finance directors and governance boards; standards a lead has authored and must sometimes enforce against resistance; data quality problems with public-facing consequences; and the constant balancing of delivery pressure against doing things properly. Strong answers consistently reflect UK public sector values — meeting user needs, transparency and openness, collaboration across teams, accessibility and inclusion — alongside the leadership behaviours this role level demands: empowering rather than rescuing, addressing root causes rather than symptoms, and communicating honestly between the technical and the non-technical.

Situational judgement scores predict how a candidate will actually behave under the routine pressures of the job, which is why they carry substantial weight in public sector selection. A candidate genuinely ready for this level will have seen most of these dilemmas in real life. The assessment simply asks the candidate to show, option by option, that they can tell wise handling from expedient handling.

### What it measures for this role

- **Communicating between the technical and non-technical** — scenarios test whether the candidate manages expectations honestly, hosts difficult discussions rather than avoids them, and translates technical trade-offs for senior stakeholders without distortion.
- **Problem management** — dilemmas about incidents and recurring failures test whether the candidate coordinates investigation, resists blame, fixes root causes, and puts preventive measures in place.
- **Data modelling, cleansing and enrichment** and **Testing** — scenarios about review findings, standards breaches, and quality shortcuts test whether the candidate upholds standards proportionately, without either rigidity or laxity.
- **Metadata management** and documentation duties — dilemmas test whether the candidate treats documentation and definitions as first-class delivery, even under time pressure.
...
A newer analytics engineer on the candidate's team submits a data model for review. It works, but it breaches the team's standards in several ways: undeclared grain, no uniqueness tests, and cryptic column names. This is their third submission with similar problems. The model is needed by a stakeholder team next week. Rate the options.

Option A: Approve it this once given the deadline, and fix the issues personally over the weekend.
...
Option C: Refuse, citing the team's change process, and copy in the candidate's own manager.
Option D: Add the measure but privately tell the director's analyst about the skew so they can mention it if asked.

**Most effective: B. Least effective: A.**

**Explanation:** Option B is the model answer for communicating between the technical and the non-technical: it neither blocks nor blindly complies; it converts a technical caveat into decision-useful plain English, offers a deliverable that serves the user need honestly by Friday, and manages expectations about the full fix. Option A is least effective: a silently skewed figure in a ministerial briefing is a transparency failure with real public consequences, and "the director owns the risk" is untrue when the candidate knew and did not say. Option C protects process over purpose — the director's need is legitimate and urgent; escalating defensively before offering options is neither collaborative nor user-focused. Option D is the quiet-corridor version of honesty: the caveat reaches the briefing only by luck, and it positions the candidate as someone who knew of a defect and routed the warning informally rather than owning it.

**Question 3 (hard) — Conflicting definitions.**
The candidate is conforming data from three business units into a single "active case" model. In workshops, each unit insists its own definition of "active" is correct, and the disagreement is becoming heated; two unit heads have separately emailed the candidate asking them to adopt their definition. Their team is blocked pending a decision. Rate the options.
...
**Explanation:** Option B treats the audit finding as a ways-of-working problem, not a diligence problem — which the retrospective comment confirms it is. Building documentation into the definition of done and into estimates removes the structural incentive to cut it; automated checks make compliance cheap; a usage-prioritised backfill is proportionate rather than heroic. This is this role's duty to create and maintain data documentation and to define and improve ways of working, done at the system level where leads operate. Option C is least effective: silently lowering a standard because it is inconvenient tells the team that all standards are negotiable under pressure, and undocumented models directly undermine reuse, onboarding, and the metadata quality stakeholders depend on. Option A punishes users of the platform for the team's process debt and creates a delivery cliff. Option D is the classic ineffective middle: restating a rule without changing any of the conditions that caused it to be cut.

**Question 7 (hard) — The difficult senior meeting.**
In a cross-directorate meeting, a senior stakeholder publicly blames the candidate's team for "wrong numbers" in a report, in front of their two most junior engineers. The candidate is fairly sure the discrepancy comes from the stakeholder's own team using an outdated extract rather than their model, but cannot prove it in the room. Rate the options.

Option A: Push back firmly in the meeting with the candidate's working theory, so the accusation does not stand unchallenged.
Option B: Say the discrepancy is being taken seriously, commit to a joint investigation with the stakeholder's team, and offer a specific date for shared findings; afterwards, check in with the candidate's junior engineers, and follow up with the stakeholder one-to-one about handling concerns before public forums.
...
**Explanation:** Option B does five things well: it stays composed and factual under public pressure; it converts an accusation into a *joint* investigation — collaborative rather than adversarial, and the fastest route to the truth whoever is right; it commits to specifics (a date, shared findings) that manage expectations; it protects and reassures the junior engineers who just watched their team attacked; and it addresses the behavioural issue privately, hosting the difficult conversation this role's skill profile explicitly includes. Option C is least effective: apologising for an error the candidate does not believe occurred concedes a "fact" that will be repeated in future meetings, demoralises the team, and — if the candidate's theory is right — leaves the actual root cause (an outdated extract) uninvestigated so the error recurs. Option A risks an evidence-free public confrontation that becomes about personalities, and it may be wrong. Option D avoids the moment but then escalates in writing to a wide audience — the same public contest, minus the chance for dialogue.

**Question 8 (moderate) — The under-review backlog.**
The candidate returns from two weeks' leave to find eleven model changes awaiting their review, some blocking other teams. Their standards require lead approval for business-critical models (six of the eleven). Two senior engineers on the team are experienced reviewers. Rate the options.

Option A: Work through all eleven personally in order of arrival, however long it takes.
Option B: Approve all eleven quickly on the senior engineers' assurance that they look fine.
Option C: Triage by impact; formally nominate the two senior engineers as deputies for business-critical approvals (as the standard allows), split the backlog, and then change the process so lead approval is never a single point of failure during absences.
Option D: Ask the requesting teams which changes are truly urgent and review only those this week.

**Most effective: C. Least effective: B.**

**Explanation:** Option C resolves today's backlog and tomorrow's: triage handles urgency, formal nomination keeps the approvals legitimate under the candidate's own standard rather than bypassing it, delegation develops the candidate's senior engineers, and the process change removes the structural single point of failure — leads who bottleneck reviews are a well-known anti-pattern, and defining better ways of working is the job of this role. Option B is least effective because "they look fine" is not a review: it launders eleven unexamined changes — six of them business-critical — through the candidate's authority, defeating the purpose of the control while pretending to honour it. Option A is diligent but slow, keeps other teams blocked, misses the leadership lesson entirely, and guarantees a repeat after the candidate's next leave. Option D usefully gathers priority information but, alone, leaves non-"urgent" teams blocked indefinitely and still fixes nothing structural.

**Question 9 (moderate) — The accessibility gap.**
A well-regarded analyst in a stakeholder team tells the candidate privately that the documentation site their team maintains is difficult for them to use with a screen reader — tables render poorly and diagrams have no text alternatives. The team is mid-sprint on a stakeholder-facing deliverable. Rate the options.

Option A: Thank them, log it in the backlog, and pick it up when the current delivery pressure eases.
Option B: Thank them, make the quick fixes now (text alternatives, heading structure), schedule the structural fixes visibly in the next sprint, ask whether they would be willing to sanity-check the improvements, and update the documentation standards so new content is accessible by default.
...
**Explanation:** Option C combines the two things leadership requires here: genuine listening — long-serving engineers' objections sometimes surface real flaws in a new standard, and refining standards in response to feedback is part of this role — and unambiguous consistency, because an informally exempted senior engineer teaches the whole team that standards are optional for the sufficiently confident. The two regressions give the conversation concrete, non-personal evidence. Option D is least effective: naming an individual's compliance failure before the group is public shaming by process — it poisons the retrospective's psychological safety, humiliates a respected colleague, and makes future honest discussion less likely. Option A trades short-term peace for the standard's credibility. Option B may eventually be necessary as a follow-through, but as an opening move it skips the conversation, the listening, and the support — enforcement without engagement reads as hostility and squanders a respected engineer's goodwill.

**Question 12 (hard) — The tempting shortcut at scale.**
A programme director offers the candidate's team significant new funding to build conformed data models for four additional business areas — but requires all four in six months, a pace the candidate's experience says will force the team to skip profiling, testing, and documentation. Declining outright risks the programme building its own shadow data team instead. Rate the options.
...
**Explanation:** Option C is expectation management at programme scale: it takes the director's need seriously, converts the candidate's delivery-quality concerns into consequences a non-technical leader can weigh (rework, unreliable figures, support burden), and offers credible alternatives rather than a bare no — the behaviour the communication skill at this level describes as supporting difficult discussions with diverse senior stakeholders. Evidence-based counter-proposals also usually beat shadow-team threats, because the director's real interest is delivered outcomes, not organisational rivalry. Option D is least effective: it is a commitment the candidate already believes to be false, and planned failure discovered late costs the director options they would have had in month one — it is the least transparent choice on the list, and it makes the eventual difficult conversation happen at the worst possible time. Option A gambles the team's health and the models' integrity on hope, and quietly plans to breach the very standards that make the team worth funding. Option B may be where a candidate ends up if negotiation fails, but leading with refusal — no options, no engagement — abandons a legitimate organisational need and invites exactly the shadow-team outcome it fears.

### Administration tips

- **Score on values, then on effectiveness.** For each option, two questions apply in order: does this reflect public sector values (user needs, transparency, collaboration, accessibility, inclusion)? And does it actually solve the problem, including its root cause? The best options score on both; the worst usually fail both.
- **Look for leadership framing, not engineer framing.** At lead level, options that involve the candidate doing the work personally, quietly absorbing problems, or bypassing their own team should generally rank lower in a candidate's answer than options that develop people, fix systems, and communicate openly — even where the hands-on option feels generous.
- **Watch for the "quietly" word.** Options containing quiet fixes, private workarounds, or selective disclosure almost always rank poorly in public sector scenarios, where transparency is a first-order value — a candidate favouring these is worth noting.
- **Distinguish interim tactics from complete answers.** Many weaker options are fine first steps (escalate, log it, ask for priorities) that fail because they are offered as the whole response. The strongest candidate answers tend to pair immediate action with a structural follow-through.
- **A candidate's own experience is useful calibration, not a substitute for the scenario.** A candidate may draw on how similar situations have unfolded for them, but a strong answer stays anchored to the scenario on the page and its stated constraints, rather than substituting a different remembered situation.
- **Ask the candidate to read every option before rating any.** Most/least formats are comparative; an option that seems strong alone may be dominated by a better-constructed neighbour, and a candidate who rates in isolation may miss this.
...
- **Selecting passive or deflecting options.** Logging, waiting, forwarding to another team, or escalating without any action of the candidate's own are classic low-scoring choices: they delay action, pass the problem on, or ignore the root issue.
- **Choosing conflict avoidance dressed as diplomacy.** Apologising for errors not yet verified, conceding definitions to the loudest stakeholder, or exempting difficult colleagues all buy short-term calm at long-term cost — and a strong candidate's answer avoids this.
...
- **Overthinking towards the exotic option.** The most effective option is usually the one that acts promptly, communicates honestly, involves the right people, and prevents recurrence. If a candidate's chosen option does all four, that answer can generally be trusted.

## Conclusion

You now have four complete, role-specific assessment resources to administer: a cognitive assessment built from the dependency graphs, standards, models, and problem investigations a lead analytics engineer works with every day; a numeric reasoning assessment grounded in the quality metrics, capacity plans, budgets, and cost-benefit cases the role presents to stakeholders; a verbal reasoning assessment drawn from the standards documents, stakeholder emails, policies, and incident reviews the role reads and writes; and a situational judgement assessment reflecting the genuine leadership dilemmas of running an analytics engineering team in the UK public sector.

**Scoring.** For the cognitive, numeric, and verbal sections, mark each item against the stated correct answer, and note both accuracy and — if you have timed the session — pace, since real recruitment tools typically report both. For the situational judgement section, compare the candidate's most/least effective choices (or full ranking) against the model answer given for each scenario, and use the explanation to judge how close a partial match is: an answer that gets the most effective option right but the least effective option wrong is a different signal from one that inverts the whole ranking. Keep a simple record for each candidate — item-by-item results plus any notes on their reasoning — so that scores are traceable and comparable across the field.

**Debrief.** Once scoring is complete, share the outcome with the candidate in a straightforward, respectful conversation. Explain broadly what each section measured and how the candidate performed against it, using the explanations in this document as a guide to what a strong answer looks like, without necessarily working through every item line by line unless your process calls for that. Give the candidate the chance to ask questions, and be honest about where an answer fell short as well as where it was strong — the goal is a fair, informative conversation, not a pass/fail verdict delivered without context.

**Consistency and records.** Administer the same items and the same time limit to every candidate assessed against this role level, so that comparisons between candidates are fair. Keep the answer key confidential until scoring is complete, retain your scoring records in line with your organisation's data retention and recruitment policies, and treat this assessment as one input among several — alongside interview, portfolio, reference, and other evidence — rather than a standalone verdict on a candidate's suitability.

Whether you are using this for a selection process or a development conversation, treat the document as a resource to return to for future candidates rather than a one-time read. The four sections were built directly from the accountabilities and skills this role level names, so they stay grounded in what the job actually requires — and administered fairly and consistently, they are a useful, if unvalidated, part of a wider evidence base for your decision.
