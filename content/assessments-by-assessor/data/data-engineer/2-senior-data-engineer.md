...
This document supports an assessor - a line manager, a panel member, or a reviewer - administering a workplace job-specific psychometric assessment to a candidate for the senior data engineer role in the UK Government Digital and Data profession. It gives structured assessment material for someone who designs and leads the implementation of data flows that connect operational systems with analytics and business intelligence platforms, leads the build of data streaming systems, optimises code for performance, and leads work on database management.

Psychometric assessments are structured, standardised exercises that measure how a candidate thinks, reasons, and makes decisions. They are widely used across the Civil Service and the wider public sector, both in recruitment and in professional development, because they offer an objective, evidence-based view of capability that complements interviews and technical exercises. For a senior data engineer, they matter for a very practical reason: the role's day-to-day work depends on exactly the abilities these assessments measure. A senior data engineer diagnoses faults in complex pipelines under time pressure, interprets throughput and capacity figures, reads dense technical standards and policy documents with precision, and exercises judgement in situations where stakeholders disagree and there is no perfectly clean answer.

This document is organised into four main assessment sections, each mirroring a type of test the candidate is likely to encounter:

1. **Workplace job-specific cognitive assessment** - pattern recognition, logical deduction, error checking, prioritisation, and problem solving using the artefacts the role genuinely handles: pipeline DAGs, schema definitions, streaming configurations, and test results.
...
4. **Workplace job-specific situational judgement assessment** - realistic dilemmas involving the role's real stakeholders: multidisciplinary teams, analysts, architects, service owners, and suppliers.

Each section explains what the assessment measures, maps it to the specific skills in the role profile, provides substantial practice-calibrated items with full worked answers, and closes with administration guidance and a scoring note on common pitfalls to watch for.
...
A workplace job-specific cognitive ability assessment is a targeted psychometric test designed to measure the exact mental processes needed for the role, rather than abstract intelligence. Instead of generic puzzles, it presents practical workplace scenarios and data formats drawn from senior data engineering: pipeline dependency graphs, schema definitions, streaming configurations, test result matrices, and metadata catalogues. The candidate's task is to reason quickly and accurately about them.

Typical format: an online, timed test of 15 to 30 minutes, with questions that mimic the everyday tasks and artefacts of the job. Score objectively, comparing results against a benchmark or norm group of professionals at a similar level. Modern platforms often adapt question difficulty based on a candidate's previous answers, and it is worth recording a breakdown of speed versus accuracy rather than a single headline score. Where a platform offers short, ungraded practice questions first, use them - they let the candidate get comfortable with the interface before scoring begins.

For a senior data engineer, cognitive assessment matters because the role is fundamentally about reasoning under constraint. When a streaming pipeline degrades at 09:00 on a Monday, the role holder must deduce the likely cause from partial evidence. When two data models disagree, they must spot the structural difference. When a release deadline approaches, they must prioritise faults by impact, not by noise. These are all cognitive operations, and this assessment measures how reliably a candidate performs them under time pressure.

### What it measures for this role

The assessment dimensions map directly onto the skills named in the role profile:

- **Problem management**: logical deduction and fault diagnosis questions test the candidate's ability to investigate patterns and trends, isolate root causes, and determine appropriate resolutions and preventative measures.
- **Data integration design**: constraint-based reasoning questions test whether the candidate can select the appropriate technologies to deliver resilient, scalable, future-proofed solutions when several options each satisfy some, but not all, requirements.
- **Data modelling**: pattern recognition and comparison questions test the candidate's ability to compare and align different data models, and to explain which models to use for which purpose.
...
- **Testing**: coverage-reasoning questions test how the candidate reviews requirements and specifications, defines test conditions, and identifies issues and risks.
- **Metadata management**: consistency-checking questions test a candidate's eye for anomalies in metadata repositories.
- **Data analysis and synthesis**: data-profiling questions test how quickly a candidate extracts meaningful signal from source system information.
...
The candidate is reviewing a target table specification against the source system contract before a build. The source contract states:
...
The target DDL the candidate's team drafted defines:
...
**Explanation:** Each approach fails exactly one mandatory requirement, and the question states all four requirements are mandatory. Logically, none qualifies. Options A, B and D each smuggle in an unstated value judgement - that some mandatory requirement can be quietly waived. In a timed cognitive test, the discipline is to reason strictly from the stated constraints, not from what might pragmatically be done in practice. Interestingly, this also reflects sound senior practice in data integration design: when no technology satisfies every stated constraint, the correct professional move is to surface the conflict explicitly - perhaps requirement 4 can be relaxed with training investment - rather than silently downgrading a requirement. Selecting resilient, scalable, future-proofed solutions starts with honest constraint analysis.

**Question 5 (moderate) - Fault diagnosis in a streaming system**

A streaming pipeline the candidate leads ingests events from an operational case-management system into an analytics platform. At 10:15 the candidate observes: consumer lag rising steadily since 09:30; producer publish rate unchanged from its normal profile; consumer CPU at 35% (normal); a schema registry change deployed at 09:25 adding a new optional field; error logs showing repeated deserialisation retries on one partition. What is the most probable root cause?
...
At 08:50 the candidate has four open issues and one hour before a cross-government publication deadline that depends on their pipelines:
...
**Explanation:** Total runtime is 55 minutes, of which step 2 accounts for 46 minutes - roughly 84%. Amdahl's reasoning applies: the achievable saving from optimising any step is bounded by that step's share of total time. Option A saves at most 2 minutes; options C and D save fractions of 5 and 4 minutes respectively. Option B attacks the dominant cost, and it attacks it structurally: 10 million individual database round-trips replaced by one bulk load and in-memory lookups typically reduces that step from minutes to seconds, because per-call network latency, not computation, dominates. Recognising the "N+1 query" anti-pattern and reasoning proportionally from a profile is exactly the duty the role profile names: optimise the code to ensure processes perform optimally. A strong candidate does the arithmetic first - the biggest number in the profile almost always identifies the right answer - then sanity-checks that the proposed fix plausibly removes most of that cost.

### Administration guidance

- **Give the candidate time to settle before scoring begins.** Use any ungraded practice items to remove interface unfamiliarity before the timed section starts.
- **State the time limit up front and hold it consistently.** With 15 to 30 minutes total, allow roughly 60-90 seconds per question, and apply the same limit and any flag-and-return facility to every candidate.
- **Watch for proportional reasoning on profile-based items.** A strong candidate finds the dominant term - the biggest cost, the largest anomaly - before doing detailed work (Question 12).
- **Watch for rule-bound answers on constraint-based items.** Where an item defines rules or constraints, the credited answer applies them exactly as written, not as pragmatic workplace judgement might suggest (Questions 4 and 11).
- **Note whether the candidate reads the question's exact wording.** "Which must be true" differs from "which is most likely"; "materially distinct" differs from "any missing case" - senior-level items turn on these qualifiers.

### Scoring guidance: common pitfalls to watch for

- **Bringing outside assumptions into rule-based questions.** Watch for candidates reasoning from how systems "should" behave rather than how the described system does behave.
- **Sinking time into one hard question.** A single multi-stage problem can consume five minutes while three easy questions go unanswered; check whether unanswered items cluster near one difficult question.
- **Speed without error checking.** A rushed wrong answer costs more than a slightly slower right one; for error-checking items in particular, check whether the candidate verified field by field.
- **Pattern-matching superficially.** Distractor options are engineered to look right at a glance (option A in Question 2, option B in Question 9); note whether the candidate's reasoning traces the logic fully before committing.
- **Ignoring the question's exact ask.** "Which must be true" differs from "which is most likely"; senior-level items turn on these qualifiers.

## Workplace job-specific numeric reasoning assessment

### About this assessment

A workplace job-specific numeric reasoning assessment evaluates a candidate's ability to analyse, interpret, and make sound decisions using numerical data. It deliberately mirrors the data-handling demands of the actual role: not abstract formulae, but the business mathematics of running data platforms - throughput figures, storage growth projections, cost tables, capacity calculations, availability targets, and performance metrics.

Typical format: a strictly timed online test, often allowing around one to two minutes per question, with an onscreen calculator provided or permitted. The emphasis is on interpretation over mental arithmetic: the challenge is extracting the right figures from tables, charts and multi-layered datasets quickly and accurately, then applying percentages, ratios, rates, and cost-benefit reasoning. Question inputs look like the artefacts a senior data engineer already handles: platform billing summaries, ingestion dashboards, capacity plans, and quality reports. As with the cognitive test, benchmark results against a norm group where available, and report both speed and accuracy.

For a senior data engineer this assessment matters because quantitative judgement is embedded in the role's leadership duties. Leading the build of data streaming systems means sizing consumers against event rates. Optimising code means reading a profile and reasoning proportionally about where the time goes. Leading database management means projecting storage growth against budget. Selecting resilient, scalable, future-proofed technologies means comparing cost models honestly, including break-even points. Presenting clear insights to colleagues - a named skill in the role profile - depends on getting the numbers right before communicating them, which is what this section tests.

### What it measures for this role

The dimensions of this assessment map to the specific skills in the role profile:
...
An onscreen calculator is assumed throughout. The explanations below always show the working, for use in scoring and debrief.

**Question 1 (easy) - Daily event volume**

A streaming platform ingests case events at an average rate of 1,250 events per second, sustained around the clock. Approximately how many events does it ingest per day?
...
**Explanation:** There are 86,400 seconds in a day (60 × 60 × 24). Multiply: 1,250 × 86,400 = 108,000,000, that is 108 million events per day. A quick estimation route: 1,250 × 86,400 is 1,250 × 86.4 thousand; 1,250 × 86.4 = 108,000, so 108,000 thousand = 108 million. Options A and C are magnitude errors - exactly the kind produced by mislaying a factor of ten under time pressure. When leading the build of data streaming systems, this conversion between per-second rates and daily volumes is the first arithmetic of any capacity conversation.

**Question 2 (easy) - Percentage improvement from optimisation**

A transformation job is optimised, reducing its runtime from 80 minutes to 62 minutes. What percentage reduction in runtime was achieved?
...
**Explanation:** Compound growth multiplies: 40 × 1.05⁶. Step by step: 1.05² = 1.1025; 1.05³ ≈ 1.1576; 1.05⁶ = (1.05³)² ≈ 1.1576² ≈ 1.3401. Then 40 × 1.3401 ≈ 53.6 TB. Option A (52 TB) is the simple-interest shortcut: 40 × (1 + 6 × 0.05) = 40 × 1.30 = 52 TB, which understates compounding. Option D applies only three months of growth. The difference between 52 and 53.6 TB looks small at six months but widens dramatically over a multi-year horizon - at 5% monthly, storage nearly doubles every 14-15 months. Distinguishing linear from compound growth is essential when assessing whether a solution is genuinely scalable and future-proofed, and when briefing finance colleagues on where the storage budget is heading.

**Question 4 (moderate) - Cost break-even between two platforms**

A candidate is comparing two managed database services for a new data product:
...
**Explanation:** Divide peak load by per-instance capacity: 6,000 ÷ 800 = 7.5. A partial instance cannot be run, so round up to 8. Then check the constraint: parallelism is capped by partition count, and 8 instances ≤ 12 partitions, so the configuration is feasible - option B. Option A forgets to round up: 7 instances give only 5,600 events per second of capacity, 400 short of peak, so lag would grow. Option C misapplies the partition constraint, which only bites when instances would exceed partitions. Option D wastes resources: 12 instances work but are not the minimum asked for. This two-step pattern - compute the raw requirement, then validate against a structural constraint - is exactly how the streaming systems this role leads get sized, and assessments reward candidates who remember the second step.
...
A candidate leads a database migration with a budget of £120,000. Committed costs so far: two contractors at £650 per day each for 60 days; interim platform costs of £2,500 per month for 6 months; and £4,800 of training for the team. What percentage of the budget remains?
...
**Explanation:** Contractors: 2 × £650 × 60 = £78,000. Platform: £2,500 × 6 = £15,000. Training: £4,800. Total committed: £78,000 + £15,000 + £4,800 = £97,800. Remaining: £120,000 − £97,800 = £22,200. As a percentage of budget: 22,200 ÷ 120,000 = 0.185 = 18.5%. Option C is the proportion spent, not remaining - always check which the question asks. Option B is what you get by misreading £22,200 as a percentage directly. Multi-component budget questions reward a methodical layout: list each component, total them, then perform the final comparison. As a senior engineer who builds teams to complete data integration services, presenting figures like these to service owners is routine, so the discipline of computing them cleanly under time pressure is doubly valuable.

**Question 9 (challenging) - Availability targets and error budgets**

A data service has a monthly availability target of 99.9%, measured over a 30-day month. This month the service has already been down for 28 minutes. How many more minutes of downtime can occur before the target is breached?
...
**Explanation:** First find capacity per unit of utilisation: 15,000 qps consumes 60% CPU, so each percentage point of average utilisation supports 15,000 ÷ 60 = 250 qps. At the 75% ceiling, the cluster safely sustains 250 × 75 = 18,750 qps. For 30,000 qps, compute the utilisation it would cause on the current cluster: 30,000 ÷ 250 = 120%, clearly beyond the ceiling. Nodes needed scale linearly: each node contributes 18,750 ÷ 6 = 3,125 safe qps, so 30,000 ÷ 3,125 = 9.6, rounding up to 10 nodes. Option A gives both figures correctly. Option C doubles the node count naively (6 × 2 = 12), ignoring that the current cluster runs below its safe ceiling and so has headroom already. Option D uses the 60% figure as the ceiling. Two-stage ratio problems like this reward writing down the intermediate quantity - safe qps per node - explicitly; it is the same arithmetic presented to platform architects when leading work on database management and making the case for scaling decisions before peak demand arrives.

### Administration guidance

- **Provide scrap paper or a notes area alongside any onscreen calculator.** Most errors in table-based items happen at extraction, not arithmetic; visible working helps at debrief.
- **Hold the per-question time budget consistently.** Around one to two minutes per question is standard; apply the same limit to every candidate.
- **Watch for estimation before calculation.** A rough order-of-magnitude check (Question 1's "about 100 million") lets a strong candidate discard implausible options instantly and catches slipped decimal points.
- **Note whether a candidate verifies by a second route when time allows.** As in Question 7, many quantities can be computed two ways; this habit prevents the most expensive errors and is worth remarking on at debrief.
- **Confirm the assessment measures reasoning, not calculator speed.** Score accuracy alongside pace.

### Scoring guidance: common pitfalls to watch for

- **Misreading scale qualifiers.** Confusing "per TB per month" with "per TB per year", or thousands with millions, silently invalidates an otherwise perfect calculation.
- **Averaging percentages without weights.** As Question 6 shows, the unweighted mean of rates from differently sized sources is almost always one of the distractor options.
...
- **Over-calculating.** Where options are far apart, estimation answers the question faster than exact arithmetic; this is not an error but may explain unanswered items elsewhere.
- **Time-sink questions.** One multi-stage problem can consume the time of four simpler ones; check whether unanswered items cluster near one difficult question.

## Workplace job-specific verbal reasoning assessment

### About this assessment

A workplace job-specific verbal reasoning assessment evaluates how well a candidate understands, analyses, and draws correct conclusions from written business information. The passages mirror the exact reading demands of senior data engineering in government: data-sharing policies, technical standards, incident reports, supplier documentation, and stakeholder emails. The defining discipline is fact-based logic: the candidate must answer based strictly on the text provided, setting aside their own professional knowledge, however expert it is.

Typical format: a series of dense passages, each followed by statements to classify as **True** (the passage logically supports the statement), **False** (the passage logically contradicts it), or **Cannot Say** (the passage provides insufficient information either way), plus some multiple-choice comprehension questions. Timing is tight - usually less than a minute per question - to simulate workplace deadlines. Benchmark scores against a norm group where available.

For a senior data engineer, this assessment matters more than it might first appear. The role profile puts communication at its head: communicating effectively with technical and non-technical stakeholders, hosting discussions in multidisciplinary teams with potentially difficult dynamics, and managing differing stakeholder perspectives. All of that rests on precise reading - of what a policy actually permits, what an architect actually committed to, what a specification actually requires. Misreading "should" as "must", or a proposal as a decision, causes real integration defects and real stakeholder friction. The Cannot Say category is especially relevant to this role's day job: recognising that a document does not answer a question - and saying so explicitly rather than assuming - is a mark of senior engineering judgement, and exactly what the role calls for when reviewing requirements and specifications and identifying issues and risks.

### What it measures for this role

- **Communicating between the technical and non-technical**: extracting the precise commitments, caveats, and asks from stakeholder correspondence, so they can be represented faithfully when advocating for the team externally.
...
Score each answer strictly from its passage, not from outside technical knowledge.
...
**Explanation:** The passage defines the minimum entry as exactly four items: "a plain-English description, a named data owner, an update frequency, and a retention period." Field-level descriptions sit in a separate sentence governed by "should also include" - the standard's vocabulary for a recommendation, not a requirement. The only stated barrier to discoverability is "Datasets lacking a complete minimum entry will not be discoverable", and this dataset's minimum entry is complete. So the statement is supported: True. The question turns entirely on the must/should distinction, which is load-bearing vocabulary in every technical standard applied when designing metadata repositories - and precisely the distinction taught when advising less experienced members of the team about metadata management.
...
**Explanation:** The passage says the platform team "reviews a sample of catalogue entries each quarter." A sample, by definition, is a subset; the passage gives no information about sample size, selection method, or coverage over time. Four quarterly samples might cumulatively cover every entry in a year - or might repeatedly draw from the same subset. The passage neither supports nor contradicts the claim of complete annual coverage, so the answer is Cannot Say. This is the classic shape of a Cannot Say item: the statement is plausible, even likely in a well-run organisation, and that plausibility is the trap. A candidate's own professional expectation of how sampling reviews ought to work must be set aside. The workplace parallel: when a specification says "records are validated", a senior engineer asks "which records, and when?" rather than assuming full coverage - the identical habit of mind.
...
- **Brief the candidate to suppress outside expertise before starting.** Explain that the question is whether the passage states or entails a claim, not whether the candidate's own professional knowledge says it is true - "where in the passage is that said?" is the working test.
- **Hold the time limit consistently.** Under a minute per question simulates workplace deadlines; apply the same limit to every candidate for a given round.
- **Watch for attention to signal words in a candidate's reasoning.** Must, may, should, all, some, unless, provided that, rather than: these small words decide answers, and senior-level items hide their logic in them.
- **Note whether a candidate scans for the governing sentence or reads passages in full first.** Under this time limit, reading the statement first and then scanning the passage for the relevant clause is the intended method.
- **Watch for attribution tracking.** Who says what - the author states, a respondent cites, a briefing predicts, a stakeholder understands - and senior-level items often turn on the difference; this is worth probing at debrief.

### Scoring guidance: common pitfalls to watch for

- **Importing outside knowledge.** The most common error for experienced professionals; watch for justifications that draw on real-world knowledge of DSAs or catalogues rather than the passage.
...
- **Treating explicit indeterminacy as Cannot Say.** When a passage states "no decision has been made", a claim that a decision went one way is False, not Cannot Say (Question 8); Cannot Say is for genuine silence.
- **Answering from one sentence.** Universal clauses elsewhere in the passage frequently close gaps that a single sentence appears to leave open (Question 2); check whether the candidate's reasoning considers the whole passage.
- **Poor time allocation.** Dense passages tempt complete, careful reading of every word before any question; targeted scanning is the intended method under this time limit.

## Workplace job-specific situational judgement assessment

### About this assessment

A workplace job-specific situational judgement assessment evaluates a candidate's decision-making, interpersonal skills, and professional conduct. Unlike logic or numeric tests, it presents realistic workplace dilemmas - short scenarios involving conflict, competing priorities, ethical tension, or operational pressure - and asks how the candidate would respond. There is usually no single objectively right answer; several responses may seem reasonable, but one aligns best with the values of the organisation and the demands of the role. For UK government digital and data work, those values are well documented: meeting user needs, transparency, collaboration across disciplines, accessibility and inclusion, and responsible stewardship of public money and public data.

Typical formats: ask the candidate to pick the **most effective** and **least effective** responses from a set, to **rank** all options from best to worst, or to **rate** each option's effectiveness independently. Timing is usually generous or untimed - the test measures natural judgement, not speed.

For a senior data engineer, this is arguably the assessment closest to daily reality. The role profile is explicit that the role holder supports and hosts discussions within a multidisciplinary team "with potentially difficult dynamics", manages differing stakeholder perspectives, acts as an advocate for the team externally, and builds teams to complete data integration services. None of that is solved by arithmetic. The scenarios below are drawn directly from those duties: governance shortcuts under deadline pressure, disagreements between architects and delivery managers, junior engineers needing development rather than rescue, and incidents that test commitment to transparency.

### What it measures for this role

- **Communicating between the technical and non-technical**: whether the candidate translates rather than lectures, de-escalates rather than wins, and keeps non-specialist stakeholders genuinely informed.
- **Managing differing stakeholder perspectives and difficult team dynamics**: whether the candidate surfaces disagreement productively, protects inclusion in discussions, and finds resolutions that respect legitimate concerns on all sides.
- **Problem management**: whether the candidate balances rapid restoration against root-cause discipline, escalates appropriately, consults specialists where required, and pursues preventative measures.
- **Data development process and testing**: whether the candidate holds quality and assurance standards under delivery pressure, and how they handle requests to cut corners.
- **Metadata management and mentoring**: whether the candidate develops less experienced colleagues rather than simply correcting them.
- **Data innovation and integration design**: whether the candidate weighs emerging technologies with evidence and proportionality rather than enthusiasm or conservatism alone.

### Practice questions

The response format is most/least effective selection or ranking, as marked on each scenario.

**Question 1 (moderate) - Governance shortcut under deadline pressure**

A policy team urgently needs case data joined with staffing data for a ministerial briefing due in three days. The data sharing agreement (DSA) covering this join has not been signed. The policy lead, senior to the candidate, says: "Just run the join in your environment - nobody will mind, it's for the minister." Choose the MOST and LEAST effective responses.

- A) Run the join as asked; the ministerial deadline justifies it, and the paperwork can be completed afterwards.
- B) Refuse by email, copying the information governance team, stating that the request was improper.
- C) Explain that the join cannot run without the DSA, and immediately offer routes that can work: contacting the Information Asset Owners for urgent countersignature, checking whether the urgent-authorisation route applies, or preparing an anonymised or aggregated version that answers the briefing question without personal data.
- D) Tell the policy lead it will be looked into, then quietly let the deadline pass without acting.
...
The candidate is designing a new integration. The technical architect insists on rebuilding the flow on the strategic platform, adding roughly six weeks. The delivery manager insists on reusing the legacy flow to hit the quarter's commitment, and both have begun lobbying the candidate's engineers separately, creating tension in stand-ups. Choose the MOST and LEAST effective responses.
...
- C) Rewrite the code personally tonight so the release is safe and the deadline holds.
...
**Explanation:** Option B does three senior things at once. It treats the defect accurately - the specification is silent, so this is an issues-and-risks finding from the testing skill, not just a coding error. It develops the junior engineer through pairing, so the learning and the ownership stay with them - how teams get built to complete data integration services, per the role profile. And it makes the ship/hold decision transparently with the delivery manager, rather than unilaterally. Option A is least effective despite its kind intention: silent data loss is among the worst defect classes in data engineering because downstream users cannot see what is missing; shipping it knowingly, undocumented, subordinates data integrity and user needs to one person's feelings - and quietly fixing it later denies the engineer the chance to learn. Option C rescues the release but teaches nothing, burns out the reviewer, and signals that hard problems get taken away from juniors. Option D is technically defensible but developmentally poor - public, list-form criticism of a proud junior colleague damages the psychological safety that inclusion in a multidisciplinary team depends on. The strongest answers separate the standard (non-negotiable) from the person (supported).

**Question 4 (challenging) - Incident transparency with a service owner**

A streaming pipeline the candidate's team owns delivered stale data to a fraud-screening service for three hours yesterday because of a configuration change the team deployed without adequate testing. The fraud team has not noticed. The service owner has a board meeting tomorrow. Choose the MOST and LEAST effective responses.

- A) Say nothing; the impact window has passed, no events were lost, and raising it now would create noise before the board.
- B) Tell the service owner and the fraud team today: what happened, the window affected, what decisions might have used stale data, and the specific testing and monitoring changes being made so it cannot recur silently.
- C) Mention it to the service owner informally as a minor blip, without specifics, so they are covered if asked.
- D) Fix the testing gap first, then disclose next week once the fix can be shown to be in place.
...
A capable engineer on the candidate's team proposes migrating the team's batch integrations to a new open-source streaming framework they have been exploring. They are enthusiastic, technically persuasive, and have prepared a demo. The framework is genuinely promising but young: small community, no managed offering on the department's platform, and no one else in the department uses it. Choose the MOST and LEAST effective responses.
...
**Explanation:** The role profile includes understanding the impact on the organisation of emerging trends in data tools - which means neither reflexive adoption nor reflexive refusal, but structured evaluation. Option C converts enthusiasm into evidence: time-boxed, criteria-led, on a flow where failure is cheap, with findings shared beyond the team so the organisation learns once rather than five times. The criteria named are the ones that kill young frameworks in government settings - operability, skills, support, exit cost - exactly the resilient, scalable, future-proofed test from the integration design skill. Option A is least effective: committing a significant pipeline to an unproven framework with no support model gambles a production service on enthusiasm, and "probably the future" is not an engineering argument; if it fails, the cost lands on users and on the engineer the approval meant to encourage. Option B is safer but wasteful - it teaches the team's best people that initiative is unwelcome, which is corrosive to the team a senior engineer is supposed to be building. Option D outsources evaluation to unpaid personal time, which is poor inclusion practice and signals the idea is not taken seriously. Reward the behaviour, bound the risk.

**Question 6 (moderate) - Analyst misreading the data**

An analyst in another team publishes an internal dashboard using the candidate's case dataset, showing a sharp regional rise in case volumes. The candidate knows the rise is an artefact: a source system migration they led means one region's historical records were backfilled last month. The analyst's finding is already being discussed by policy colleagues. Choose the MOST and LEAST effective responses.
...
**Explanation:** Option A fixes the immediate error through the person responsible for it, preserves the analyst's standing by helping rather than exposing them, and - the distinctly senior move - fixes the systemic cause: the backfill was known but invisible in the metadata, so the catalogue entry gets improved. That last step is the metadata management skill doing its real job: supporting the end use of the data by people the role holder will never meet. Option B corrects the record but routes around the analyst, embarrassing them before their stakeholders and teaching them nothing; it should be the follow-up only if the analyst will not act and decisions are imminent. Option C spreads awareness but publicises an individual's error before giving them the chance to correct it - poor collaboration hygiene. Option D is least effective: a known-wrong finding is actively influencing policy discussion now, and waiting three weeks on procedural grounds lets misinformation compound daily. "Not my dashboard" is technically true and professionally wrong; presenting clear insights to support the end use of the data includes stopping data being misread when the role holder is the only person who knows why.

**Question 7 (challenging) - Ranking under incident pressure**

At 07:40, the overnight load feeding a statutory 09:30 publication has failed. Initial logs suggest an upstream schema change. The candidate's two platform specialists are both on leave today. Rank ALL FOUR actions from most to least effective as the FIRST move.

- A) Start debugging the failure personally and immediately, as the strongest engineer available, since time is short.
- B) Spend five minutes establishing facts - failure point, error, scope, feasibility of a rerun - then alert the publication team and the service owner with an honest early assessment and a next-update time, while the best available engineer starts diagnosis in parallel.
- C) Call one of the specialists on leave; the statutory deadline justifies the intrusion.
- D) Wait until the problem is fully understood before telling anyone, so as not to cause unnecessary alarm.

**Correct ranking: B, A, C, D.**

**Explanation:** B ranks first because it does the two things that matter in the first minutes of an incident: establish enough facts to speak accurately, and give affected stakeholders early, honest notice with a committed next update - transparency that lets the publication team activate their own contingencies (delay notices, fallback figures), which are theirs to invoke, not the data engineer's. Diagnosis proceeding in parallel loses almost nothing. A ranks second: energetic and personally competent, it fixes the technical problem while leaving the publication team blind; as a first move it also puts the senior leader heads-down where coordination should be. C ranks third: it might work, but as a first move it disrespects leave boundaries before establishing whether the on-shift team can cope - consulting specialists where required is the problem management skill, but it follows fact-finding, not fear. D ranks last: silence until full understanding is the classic incident anti-pattern; stakeholders lose their reaction time, and the eventual conversation is worse ("you knew at 07:40?"). Notice the general principle assessments reward: communicate early with what you know and when you will know more - never wait for certainty that may arrive after the deadline.

**Question 8 (moderate) - Pressure to skip reconciliation**

A migration the candidate leads has completed its final dress rehearsal. Reconciliation shows a small unexplained variance in financial totals for one table. The service owner, under pressure to hit a publicly announced go-live date, says: "It is 0.003% - accept it and let's go." The test strategy states any variance in financial totals must be investigated before go-live. Choose the MOST and LEAST effective responses.
...
- D) Say nothing in the meeting, then raise concerns informally with a line manager afterwards.

**Most effective: A. Least effective: B.**

**Explanation:** Being an advocate for the team externally - a named behaviour in the role profile - means correcting the record with evidence, through a process that lets decision-makers change course without losing face. Option A does this precisely: it validates the director's underlying concern (delivery pressure is real), replaces anecdote with data on a short timescale, and asks only that an irreversible decision wait for evidence - a request no reasonable forum refuses. The closing move matters most for the team: they heard the criticism, so they must hear the defence, or morale damage compounds. Option B is least effective: it accepts a false narrative into the programme's record, loses two engineers on the strength of it, and - worse - shows the team that no one defends them, which is how good engineers start leaving. Option C has the right facts and the wrong forum: a from-memory rebuttal that blames the director's supplier turns a resourcing discussion into a public contest the relationship may not survive, and any misremembered figure destroys credibility. Option D preserves harmony that is not needed at the cost of influence that is - the line manager was not in the room and cannot un-say what was said there. Evidence, delivered calmly, in a process: that is senior advocacy.

**Question 11 (moderate) - The metadata shortcut**

Mid-sprint, the candidate discovers the team has been publishing new datasets without catalogue entries for six weeks - "we'll backfill the metadata when the delivery pressure eases", says the team's most senior engineer, who took the decision while the candidate was focused on the migration. Downstream analysts have begun asking what the new fields mean. Choose the MOST and LEAST effective responses.
...
- **Check who owns the decision.** Many scenarios hinge on governance: variances belong to migration boards, contingency choices to publication teams, funding to planning rounds. Options where the candidate quietly takes or dodges someone else's decision should generally score lower.
- **Score "most" and "least" separately.** The least effective option is often easier to identify - look for passivity, concealment, or self-protection - and use that to sharpen judgement of the best.
- **Debrief on both audiences.** A senior data engineer answers upwards (service owners, boards) and downwards (their team) simultaneously; the debrief is a good place to check whether a candidate's reasoning considered both.
...
- **Selecting passive options.** Waiting, deferring to forums weeks away, handing the problem back, or fixing things quietly alone - these consistently rate poorly because they delay action, dodge ownership, or hide problems.
- **Ignoring the people in people problems.** Options that are technically correct but publicly humiliate a colleague, or bypass the person responsible, lose marks on collaboration and inclusion.
...
- **Missing the dual audience.** The strongest options usually communicate honestly both upwards and downwards, as in the incident and advocacy scenarios; note where a candidate's answer addresses only one.

## Conclusion

You have now worked through a complete set of assessment material built around the senior data engineer role: the cognitive assessment's pattern recognition, fault diagnosis, and constraint-based reasoning; the numeric assessment's throughput, capacity, cost, and break-even arithmetic; the verbal assessment's disciplined reading of policies, standards, and stakeholder correspondence; and the situational judgement assessment's dilemmas of governance, transparency, team dynamics and advocacy. Every question is built from the real substance of the role - the data flows it designs, the streaming systems it leads, the code it optimises, the databases it manages, and the multidisciplinary relationships it navigates.
...
Debrief every candidate, whatever the outcome, using the explanations in this document to talk through a sample of their answers. This is also where a candidate can raise fair questions about an item's clarity, and where genuine two-way learning happens regardless of the outcome.
