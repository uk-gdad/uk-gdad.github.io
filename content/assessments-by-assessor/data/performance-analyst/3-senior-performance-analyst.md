...
This document is a practical companion for the assessor administering a workplace job-specific psychometric assessment to a candidate for the senior performance analyst role — a line manager, a panel, or a reviewer running a structured assessment as part of hiring or promotion into this role level. It covers the four assessment types most commonly used for analytical roles at this level: cognitive ability, numeric reasoning, verbal reasoning, and situational judgement.

Psychometric assessments of this kind are structured, standardised exercises that measure how a candidate thinks, reasons, and decides. Rather than testing what a candidate already knows about a specific tool or dataset, they measure the underlying capabilities that make someone effective day to day: spotting patterns in performance data, drawing sound conclusions from dense policy documents, working accurately with percentages and ratios under time pressure, and choosing wise courses of action when stakeholders pull in different directions. Employers use them because, administered consistently, they are objective, fair, and strongly predictive of on-the-job performance — and because, for a role like this, they mirror the real work remarkably closely. Administering it fairly is the assessor's responsibility: present the same items under the same time limit to every candidate assessed for this role level, and hold the worked answers and explanations back as an answer key until scoring and debrief.

Why does this assessment matter for this role specifically? A senior performance analyst leads the development of performance measurement frameworks and Key Performance Indicators (KPIs), carries out increasingly complex analysis, assures the quality of data and analysis, and turns complex data into compelling, clear and actionable stories for stakeholders. The role may also manage other performance analysts, contribute to the performance analysis community across the organisation, and forge links with other analytical disciplines. Every one of those responsibilities draws directly on the capabilities this assessment measures.

This document is organised into four main assessment sections, each following the same structure:

- **About this assessment** — what it measures, why it matters for this role, and the typical format and timing.
- **What it measures for this role** — an explicit mapping to the GDAD skills for a senior performance analyst.
- **Practice questions** — realistic, role-specific items with full scenarios, answer options, correct answers, and worked explanations, forming the answer key.
- **Administering this section** — how to run the section fairly and consistently.
- **Scoring guidance** — patterns to watch for when marking and debriefing.

How should the assessor use it? Present the items under the timing given, using the same items and conditions for every candidate assessed at this role level. Keep the worked answers and explanations out of the candidate's sight until scoring and debrief is complete. After scoring, use the explanations as the basis of a debrief with the candidate — what a strong answer showed, and where a weaker one fell short — rather than simply reading out a total. Keep a record of what was administered and how it was scored, so the process is repeatable and defensible across candidates. This assessment is one input among several into a hiring or promotion decision, never the sole basis for one — see the advisory below.
...
A workplace job-specific cognitive ability assessment is a targeted psychometric test that measures the exact mental processes needed for this role — not abstract intelligence, but applied thinking. Unlike a generic IQ test, it uses practical workplace scenarios and data formats drawn from the everyday work of a senior performance analyst: dashboards, KPI frameworks, analytics reports, data pipelines, and service performance reviews. The purpose is to predict how well a candidate will reason, prioritise, diagnose, and solve problems when confronted with the real artefacts of the job.

Typical format and timing: assessments of this kind are usually delivered online and timed, most commonly 15 to 30 minutes for 20 to 30 questions. Consider offering short, ungraded practice questions first so the candidate can get comfortable with the interface, and apply the same allowance to every candidate. Score objectively: compare a candidate's results against a benchmark or norm group, and record a breakdown of speed versus accuracy rather than a single number.

For a senior performance analyst, the cognitive assessment matters because the role is fundamentally about structured thinking under complexity: designing measurement frameworks that must be logically coherent, diagnosing why a KPI has moved — separating genuine service change from tracking faults, seasonality, or data quality issues — checking other analysts' work for errors before it reaches decision-makers, and prioritising competing analytical requests from multiple teams. A cognitive assessment compresses those daily demands into a short, standardised exercise.

### What it measures for this role

Each cognitive dimension maps directly onto the GDAD skills defined for this role:

- **Pattern recognition and logical deduction** map to the **Analysis and insight** skill: understanding and helping teams apply a range of techniques to analyse data and provide insight, and applying innovative approaches to resolve problems. Spotting that a weekly completion-rate series has a structural break, or deducing which pipeline stage introduced an anomaly, is pattern recognition in action.
- **Error checking** maps to the **Quality assurance of data and analysis** skill: specifying how data should be cleansed and prepared, bringing data together from different sources, and communicating the limitations of data. As a technical specialist senior performance analyst, the role is responsible for the accuracy and quality of data and analysis — error checking is the cognitive core of that responsibility.
- **Prioritisation and problem solving** map to the **Understanding analysis across the product life cycle** skill: recognising when to move forward and when to stop, recognising the appropriate deliverables, and identifying which tools and techniques should be used at each stage.
- **Systems thinking** maps to the **Technical understanding** skill: showing a deep understanding of technical concepts and how they fit into the wider technical landscape, including the limitations of digital technology.
- **Framework logic** maps to the **Performance measurement** skill: leading the development of KPI frameworks and turning business goals into tangible performance measures — which demands rigorous deductive reasoning about what a metric can and cannot tell you.

### Practice questions

**Question 1 (easy) — Pattern recognition in a KPI series**

A candidate is reviewing weekly digital take-up figures for a licence renewal service. The percentage of renewals completed online over eight consecutive weeks is: 62%, 64%, 66%, 68%, 70%, 72%, 61%, 63%. Assuming the underlying pattern before week 7 continues in parallel after the disruption, which is the most reasonable expectation for week 9?
...
**Explanation:** The framework's only prohibition on board reporting is that "no health metric may be reported to the programme board". If the KPI is also a health metric, reporting it would breach that rule. Option A is compliant ("at least one" permits more). Option C is compliant for the same reason. Option D concerns frequency, about which the framework says nothing. This is a pure deduction exercise: apply the stated rules exactly, adding nothing. Leading the development of performance measurement frameworks depends on this kind of precise rule-application to keep governance clean.

**Question 3 (moderate) — Error checking a data pipeline output**

A candidate has specified a cleansing rule: "Remove sessions with duration under 2 seconds; deduplicate on session ID; then join to the survey table on session ID." An analyst on their team runs the pipeline and reports: raw sessions 84,200; after duration filter 79,950; after deduplication 81,100; after join 61,300 rows. Which figure is logically impossible?
...
**Explanation:** Deduplication can only remove rows or leave the count unchanged; it can never increase a row count. Going from 79,950 to 81,100 is therefore impossible if the steps ran in the stated order. The duration filter reducing 84,200 to 79,950 is plausible (about 5% short sessions). The join reducing rows to 61,300 is plausible if it is an inner join and not every session has a survey response. The likely real-world causes are steps run out of order, a join accidentally executed before deduplication, or a many-to-many join inflating rows. As the person responsible for the accuracy and quality of data and analysis, spotting "impossible numbers" quickly — before stakeholders do — is one of the most valuable habits this item is designed to surface.

**Question 4 (moderate) — Prioritisation under competing demands**

It is Monday morning. Four requests are in a candidate's queue:

1. The head of performance analysis needs the candidate's input to the quarterly strategy pack by Thursday.
...
3. A junior analyst managed by the candidate has asked for a 30-minute review of their first funnel analysis, due to be presented Wednesday.
4. Another department has asked the candidate to speak at a performance analysis community session in three weeks.
...
**Explanation:** The goal has two components: contact volume and avoidability. Option B normalises calls against service demand (so growth in users does not masquerade as failure) and segments by reason (so avoidable contact — "where is my application?" — can be isolated from unavoidable contact). Option A confounds demand growth with performance: total calls could rise simply because more people use the service. Option C measures workforce efficiency, not user behaviour. Option D measures digital uptake but says nothing about contact. Turning business goals into tangible, decision-ready measures is the heart of the performance measurement skill this role names, and the reasoning here — normalise, segment, align to the goal's actual wording — is the pattern a strong candidate should apply.

**Question 7 (hard) — Multi-step logical deduction across data sources**

A candidate brings together three data sources for a cross-government report:
...
- A) Both parts can be answered accurately from the three sources.
- B) Starts and completions can be estimated with known limitations, but abandoners' satisfaction cannot be measured at all from these sources.
- C) Abandoners' satisfaction can be measured from Source Z but not abandonment volume.
- D) No part of the question can be answered until Friday.

**Correct answer: B.**

**Explanation:** Work through each part deductively. Abandonment estimate: Source X gives starts and completions daily, so a mid-week estimate is possible — but it excludes the roughly 20% who decline cookies, so it is an estimate with a known coverage limitation, and Source Y cannot help mid-week because it updates on Fridays. Abandoners' satisfaction: Source Z surveys only users who completed, so by construction it contains zero abandoners; no analysis of Z can ever describe them. Option A ignores both limitations. Option C reverses the logic — Z is precisely the source that cannot describe abandoners. Option D is too pessimistic: partial, caveated answers are available now. The professional move, matching the quality assurance skill this role carries, is to give the director the caveated abandonment estimate immediately, state plainly that abandoners' satisfaction is unmeasurable with current instruments, and propose an exit survey or user research to close the gap — the answer a strong candidate should give.

**Question 8 (hard) — Spotting the flaw in an analytical argument**

A performance analyst on a candidate's team presents this argument: "Users who watch the tutorial video complete their application at 91%, versus 74% for users who do not. Therefore, making the video mandatory will raise overall completion to roughly 91%." What is the primary logical flaw?
...
**Explanation:** Test each criterion exactly as written. (i) Nine weeks of baseline exceeds eight — met. (ii) The requirement is the last 4 consecutive weeks at or above 65%: those are 66%, 67%, 68%, 69% — all pass; the 63% was five weeks ago and falls outside the window — met. (iii) The prohibition covers severity-1 issues only; the sole severity-1 issue is closed, and the open severity-2 issue is simply not covered by the criterion — met. Option A misreads the window; option B upgrades a severity-2 into scope; option D invents ambiguity where the rule is silent but precise. Note the professional nuance worth probing at debrief: meeting the letter of the criteria does not stop a strong candidate flagging the severity-2 issue in their recommendation — recognising when to move forward while communicating residual risk is exactly what the product life cycle skill describes.

**Question 10 (moderate) — Pattern recognition in segmentation**

A candidate segments a service's completion rate by channel and device. Desktop: 81%. Mobile: 58%. Overall: 74%. A stakeholder says: "Mobile is failing — fix mobile and overall completion will approach 81%." Roughly what share of traffic is mobile, and is the stakeholder's expectation arithmetically sound?
...
- Record both accuracy and time taken separately, so a candidate who works fewer items carefully is not conflated with one who rushes through more of them with errors.
- If a candidate is visibly stuck on one item, let the session continue rather than intervening; note it for the debrief.
- At debrief, use the explanations to talk through what a strong answer showed and where a weaker one fell short, rather than only giving a score.

### Scoring guidance

- Watch for a candidate treating this as a generic IQ test rather than reading the scenario carefully — the pipeline order, the framework wording — since candidates who slow down to read outperform candidates who pattern-match superficially, and this difference is worth noting.
- Watch for a candidate selecting a "half-right" paired option (Question 12) that bundles one correct element with one wrong one, rather than evaluating every element.
- Note whether a candidate's first hypothesis for a dramatic number is a measurement artefact rather than a real-world change (Questions 3, 5, 11) — this is one of the more diagnostic patterns for this specific role.
- Note the balance of speed versus accuracy in the candidate's results; wild guessing to finish and unfinished perfectionism are both worth flagging as different patterns at debrief.
- If the platform used is adaptive, be aware that increasing difficulty indicates the candidate is doing well, not struggling — do not let apparent difficulty bias scoring impressions ahead of the actual results.

## Workplace job-specific numeric reasoning assessment

### About this assessment

A workplace job-specific numeric reasoning assessment tests whether a candidate can reach a defensible conclusion from data under time pressure. As a senior performance analyst the data is exactly what the role works with daily: service volumes, conversion and completion rates, satisfaction scores, cost per transaction, and the figures other people produce and ask the postholder to interpret.

The format is typically fifteen to twenty-five minutes with a calculator permitted, presenting tables and charts followed by multiple-choice items — apply the same allowance and permissions to every candidate. Some questions are computational; others ask what the data does and does not support, and the second kind carries more weight at this level, because much of the job is judging other people's arithmetic.

Three habits matter particularly, and are worth watching for when scoring.
...
The third is that a senior performance analyst increasingly carries out complex analysis and supervises others doing the same, which means being quick enough at the underlying arithmetic to notice when a figure is wrong by an order of magnitude. The magnitude check is worth more than the precise answer.
...
**Explanation:** 105,830 completions from 136,000 starts = 77.8%, so about 77.6% as the nearest option. The unweighted mean of the four channel rates is (71 + 88 + 92 + 79) / 4 = 82.5%, which is option A and the standard trap — it treats the 9,000 paper transactions as equal in weight to the 84,000 digital ones. A candidate who picks A rather than recomputing from totals is showing a common and consequential error, since weighting matters whenever groups of different sizes are aggregated.
...
**Explanation:** Statistical significance tells you an effect is distinguishable from zero; it says nothing about whether the effect is worth having. At 60,000 users a test has power to detect differences well below the threshold at which anyone should act — here, 0.3 points on the digital channel's volume would be roughly 250 additional completions a quarter, which may or may not repay the work. Option B separates the two questions and names the criterion. Option C misunderstands what significance means. Option D asks for more of what is already sufficient. A candidate who reaches option B unprompted is demonstrating exactly the judgement this level of role requires.
...
- B) The series moves between 19,200 and 21,300 with no clear direction — the first four average 20,150 and the last four 20,175, a difference well inside the month-to-month variation — so completions are essentially flat and the apparent rise depends entirely on which months are compared.
...
- Hold every candidate to the same time limit (around fifteen to twenty-five minutes is typical for the set above), and use the same table and items for every candidate compared for a given decision.
- Keep the worked answers out of the candidate's sight until scoring; they are the answer key.
- At debrief, use one or two of the "what the data supports" items (Questions 9, 10, 12, 13) to explore the candidate's reasoning directly — these carry more weight at this level than the purely computational items, since much of the job is judging other people's arithmetic and interpretation.

### Scoring guidance

- Watch for a candidate taking the unweighted mean of rates across groups of very different sizes (Question 4) rather than recomputing from totals — a common and consequential error worth flagging.
- Note whether a candidate compares channel completion rates without noting who self-selects into each channel (Question 9) — this is one of the more diagnostic distinctions for this role.
- Note whether a candidate reports a rate improvement without the volume behind it, and whether they separate statistical significance from practical importance (Question 11) rather than treating "significant" as "important".
- Note whether a candidate compares the halves of a noisy time series rather than just its first and last points (Question 12), and whether they report a survey estimate against its interval rather than as a bare point (Question 13).
- Note whether a candidate applies a per-completion cost to completions rather than to starts (Question 5) — a small but frequent error worth checking specifically.

## Workplace job-specific verbal reasoning assessment

### About this assessment

A workplace job-specific verbal reasoning assessment tests whether a candidate can determine exactly what a piece of written material establishes. As a senior performance analyst the material is metric definitions and measurement frameworks, data quality documentation, tool and supplier terms, research reports, and the summaries other people write about the team's analysis.

The commonest format presents a passage followed by statements to be marked True, False or Cannot Say. True means the passage establishes it, False means it contradicts it, and Cannot Say means it settles neither — regardless of what the candidate may separately know professionally.

A candidate's expertise is the main hazard here. They may well know how a metric is usually calculated and what a movement usually means, while the passage in front of them describes this particular case — and at this level, a candidate's reading frequently becomes their team's reading, which raises the stakes of reading it correctly.

Three specifics matter particularly, worth bearing in mind when scoring.

The first is that claims are routinely attributed rather than established. "The supplier states", "the review found", "the report recommends" — each tells the reader the source and nothing about whether it is true. A senior performance analyst supervises others' analysis, so spotting the difference is something the candidate needs to model as well as practise.

The second is that measurement definitions turn on small words, and organisations vary them. This role names setting up performance measurement frameworks, which means the postholder is the person whose definitions other people will read this closely.

The third is that a report which names what it did not examine is telling the reader where the evidence stops, and the unexamined question is frequently the important one.
...
**Explanation:** The framework says such analysis "should be reviewed" in that case. "Should" is a strong expectation permitting justified departure, and the statement's "must" overstates it — in a passage that uses "must" three times elsewhere, the choice of "should" here is clearly deliberate. Practically, this means a senior analyst can require this locally and the framework does not require it of everyone, which is worth exploring at debrief if a candidate marks this True.
...
**Explanation:** The review "notes that no data was available on users who abandoned both channels". Those people attempted the service and are absent from the analysis entirely, so the findings do not cover the full population — the passage contradicts the statement by its own admission. This is the most important sentence in the passage: the group missing from the data is the group for whom the service failed completely, and any conclusion about how well the service works that excludes them is measuring the experience of the people it already worked for. A candidate who identifies this unprompted is showing exactly the reading this role requires.

### Administering this section

- Use the same three passages and thirteen items for every candidate assessed at this role level, and hold every candidate to the same time limit.
- Keep the worked answers out of the candidate's sight until scoring.
- At debrief, focus discussion on the "Cannot Say" items (Questions 6, 10, 12, and 13) — these are usually the most instructive, since they are where a candidate's professional assumptions are most likely to override what the passage actually says, and at this level that reading frequently becomes a team's reading.

### Scoring guidance

- Note whether a candidate reads "should be reviewed" as a requirement (Question 2) rather than distinguishing it from "must" — a distinction that matters directly to how a senior analyst would apply the framework in practice.
- Note whether a candidate accepts a supplier's statement about their own change (Question 6) as established rather than as an attributed claim requiring independent testing.
- Note whether a candidate treats a sampled segment figure as precise (Question 7), or attributes an outcome difference to a channel without accounting for the populations differing (Question 10).
- Note whether a candidate reads end-of-transaction satisfaction as representing all users of a channel (Question 11) rather than only those who completed.
- Note whether a candidate treats a recommendation as an established remedy (Question 12), and whether they catch the sentence naming the group absent from the data (Question 13) — this last item is one of the strongest signals in the section for this specific role.
...
The first is that the role summary names delegating, automating or upskilling others "where possible and appropriate". Doing the analysis oneself is nearly always faster today and is the reason a team's capacity never grows.

The second is that the role is responsible for the accuracy and quality of data and analysis *and for how they are used*. A correct number being misread is the analyst's problem, which means intervening at the point of interpretation rather than only at the point of calculation.

The third is that a senior performance analyst is increasingly the person who says the unwelcome thing. Analysis that only ever confirms what people hoped is analysis nobody needs, and the credibility to be believed on the difficult findings is built on the ordinary ones.

### What it measures for this role

- **Delegation and upskilling** maps to this role level's description.
...
**Question 1 (easy) — A piece of analysis that could be done faster**

An analyst supervised by a candidate is working on something the candidate could complete in an hour. It will take them a day. What is the most effective response?

- A) Do it personally; the team is busy.
- B) Let them do it, checking in early enough to correct a wrong direction — because an hour saved today costs the same hour every time the task recurs, and this role names upskilling others where possible.
- C) Do it together.
- D) Let them do it and review the result.

**Correct answer: B. Least effective: A**

**Explanation:** The arithmetic of delegation is that doing it yourself is faster once and slower forever. Option B adds the safeguard that makes it work — an early check catches a wrong direction before the day is spent, which is the main risk of delegating. Option A is least effective as a habit: it keeps the capability concentrated in one person and is precisely what this role level asks the postholder to stop doing. Option C is a good option for genuinely novel work and expensive for a routine task. Option D is fine and reviews only after a day has gone.

**Question 2 (easy) — A number being misused**

In a meeting, a service owner describes a candidate's analysis as showing that a change worked, when it shows an association with a confounding factor. What is the most effective response?
...
**Explanation:** The role's accountability covers how analysis is used, and a misinterpretation stated in a meeting becomes the room's shared understanding within minutes. Option A gets the correction in without a confrontation — "worth being clear what this can and can't tell us" does it. Option C is least effective: allowing a causal claim the analysis does not support is how a team's work ends up backing a conclusion the analyst would not defend. Option B reaches fewer people than were in the room. Option D achieves the right outcome in a form that invites defensiveness.

**Question 3 (moderate) — An analyst's method a candidate doubts**

An analyst supervised by a candidate has used an approach the candidate thinks is wrong, and they have clearly thought about it. What is the most effective response?

- A) Ask what led them to it and what alternatives they weighed, then set out the concern with reasons — because they may have hit a constraint not visible to the candidate, and if not, the reasoning is what transfers.
- B) Tell them to use the approach the candidate would have used.
- C) Accept it; it is their analysis.
- D) Rework it personally before it goes out.

**Correct answer: A. Least effective: D**

**Explanation:** Option A costs a conversation and buys the possibility that the analyst is right plus an analyst who can make the judgement unaided next time. Option D is least effective — it removes the work, teaches nothing, and tells the team their analysis becomes the supervisor's own when there is disagreement. Option B transfers a conclusion without reasoning, which works once. Option C declines the assurance responsibility the role holds.

**Question 4 (moderate) — Automating a recurring task**

A team spends significant time each month on a recurring report. Automating it would take about two weeks. What is the most effective response?
...
- A) Take the challenge seriously and check it properly, then report the outcome — whether it confirms, qualifies or overturns the original finding — because the challenge may be right and treating it seriously is what makes the confirmation credible.
...
**Explanation:** A challenge from an interested party may still be correct, and checking costs little compared with the credibility of being able to say the check was done. Option A leaves the analyst in the strongest position whichever way it turns out. Option C is least effective: softening the presentation of a finding believed to be right, under pressure, changes the analysis by another route and is how findings quietly stop meaning anything. Option D does the same more subtly. Option B may be correct and forgoes the check that would demonstrate it.

**Question 8 (harder) — A skills gap in a candidate's team**

A candidate realises that only they can maintain the pipeline several of the team's reports depend on. What is the most effective response?

- A) Document it thoroughly.
- B) Have an analyst work through real maintenance with the candidate — a change, a failure — because what is done without thinking is invisible to the person doing it, and documentation systematically misses exactly that.
- C) Simplify the pipeline.
- D) Ask for a second specialist to be recruited.

**Correct answer: B. Least effective: D**

**Explanation:** This role names ensuring the team has the required skills, and tacit knowledge transfers through supervised practice rather than prose — a person cannot document what they do not know they are doing. Option D is least effective as the primary response: it is slow, uncertain, expensive, and ends with the knowledge in two heads rather than distributed. Option C is often the best long-term answer and does nothing about the next quarter. Option A is worth doing alongside B and is reliably incomplete alone.

**Question 9 (harder) — An error already published**

A candidate discovers an error in analysis their team supplied three weeks ago, which has informed a decision. What is the most effective response?
...
- At debrief, discuss not just which options a candidate chose but the reasoning they gave — several of these scenarios are defensible from more than one angle, and the reasoning is where the signal is.

### Scoring guidance

- Watch for a candidate who would do the analysis themselves rather than delegate with an early check (Question 1), or automate unsanctioned in spare time rather than making the business case (Question 4) — both speak to how the candidate would scale a team's capacity.
- Note whether a candidate would correct a misinterpretation in the room where it was made, or only in a written report afterwards (Question 2).
- Note whether a candidate treats a methodological challenge from an interested party as worth checking properly (Question 7), and whether they would front a team's own error to the people affected rather than delegate the disclosure (Question 9).
- Note whether a candidate proposes transferring tacit knowledge through supervised practice rather than documentation alone (Question 8) — a distinction that matters for how the candidate would actually close a single-point-of-failure risk.
- Note whether a candidate treats distrust of their own past approach as evidence to investigate honestly (Question 12), rather than as a comprehension problem to explain away — this is one of the more revealing items in the section for seniority and self-awareness.

## Conclusion

This document has given the assessor four assessments pitched at senior performance analyst, and once scoring is complete it is worth drawing the threads together rather than reading each section in isolation.

The cognitive section tests reading data structurally — finding the cause behind a pattern, following a definition through its conditions, and separating a measurement artefact from a change in the world.

The numeric section returns repeatedly to one discipline: converting rates into counts before interpreting them. A completion rate is a proportion of whoever turned up, and 71% of 84,000 people is a different fact from 92% of 31,000. The channel-shift questions made the trade-off explicit — roughly £61,600 saved per quarter and around 650 people who no longer complete — and a candidate who reaches for that pair of numbers unprompted is demonstrating exactly what this role exists to put in front of a decision-maker.

The verbal section tests reading definitions, frameworks and attributed claims exactly. "Should" is not "must". A supplier's statement about their own change is a claim. And the sentence noting that no data was available on people who abandoned both channels is the most important line in that passage, because it names the group for whom the service failed entirely.

The situational judgement section tests the shift this level represents: delegating, automating and upskilling rather than doing everything personally; being accountable for how analysis is used as well as whether it is right; and increasingly being the person who says the thing nobody wants to hear. The strong answers checked a hostile methodological challenge properly, corrected a misreading in the room, and treated distrust of a measure the candidate had introduced as evidence rather than as a comprehension problem.

When scoring, weigh the pattern across all four sections rather than any single item, and record where a candidate's answers diverged from the key and why — that record is what makes the debrief useful and the decision defensible if it is later questioned. Use the same items, time limits and scoring key for every candidate assessed at this role level, and treat this assessment as one input among several: pair it with interview, references, or evidence of past work rather than deciding on it alone. If one thread runs through all four sections, it is that this role's value is in being right about uncomfortable things — anyone can report a number that confirms what people hoped, and the credibility to be believed when a finding is unwelcome is built slowly and spent quickly, which is worth watching for throughout a candidate's answers.
