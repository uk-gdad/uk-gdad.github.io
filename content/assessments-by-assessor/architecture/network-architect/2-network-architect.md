...
This document is written for you: an assessor administering a workplace job-specific psychometric assessment to a candidate for network architect within the UK Government Digital and Data profession. Whether you are running an internal progression board, sifting candidates for a new post, or building a consistent evaluation process across a hiring round, this document gives you structured, role-specific material to present to a candidate, together with the answer key you need to score it.

Psychometric assessments are structured, standardised exercises that measure the thinking skills behind good job performance. They are widely used across the Civil Service and the wider public sector because they are fair, evidence-based, and focused on capability rather than background. For a network architect, they matter because the role sits at the intersection of deep technical judgement and broad stakeholder engagement: designing networks and specifications that support business strategies, researching new technologies and assuring them appropriately, agreeing service level agreements with third party vendors, co-ordinating programme activities, and coaching associate network architects. Every one of those duties draws on the four ability areas covered in this document.

This document is organised into four main assessment sections, each of which mirrors a common workplace psychometric test:

1. **Workplace job-specific cognitive assessment** - pattern recognition, logical deduction, error checking, prioritisation, and problem solving using the artefacts a network architect really handles: network diagrams, IP addressing plans, design review comments, and change schedules.
...
4. **Workplace job-specific situational judgement assessment** - realistic dilemmas involving engineers, vendors, associate architects, security teams, and programme stakeholders, where the candidate judges which responses are most and least effective.

Each section explains what the assessment measures, maps it explicitly to the skills in the role profile (governance and assurance, IT infrastructure, systems design, systems integration, and troubleshooting and problem resolution), provides a generous set of practice questions with full worked explanations held back as your answer key, and closes with notes on administration and what marks out a strong or weak response.
...
A workplace job-specific cognitive ability assessment measures the exact mental processes needed for the role, using practical workplace scenarios rather than abstract puzzles. Unlike a generic IQ test, the questions mimic the everyday tasks and data formats of a network architect's job: reading network topology descriptions, spotting inconsistencies in design documents, deducing the cause of a fault from a set of symptoms, and deciding what to do first when several demands compete.

Administer this section timed, usually 15 to 30 minutes online, and score it objectively against the worked answers below. Some platforms adapt question difficulty to a candidate's answers, and recording a breakdown of speed versus accuracy, rather than a single score, is worthwhile. Most providers offer short, ungraded practice questions first so the candidate can get used to the interface - it is worth giving the candidate time to use these.

This assessment matters for the role because network architecture is fundamentally a reasoning discipline. Designing a system characterised by medium levels of risk, impact, and business or technical complexity requires structured logical deduction. Reviewing the systems designs of others to check technology selection and efficient use of resources is error checking. Breaking a problem down into its component parts to identify root causes is exactly what a cognitive test measures - just with routers, firewalls, and cloud networks instead of shapes and sequences.

### What it measures for this role level

- **Pattern recognition** maps to the **IT infrastructure** skill: recognising recurring structures in LAN, WAN, wifi, data-centre LAN and software-defined network designs, spotting the addressing scheme behind a set of subnets, and seeing where a proposed cloud network departs from an established pattern.
- **Logical deduction** maps to the **troubleshooting and problem resolution** skill: breaking a problem into component parts, ruling causes in or out from evidence, and diagnosing faults across different technology capabilities.
- **Error checking** maps to the **systems design** skill: reviewing the designs of others to ensure appropriate technology selection, efficient use of resources, and correct integration of multiple systems and technologies.
- **Prioritisation and planning** maps to programme duties: planning, directing and co-ordinating activities, engaging stakeholders, and sequencing an integration build so dependencies are respected.
- **Rule application** maps to the **governance and assurance** skill: applying design standards and assurance processes consistently, and identifying where a document breaches an agreed rule.

### Practice questions

**Question 1 (easy) - Pattern recognition in an addressing plan**

A department allocates site subnets in a consistent pattern: Site A uses 10.1.0.0/16, Site B uses 10.2.0.0/16, Site C uses 10.3.0.0/16. Within each site, user VLANs take the .10 to .19 third octet, printers take .20 to .29, and building management systems take .30 to .39. An associate architect proposes 10.4.25.0/24 for a new user VLAN at Site D.
...
**Explanation:** Sites follow 10.N.0.0/16 in sequence, so Site D correctly takes 10.4.0.0/16 - the second octet is fine. However, the third octet 25 falls within the .20 to .29 range, which the pattern reserves for printers. A user VLAN should use .10 to .19, for example 10.4.12.0/24. This is exactly the kind of quiet inconsistency a network architect catches when reviewing the designs of others: nothing is technically broken, but the standard is not being applied, which will confuse operations teams later.

**Question 2 (easy) - Logical deduction from monitoring evidence**

A network management solution reports: all users at the Leeds office have lost access to a cloud-hosted service; users at Manchester and Bristol can reach it normally; Leeds users can still reach on-premises services and the internet generally. The cloud service provider reports no incidents.
...
The candidate is reviewing a high-level design produced by a supplier. It contains these four statements:
...
Three third party vendors support parts of a network. The contracts state:
...
A priority 1 incident occurs at 02:00 on the campus wifi. Which is the fastest contractual response that can be relied on?
...
**Explanation:** The plan destroys its own safety net: once the old firewalls are decommissioned and wiped at 21:30, "rollback to the old firewalls" at 22:30 is impossible. Decommissioning must move to after the success declaration - typically days later, after a stability period. A is wrong because freezing other changes during a major migration is good practice, reducing confounding variables if troubleshooting is needed. B is backwards - backups must precede risky work. D is wrong because a cutover that has not happened cannot be integration-tested. Spotting the step that invalidates a later step is classic error checking, and precisely what a network architect does when reviewing implementation plans with engineers to ensure systems are practically designed.

**Question 9 (hard) - Root cause isolation across capabilities**

Users report that voice calls on the unified communications platform break up every weekday between 09:00 and 09:30, then recover. The candidate gathers these facts:
...
It is 09:00 Monday. Four items compete for the candidate's day:

1. A programme board at 14:00 needs the candidate's network design decision paper, currently 80% complete; without it, a procurement decision slips a month.
2. An associate network architect the candidate mentors has asked for an hour to walk through their first solo design before their 11:00 review.
...
**Explanation:** Options A, B and D are network technology domains a network architect is expected to understand at this role level. A service level agreement is a commercial and contractual construct - something agreed with third party vendors - not a technology. Classification questions like this appear simple, but under time pressure candidates often pick D because "communications" feels less network-like than the others. A candidate who slows down for two seconds and sorts by kind (technology versus contract), rather than surface familiarity, avoids the trap.

**Question 14 (hard) - Problem solving within change windows**

A change policy allows network changes only in these windows: Tuesday 22:00-02:00 and Saturday 20:00-06:00. A WAN migration needs three sequential steps: Step 1 takes 3 hours; Step 2 takes 5 hours and must start within 48 hours of Step 1 finishing; Step 3 takes 2 hours and must complete within 24 hours of Step 2 finishing. Steps cannot be split across windows.
...
**Explanation:** Check the durations against the windows first. The Tuesday window is 4 hours long, so Step 2 (5 hours) can never run on a Tuesday - that eliminates A and C immediately. Option B fits Steps 1 and 2 into Saturday's 10-hour window (3 + 5 = 8 hours), but then Step 3 must complete within 24 hours of Step 2 finishing; the next Tuesday window opens roughly three days later, far outside 24 hours - eliminated. Option D: all three steps total 3 + 5 + 2 = 10 hours, exactly filling the Saturday 20:00-06:00 window; run sequentially, every constraint is met - Step 2 starts immediately after Step 1 (well within 48 hours) and Step 3 completes immediately after Step 2 (well within 24 hours). The exam skill is elimination by hardest constraint first (Step 2's length), then verifying the survivor completely, including the tight 10-hours-in-10-hours fit. The workplace skill is identical: when planning and co-ordinating implementation activities, the binding constraint is usually one step's duration against one window, and finding it early saves redesigning the whole schedule later.

### Notes for the assessor

- **Look for fluency with real artefacts.** A candidate who has practised hunting a real (anonymised) design document for internal contradictions, unstated assumptions, and standard breaches tends to show the error-checking habit this section measures.
- **Listen for narrated elimination.** A candidate who narrates their deduction while troubleshooting - "it cannot be the WAN because... it cannot be the LAN because..." - is applying the systematic elimination the items reward, worth drawing out in debrief even if not required during the timed section.
- **Note whether dependency ordering is treated as routine.** A candidate who sketches build sequences and checks each step's prerequisites, rather than reasoning purely by feel, tends to answer items like Question 4 quickly and correctly.
- **Time the section at roughly one minute per question.** Accuracy under time pressure varies with rehearsal; unfamiliarity with a timed format alone can cost marks unrelated to ability.
- **Watch for careful reading of boundary conditions.** "Exceeds 90%" is not "reaches 90%"; "before procurement" is not "before build". Many wrong answers are correct answers to a slightly different question, and this is worth flagging in debrief.

### Signals of a weaker response

- **Answering from experience instead of the question.** A candidate's network expertise is real, but the correct answer must follow from the facts given; marking a scenario down in their head for a factor the text never raises (such as redundancy) is a sign of this.
- **Grabbing the smallest number or the strongest word.** In SLA-style logic questions, qualifiers (business hours only, data-centre only) change the answer; a candidate who reads every clause avoids this.
- **Time sink questions.** A candidate who gets stuck on one hard deduction instead of flagging it, answering the easier ones, and returning is showing weaker self-management - the section scores total correct answers, not heroism on one item.
- **Confusing "could be true" with "must be true".** Under pressure, plausible inferences can feel proven; a stronger candidate checks whether the text forces the conclusion.
- **Skipping any ungraded practice items offered.** If the platform provides them, encourage the candidate to use them - they remove interface surprises that can otherwise cost marks unrelated to ability.

## Workplace job-specific numeric reasoning assessment

### About this assessment

A workplace job-specific numeric reasoning assessment evaluates a candidate's ability to analyse, interpret, and make logical decisions using numerical data drawn from the real demands of the role. It is business mathematics, not abstract algebra: the inputs are tables, charts, budgets, capacity figures and performance metrics, and the operations are percentages, ratios, rates, conversions, and cost-benefit estimations. Provide or allow an onscreen calculator, because the section prioritises interpretation over mental arithmetic - the skill being measured is extracting the right numbers and combining them correctly, quickly, under a strict time limit.

For a network architect, numbers are everywhere. Agreeing SLAs with third party vendors means converting availability percentages into minutes of allowable downtime and checking penalty clauses. Planning and co-ordinating programme activities means budgets, day rates, and schedule arithmetic. Designing LAN, WAN, wifi, and cloud networks means bandwidth, throughput, capacity headroom, growth projections, port counts, and licensing. Reviewing others' designs for efficient use of resources is often a numbers-versus-requirements comparison. This section measures whether a candidate can do that quickly and accurately.

### What it measures for this role level

- **Capacity and utilisation arithmetic** maps to the **IT infrastructure** skill: bandwidth, throughput, port counts, wireless client densities, and data-centre LAN sizing.
- **SLA and availability calculations** map to the duty to **agree SLAs and engage with third party vendors**: converting "nines" into downtime minutes, calculating service credits, and comparing vendor offers.
- **Budget and cost comparison** maps to programme duties: comparing total cost across options, tracking spend against forecast, and evaluating cloud versus on-premises costs in **systems design** reviews.
...
**Explanation:** After one quarter: 6.8 + 0.4 = 7.2 Gbps, which exceeds 7.0 Gbps. So the threshold is breached after just one quarter. The arithmetic is trivial; the value of the question is the planning insight - procurement and change lead times for WAN and data-centre upgrades are often longer than a quarter, so "one quarter to breach" means the upgrade activity needed to start already. Assessments pair small numbers with realistic consequences to check that a candidate interprets, not just calculates.

**Question 4 (moderate) - Vendor cost comparison over a contract term**

The candidate is comparing two third party vendor offers for managed SD-WAN across 40 sites over 3 years:
...
**Explanation:** The shortfall is 99.5% − 99.13% = 0.37 percentage points. Only full 0.1-point steps count: 0.37 contains three full steps (0.3), not four. Credit = 3 × 5% = 15% of £22,000 = £3,300. Option C (£4,070) comes from applying 18.5% (treating 0.37 as 3.7 steps), and D from rounding 0.37 up to 0.4. The lesson for the vendor engagement duty: credit mechanisms hinge on exact wording like "each full 0.1 percentage point", and the arithmetic must follow the words. In negotiations, the 25% cap is also worth noting - it means credits stop growing once availability drops 0.5 points below target, a real design consideration when judging whether an SLA genuinely protects the business.

**Question 6 (moderate) - Wireless capacity ratio**

A new office floor will seat 360 staff. A wifi design standard assumes 2.2 devices per person and a maximum of 55 devices per access point for acceptable performance.
...
**Explanation:** Total devices: 360 × 2.2 = 792. Access points: 792 ÷ 55 = 14.4. It is not possible to deploy 0.4 of an access point, and 14 APs would carry 14 × 55 = 770 devices - 22 short. So round up to 15. Option A is the round-down trap; D pads without justification. Capacity questions in numeric tests almost always include a rounding decision, and for infrastructure the rule is: capacity requirements round up. In a real design, coverage (not just capacity) analysis and growth headroom would follow, but the question only asks for the standard's minimum.

**Question 7 (moderate) - Programme budget burn rate**

A network transformation programme has a £1.44 million budget for a 12-month delivery. At the end of month 5, spend to date is £684,000.
...
**Explanation:** Cycle 1 pass rate: 168 ÷ 240 = 0.70 = 70%. Cycle 3: 234 ÷ 260 = 0.90 = 90%. Improvement: 90 − 70 = 20 percentage points. Note the units: percentage points, not percent change (the relative change would be 20 ÷ 70 ≈ 28.6%). Option D is the cycle 3 rate alone; option C is a decoy for misreading. Supporting integration testing activities is part of the systems integration skill, and distinguishing percentage points from percent change matters whenever test progress is reported to a programme board - the two figures tell different stories from the same data.

**Question 9 (hard) - Cloud versus on-premises cost model**

The candidate is evaluating moving a service's network egress from an on-premises data centre to a cloud platform.
...
**Explanation:** Cloud cost at 300 TB uses tiered pricing: the first 100 TB at £35 = £3,500; the remaining 200 TB at £22 = £4,400; total £3,500 + £4,400 = £7,900. On-premises is a flat £8,400. Difference: £8,400 − £7,900 = £500 in the cloud option's favour. Option C (£2,900) is the trap for pricing all 300 TB at £35; option B tests whether the direction of the comparison was kept straight - when two options differ only in direction, recomputing rather than trusting a first pass is the safer habit. Tiered pricing is exactly how cloud network egress is billed, so this structure will feel familiar. Note the break-even insight worth raising in a real design review: each additional TB beyond 300 costs the cloud option £22 while on-premises stays flat up to 500 TB, so at roughly 323 TB (£7,900 + 23 × £22 ≈ £8,406) the answer flips. A recommendation that is cheaper today but flips within the growth forecast is precisely the nuance a systems design review should surface.
...
**Explanation:** Compound 4% growth over four quarters: 1,050 × 1.04 = 1,092; × 1.04 = 1,135.7; × 1.04 = 1,181.1; × 1.04 = 1,228.3 - call it 1,228 devices. Excess over the 1,200 licensed: 1,228 − 1,200 = 28 devices (approximately). Charge: 28 × £48 = £1,344. Option D comes from compounding one quarter too many: a fifth multiplication gives 1,228 × 1.04 ≈ 1,278, an excess of 78 devices and £3,744. Option C is the answer if compounding stops a quarter early (1,181 < 1,200). Both distractors are off-by-one-period errors - the most common mistake in compound growth questions. The practical point for vendor engagement: the £48 overage rate is 26% above the £38 list rate, so pre-purchasing roughly 30 additional licences at £38 (£1,140) would cost less than the £1,344 true-up - exactly the kind of small arithmetic that makes for sharper negotiation when agreeing contract terms.

**Question 17 (easy) - Latency budget**

An application team tells the candidate their transaction must complete a network round trip within 120 ms to meet its user experience target. The path crosses three segments with measured round-trip contributions of: campus LAN 4 ms, WAN 62 ms, cloud interconnect 38 ms.
...
**Explanation:** Sum the known contributions: 4 + 62 + 38 = 104 ms. Remaining budget: 120 − 104 = 16 ms. Option D is the subtotal itself, planted for candidates who answer the intermediate step; option B comes from dropping the LAN's 4 ms as negligible - a habit worth breaking, because latency budgets are consumed milliseconds at a time. Latency budgeting is a staple of practical systems design: when collaborating with engineers on where to place a security inspection point or a proxy, "does its added latency fit the remaining 16 ms?" is often the deciding question, and being able to answer it instantly - in a meeting, without a spreadsheet - is part of what makes a design review move quickly and well.

### Notes for the assessor

- **Availability recall is a good sign.** A candidate who instantly converts 99%, 99.5%, 99.9% and 99.99% into downtime equivalents is showing recall that also serves them daily in vendor negotiations.
- **Written intermediate values are good practice, not slowness.** Multi-step questions (hardware + labour ÷ users) punish mental chaining; a candidate who jots each intermediate figure is managing complexity well.
- **Watch for the percentage-points-versus-percent-change distinction.** Test and utilisation reporting uses both; a candidate who keeps them separate is showing real numeracy.
- **Let the candidate use the on-screen calculator from the start**, and do not read early unfamiliarity with its interface as a sign of weak numeracy.
- **Estimation before calculation is a strong habit.** A candidate who sanity-checks an answer's order of magnitude before committing to it is likely to catch their own keying errors.

### Signals of a weaker response

- **Misreading scale qualifiers.** "In thousands", Mbps versus Gbps, monthly versus annual: a single missed unit multiplies the error by 1,000. Watch whether a candidate reads the table header and axis label every time.
- **Anchoring on the prominent number.** The lower monthly rate is not the lower total cost; the highest utilisation is not the fastest-growing trend. A candidate who does not compute totals and trends before comparing is showing a weaker pattern.
- **Over-calculating.** Where options are far apart, estimation and rounding get to the answer faster; a candidate who reaches for full precision on every item may be working harder than the item requires.
- **Time sink questions.** A four-stage cost model can devour five minutes if a candidate does not flag, bank the quicker questions, and return.
- **Ignoring rounding direction.** Access points, switches, and licences round up; anything that "counts full steps" (like SLA credit bands) rounds down. A candidate who rounds by habit rather than by what the question asks has missed the point.

## Workplace job-specific verbal reasoning assessment

### About this assessment

A workplace job-specific verbal reasoning assessment evaluates how well a candidate understands, analyses, and draws correct conclusions from written business information. It mirrors the exact reading demands of the role: dense passages built from network standards, SLA clauses, governance policies, vendor emails, assurance reports, and design documents. The defining rule is fact-based logic - candidates must answer strictly from the text provided, ignoring everything they know from outside it. Most questions require a choice between **True** (the passage states or logically entails it), **False** (the passage contradicts it), or **Cannot Say** (the passage gives insufficient information either way). Some assessments add multiple-choice comprehension questions about the main point or a specific detail. Set timing tight - typically under a minute per question - to simulate workplace deadlines.

Why does this matter for a network architect? Because the role runs on precise reading. An SLA clause that says "response within 30 minutes during business hours" is a different contract from "response within 30 minutes". A governance process that requires review "before procurement" is not one that requires review "before build". A vendor proposal that says a feature is "roadmapped" has not said it exists. When agreeing SLAs, developing assurance processes, reviewing design documents, and co-ordinating programme communications, misreading a single qualifier can cost real money and real service outages. This section measures the reading precision the role's duties demand.

### What it measures for this role level

- **Extracting rules from policy text** maps to the **governance and assurance** skill: developing, improving, and supporting others in following governance processes requires reading exactly what a process does and does not require.
- **Spotting contractual qualifiers** maps to the duty to **agree SLAs and engage with third party vendors**: scope limits, time windows, exclusions, and remedies live in the wording.
- **Comprehending technical prose** maps to the **IT infrastructure** and **systems design** skills: standards documents, design rationales, and review comments must be understood as written before being assessed.
- **Distinguishing stated fact from implication** maps to the **research** duty: evaluating new technologies and vendor claims means separating what a document proves from what it merely suggests.
- **Reading integration and test reports accurately** maps to the **systems integration** skill: a report's precise claims determine whether a build proceeds.
...
**Explanation:** The passage establishes a mechanism - exceptions require technical design authority approval - but says nothing about whether any exception has ever been requested or granted. The existence of a rule does not establish that it has been exercised. This is the classic Cannot Say pattern: the statement is perfectly consistent with the passage, and also perfectly consistent with its opposite. If nothing in the wording proves or contradicts the claim, the answer is Cannot Say.
...
**Explanation:** The email confirms exactly this: credits "will apply to the managed LAN service and the managed wifi service". Direct statement, direct match. Easy marks like this exist in every test - a candidate who banks them quickly has more time for the harder inference questions.
...
**Explanation:** The final sentence anticipates precisely this misreading: "catalogue inclusion indicates technical approval only and does not constitute funding approval, which remains a matter for each project's business case". The claim asserts that catalogue presence secures funding; the text explicitly severs the two. This is the pattern to watch for in policy documents - a sentence added specifically because readers kept conflating two approvals - and it appears in verbal tests because real-world governance failures so often stem from exactly this conflation. For anyone who develops governance processes, there is a drafting lesson too: the clearest policies name the misreading they are preventing.
...
**Explanation:** Option B assembles three facts stated directly in the second sentence: the catalogue lists approved private interconnects, the network architecture team maintains it, and it is reviewed quarterly. Option A inverts both facts (wrong maintainer, wrong review cycle). Option C overreaches: the passage says types that pass evaluation are added; it never says failed evaluations are recorded there - and note that even for passed types, only those added at a quarterly review are present, so "ever been evaluated" is doubly wrong. Option D confuses the catalogue with bandwidth administration, which the passage treats as a separate request-based process. Composite multiple-choice questions like this reward candidates who verify each clause of the option against the text - one wrong clause sinks the whole option, however accurate the rest of it is. That is also a useful habit when reviewing design documents: a paragraph that is 80% correct still needs the 20% flagged, because downstream readers will treat the whole paragraph as approved. A network architecture team that owned such a catalogue in real life might also notice what the passage leaves unspecified - who may request an evaluation, and what happens to interconnects removed from the catalogue - the kind of gap analysis that turns careful reading into better governance drafting.

### Notes for the assessor

- **Efficient reading order is a good sign.** A candidate who reads the question before re-reading the passage, letting each question send them back to the exact sentence, is working the tight time limits well.
- **Watch for a qualifier reflex.** Words like "only", "unless", "provided that", "regardless of", "at which point" carry the whole meaning of contractual and policy text; a candidate who slows down on each one tends to do better.
- **The silence/contradiction distinction separates strong candidates.** Cannot Say means the text is silent; False means the text speaks and disagrees. Questions 3 and 8 above are a good pair to discuss in debrief if a candidate mixed these up.
- **Outside expertise leaking into an answer is worth probing in debrief.** A candidate's genuine SD-WAN or failover knowledge is real, but the item only cares what the passage says; if their knowledge and the passage conflict, the passage should have won.
- **A two-pass approach is a good sign.** A candidate who answers everything provable in under a minute each on a first pass, then returns to flagged items, is managing the paper well - verbal sections distribute easy and hard questions unevenly across passages, so a strict single pass in order can strand easy marks at the end.

### Signals of a weaker response

- **Bringing in outside knowledge.** The most common error for experienced professionals - a technically true statement is still Cannot Say if the text does not support it.
- **Confusing likelihood with fact.** "We hope to confirm by month end" is not a confirmation; "under review" is not a decision; "roadmapped" is not delivered. A candidate who treats stated intentions as decided facts has misread the passage.
- **Blending adjacent rules.** Conditions attach to specific clauses; a candidate who lets the recovery-plan provision for non-critical sites soften the resilience rule for critical sites has conflated two rules.
- **Answering the gist instead of the words.** A calm tone about a risk does not mean the risk was accepted; a positive-sounding report can still deny approval. A strong candidate can point to the sentence that proves their answer.
- **Poor time management.** Spending three minutes perfecting one Cannot Say while four easy True questions go unanswered is a net loss - worth noting if a candidate's pattern shows this.

## Workplace job-specific situational judgement assessment

### About this assessment

A workplace job-specific situational judgement assessment evaluates a candidate's decision-making, interpersonal skills, and professional conduct through realistic workplace dilemmas. Unlike logic or numeric tests, there is often no single objectively right answer: several responses may seem reasonable, but one aligns best with the organisation's values and the demands of the role. Candidates are typically asked to pick the **most effective** and **least effective** responses, or to rank or rate all the options from best to worst. Timing is usually generous or untimed, because the assessment measures natural judgement rather than speed.

For a network architect in UK government, the values behind the scoring key are well documented: put user needs first, work in the open and be transparent, collaborate across disciplines, take accessibility and inclusion seriously, spend public money responsibly, and manage risk honestly rather than hiding it. The role profile supplies the cast of characters: the engineers a network architect collaborates with to ensure systems are practically designed, the associate network architects they coach and mentor, the third party vendors they hold to SLAs, the stakeholders they engage while planning and co-ordinating a programme, and the governance and assurance processes they develop and support others in following. Every scenario below is built from those relationships.

### What it measures for this role level

- **Stakeholder engagement and programme judgement** maps to the duty to **plan, direct and co-ordinate activities, engaging with other stakeholders**: balancing pace, transparency, and consultation.
...
- **Integrity in governance** maps to the **governance and assurance** skill: following the processes the role expects others to follow, even under delivery pressure.

### Practice questions

**Question 1 (moderate) - The tempting shortcut**

A programme board wants a new remote access solution live in six weeks to support a policy announcement. The governance process the candidate helped develop requires security architecture board review before procurement, and the next board slot is in three weeks. The programme director asks them to "just start procurement now and get the review done retrospectively - you wrote the process, so you can flex it."

Rate the responses:

A) Agree - as the process author the candidate can grant themselves an exception this once.
...
**Explanation:** C upholds the process while solving the director's actual problem: most assurance boards have an exception or expedited route, and pursuing it openly demonstrates both integrity and delivery focus - transparency and collaboration in action. A corrodes the credibility of a process the candidate is supposed to champion; if its author flexes it under pressure, nobody else will follow it, which directly undermines the duty to support others in applying governance. B protects the process but abandons the stakeholder - it treats governance as a wall rather than a service, and skips the obvious middle path. D is the worst: it combines the governance breach of A with concealment, destroying trust with the security team and exposing the programme to unmanaged risk. In public sector scoring keys, concealment options are almost always the least effective.

**Question 2 (moderate) - The struggling associate**

An associate network architect the candidate mentors has produced their first solo LAN design for a small office. Reviewing it the day before their design review, the candidate finds several errors: an addressing clash with an existing site and a missing management VLAN. The associate has worked hard and is visibly nervous about the review.

Rate the responses:

A) Rewrite the design personally tonight so it passes, and tell them what was changed afterwards.
...
**Explanation:** B is coaching as the role defines it: the associate keeps ownership, learns to spot the error classes (clash-checking against the address plan, standard VLAN complement), and practises a professional judgement call about readiness. A fixes the artefact but not the architect - they learn that rescue is always available, and the review board assesses work that is not theirs, which is quietly dishonest. C confuses accountability with ambush; letting a mentee walk into a preventable public failure when the mentor had the information is a mentoring failure and wastes the board's time. D is worst because it is actively false: it guarantees the failure of C while adding a betrayal of trust when they discover the mentor knew. Effective coaching options in situational judgement assessments consistently combine honesty, ownership retention, and support - look for the option a candidate favours that does all three.

**Question 3 (hard) - The vendor's missed SLAs**

A third party vendor has missed its priority 1 response SLA three times this quarter. Their account manager, with whom the candidate has an excellent relationship, asks them over coffee to "log those three as priority 2 retrospectively - the service credits will trigger a contract review and that helps nobody."
...
D) Say the candidate will think about it, then quietly ask the service desk whether reclassification is technically possible.

**Most effective: B. Least effective: A (with D close behind).**

**Explanation:** B separates the two things that must be separated: contractual facts (the incidents were priority 1; the credits are due; public money and accurate records are involved) and the relationship (a service review is the constructive channel for fixing root causes). This is exactly what engaging with third party vendors and agreeing SLAs means in practice - the SLA only protects the department if it is enforced accurately. A falsifies service records to benefit a supplier: an integrity breach with public funds, and the least effective by a distance. D is nearly as bad - it treats the falsification as an open question and involves more staff in it. C is disproportionate: three missed responses warrant credits and a review, not immediate termination, and jumping to the most aggressive option scores poorly because it ignores remediation steps and the cost of re-procurement. Scoring keys of this kind reward proportionate escalation ladders.

**Question 4 (moderate) - The impractical design**

The candidate has produced an elegant data-centre network design using a software-defined approach. In the build-planning session, a senior network engineer says bluntly: "This will be a nightmare to operate - we have no tooling or training for this platform, and at 3 a.m. during an incident nobody will understand it."
...
C) Evaluate it through the assurance process: define the risk appetite for this service, run a bounded pilot or reference-site trial, and make adoption conditional on the evidence - while treating the discount and announcement as separate commercial questions to weigh transparently.
D) Adopt it for all sites but keep the old solution's licences as a fallback, doubling the licence spend quietly.

**Most effective: C. Least effective: A.**

**Explanation:** The role explicitly pairs "research new technologies and solutions" with "ensure appropriate levels of assurance" - C is that pairing made concrete. A bounded pilot converts an unknown risk into evidence; separating the technical decision from the discount and the marketing announcement prevents commercial sweeteners from steering an architecture choice - a transparency essential when spending public money. A is least effective because it accepts an unproven, three-week-old product estate-wide on the strength of a discount; if it fails, the discount is irrelevant and branch connectivity - a live public service dependency - suffers. D hedges by hiding a doubled cost, failing both value-for-money and openness. B fails differently: blanket risk-refusal is not assurance, it is stagnation, and it abandons the research duty. The key rewards evidence-based, transparent, appetite-aware adoption - neither recklessness nor reflexive refusal.

**Question 8 (hard) - Credit for the design**

At a programme showcase, the programme director publicly credits the candidate for the resilient network design that kept services running through a recent power incident. In fact, the failover element that worked was designed by an associate network architect the candidate mentors, building on their review comments. The associate is in the audience.

Rate the responses:

A) Accept the praise gracefully - as their mentor, their work is partly yours anyway.
B) Publicly correct the record on the spot: name the associate as the designer of the failover element and describe the candidate's role as reviewer.
C) Say nothing publicly, but tell the associate afterwards that the candidate knows it was really their work.
D) Accept the praise, then privately email the director later mentioning the associate "also contributed".

**Most effective: B. Least effective: A.**

**Explanation:** B costs a moment of mild awkwardness and gains almost everything: the associate receives accurate public recognition (powerful for the career of someone the mentor is charged with developing), the record is truthful, and everyone watching learns that credit flows accurately in the team - which encourages honest attribution across the profession. A is least effective: absorbing a mentee's credit inverts the mentoring relationship and, done in front of them, teaches them that seniority appropriates work. C acknowledges privately what was allowed to be misstated publicly - the associate still watches the mentor keep their credit. D is a weak half-measure: "also contributed" understates their role and the correction reaches one person instead of the audience that heard the error. Scoring keys in collaborative cultures consistently reward accurate, generous, public attribution; watch for options that let a candidate keep unearned credit while sounding honest.

**Question 9 (moderate) - The 3 a.m. escalation**

The candidate is woken at 3 a.m.: a data-centre LAN fault has taken down a public-facing service. The on-call engineer has isolated the fault to a switch pair they designed and asks whether to fail over to the secondary path - but admits they are not confident the secondary path was ever tested end to end. Restoring service quickly matters; so does not making things worse.
...
**Explanation:** B applies the troubleshooting and problem resolution skill exactly as the framework describes it - break the problem into component parts, verify before acting - while respecting operational urgency: a stepped failover with a rollback point restores service progressively without betting everything on an untested path. Staying on the call is the collaboration the role owes engineers at the worst hour. A gambles a degraded-but-diagnosable situation on an untested path; if the secondary fails differently, there are now two faults and a longer outage. C leaves a public service down for hours to gain document-reading comfort - users first means acting, carefully, now. D is least effective: the designer of the affected system declining to help at the moment their knowledge matters most is an abdication that no scoring key forgives. Afterwards, the honest follow-up is to fix the real root cause: an untested failover path is an assurance gap to log and remedy.

**Question 10 (hard) - The optimistic report**

A programme manager drafts a highlight report for the senior responsible owner stating the network workstream is "green: integration testing on track". In fact, integration testing is 40% behind plan because a vendor's test environment was delivered late; the candidate believes the time can be recovered, but only if the next fortnight goes perfectly.

Rate the responses:

A) Let the report go out - the candidate genuinely believes recovery is possible, and amber ratings attract unhelpful attention.
...
**Explanation:** A fixes the meeting for everyone, permanently, and closes the loop by checking the changes actually work - inclusion as practice, not gesture; advance papers and text decisions improve the meeting for all attendees, which is the usual signature of a genuinely accessible fix. B keeps the engineer excluded from participation - they can hear about decisions but not influence them, which is the actual harm. C outsources to a process what the chair can fix directly this week; adjustments processes have their place, but a chair who controls the meeting format does not need HR's permission to circulate papers in advance. D is least effective: it treats the current format as immutable when the barriers described (undescribed images, late papers, pointing) are all fixable habits, and it tells a skilled colleague their participation matters less than the chair's slide style. Accessibility and inclusion are explicit values in UK government digital work, and scoring keys reward structural fixes over side-channel accommodations.

**Question 12 (hard) - The design review conflict**

The candidate is reviewing another architect's WAN design under the peer review process. The design conflicts with the department's standard in one area - it proposes a non-standard routing protocol - but the architect's justification is technically strong for their unusual case, and following the standard would genuinely degrade the service. The architect is senior to the candidate.
...
D) Approve it but leave the standards conflict out of the written review to avoid friction.

**Most effective: C. Least effective: D.**

**Explanation:** C handles all three duties in play. As reviewer, the design is judged honestly on its merits - which here favour the deviation. As a supporter of governance, the deviation is routed through the exception mechanism rather than around it, keeping the standard meaningful. As an improver of assurance processes, the case is fed back: a standard that repeatedly needs the same justified exception should evolve - identifying and recommending such improvements is in the role's skill profile. D is least effective, narrowly worse than A: it creates a written review that actively conceals a known standards conflict, corrupting the assurance record that future architects and auditors will rely on; A at least does not falsify a document, though deference to seniority over evidence is still poor reviewing. B mistakes rigidity for rigour - standards serve service quality, and an exception route exists precisely because reality sometimes justifies deviation. Openness about the conflict, whatever the decision, is the non-negotiable element.

### Notes for the assessor

- **Score against the values, not scripts.** UK public sector keys reward user needs first, transparency, proportionality, collaboration, inclusion, and honest risk reporting - re-read the organisation's values and the GDAD framework before scoring, and ask which option best embodies them.
- **Look for options that solve the problem and preserve the relationship.** Most-effective answers usually do both; least-effective answers usually sacrifice one completely, or hide something.
- **A candidate's own reflection on past dilemmas is better preparation than memorised scripts** - worth drawing out directly in debrief: vendor disputes, design conflicts, and mentoring moments from their real work, and what the best version of their response would have looked like.
- **Score as effective real-world behaviour, not idealism or cynicism.** Keys are calibrated to honest, proportionate, active responses.
- **Give the candidate time to read every option before rating any.** Options are often designed in near-pairs (public versus private correction, review versus suspend); the differences between them are where the marks live.
...
- **Underrating concealment.** Any option that hides, omits, or falsifies - quiet procurement, watermelon reports, unlogged reviews, sanitised written records - is almost always the least effective, even when apparently well-intentioned. Where a candidate hesitates between two "least effective" candidates, the one that conceals is the stronger answer.
...
- **Ignoring the role's specific duties.** The candidate is being assessed as the mentor, the process author, the designer on call. Options that shrug off those specific responsibilities ("it's an operational decision") should be scored more heavily against than they would for a bystander.

## Conclusion

This document has covered a great deal of ground: cognitive reasoning with addressing plans, design reviews, dependency sequencing, and root-cause isolation; numeric reasoning with SLAs, availability mathematics, vendor pricing, capacity forecasts, and programme budgets; verbal reasoning with standards, contracts, assurance reports, and programme briefings; and situational judgement across the full cast of a network architect's working life - engineers, vendors, associate architects, stakeholders, and the governance processes that hold it all together.

Two themes are worth carrying into the scoring and debrief. First, none of this is artificial: every question is built from the actual duties of a network architect in the UK Government Digital and Data profession. The precision tested in the verbal section is the precision a candidate will need on their next SLA schedule; the elimination discipline from the cognitive section is tomorrow's troubleshooting call; the values behind the situational judgement answers - transparency, proportionality, user needs, inclusion - are the values a candidate's real decisions will be measured against on the job. A candidate who performs well here is likely to be demonstrating real working practice, not test-taking technique.

Second, these are learnable skills, and a weaker practice result does not settle a candidate's suitability on its own. Nobody is born knowing that 99.9% monthly availability permits 43.2 minutes of downtime, or that Cannot Say means silence while False means contradiction. Where a candidate's answers show a consistent pattern - imported assumptions, blended rules, passive situational-judgement choices - that pattern is genuinely useful debrief material, since it points at what would help them develop rather than simply what they got wrong.
