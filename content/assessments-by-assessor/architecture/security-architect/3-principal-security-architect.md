...
This document is written for you: an assessor administering a workplace psychometric assessment to a candidate for principal security architect within the UK Government Digital and Data (GDAD) profession. Whether you are running an internal promotion board, a departmental transfer panel, or an external recruitment process, it gives you structured, job-specific material to present to the candidate — cognitive, numeric reasoning, verbal reasoning, and situational judgement items — each with a worked answer and explanation that forms your answer key. Hold the answer key back from the candidate until the session is complete and you are ready to score and debrief; showing it in advance defeats the exercise.

Psychometric assessments are structured, standardised exercises that measure the underlying capabilities a role demands: how quickly and accurately a candidate processes information, how they reason with numbers and language, and how they exercise judgement in realistic workplace situations. At principal level, these assessments are rarely about raw speed alone. You are looking for evidence that a candidate can absorb complex, ambiguous material — a board paper on cyber risk, a supplier's architecture proposal, a set of vulnerability metrics — and reach sound, defensible conclusions under time pressure. That is, in essence, a compressed version of what the role itself demands: setting security strategy across an organisation, acting as a point of escalation for senior risk owners, and making architectural decisions on services of high complexity and risk.

This document is organised into four assessment areas, each aligned to the role's skills as defined in the GDAD Profession Capability Framework:

1. **Workplace job-specific cognitive assessment** — pattern recognition, logical deduction, error checking and prioritisation using the artefacts a principal security architect genuinely handles: architecture diagrams, control frameworks, firewall rule sets, and threat models. Typically delivered online and timed at around 15 to 30 minutes in total — budget roughly a minute per item when you plan a session.
2. **Workplace job-specific numeric reasoning assessment** — interpreting risk metrics, security programme budgets, remediation statistics and capacity figures, and reasoning about cost-benefit trade-offs in controls. Nearly always time-limited; plan for a similarly brisk pace, with an on-screen calculator available to the candidate.
3. **Workplace job-specific verbal reasoning assessment** — drawing precise conclusions from dense written material such as security policies, assurance reports, legal guidance, and senior stakeholder correspondence. Timing is tight, typically under a minute per question.
4. **Workplace job-specific situational judgement assessment** — exercising professional judgement in realistic dilemmas involving senior risk owners, delivery teams, suppliers, and cross-government stakeholders. Timing is usually generous or absent here, because this section targets judgement rather than speed.

Each section explains what the assessment measures, maps it explicitly to the role's skills, provides a substantial set of practice questions with worked answers, and closes with notes for you on administering that section and the pitfalls candidates commonly fall into. Present the same items, in the same order, under the same time limit, to every candidate for this role level — consistency is what makes any comparison across candidates defensible. Tell the candidate beforehand roughly how long each section will take and what format to expect (multiple choice, True / False / Cannot Say, most-effective / least-effective ranking), but do not share the worked answers or explanations until scoring is complete. Once the candidate has finished all four sections, use the worked answers to score their responses, and use the explanations to debrief them afterwards on what a strong answer looked like and why.
...
For a principal-level role, cognitive assessments carry particular weight because the role is defined by decision quality on services of high complexity and risk. Setting a long-term security strategy across the breadth of an organisation requires sustained acts of pattern recognition (what recurs across these systems?), logical deduction (what must follow if this trust assumption fails?), error checking (which of these configurations contradicts the stated principle?) and prioritisation (which of these unprecedented problems should be solved first?). This assessment compresses those acts into short, sharply timed items you present to the candidate. Expect a mixture of diagrammatic reasoning, rule-based deduction, consistency checking across documents, and ordering or scheduling problems. Modern platforms may adapt difficulty to the candidate's answers, and employers usually receive a breakdown of speed versus accuracy rather than a single score — so weigh disciplined accuracy as heavily as pace when you interpret the result.

### What it measures for this role level

The assessment dimensions map directly onto the skills in this role level's profile:

- **Analysis.** Items that ask the candidate to monitor a changing technical environment — a revised architecture, a new dependency, an altered data flow — and judge whether risks remain at acceptable levels or whether a previous decision must be revisited. Pattern-recognition and consistency-checking questions exercise exactly this.
- **Designing secure systems.** Deduction items built on system architectures test the candidate's ability to lead design and review of solutions to complex problems by reasoning from patterns and principles: given these components, trust boundaries and controls, what necessarily holds, and what has been violated?
- **Security technology.** Fault-finding items reflect the ability to explain complex system architectures and to identify how easy or difficult it would be to exploit a vulnerability, tracing exploit paths through layered controls.
- **Enabling and informing risk-based decisions.** Prioritisation items mirror the role's work applying risk methodologies at the most complex levels: ranking findings, sequencing remediations, and deciding what to escalate.
...
Present these to the candidate under standard timed conditions — plan for around a minute per item across the whole test. The worked answer and explanation below each item form your answer key; read them carefully so you can debrief the reasoning as well as the result, since the reasoning path is often the most useful part of the debrief, even where the candidate answered correctly.

**Question 1 (easy) — Pattern recognition in firewall rule sets**

The candidate is reviewing a perimeter rule set migrated between platforms. The original policy intent is: "Only the presentation tier may accept inbound internet traffic; the application tier accepts traffic only from the presentation tier; the database tier accepts traffic only from the application tier." The migrated rules are:
...
**Explanation:** The intent permits internet traffic only to the presentation tier. Rules 1–3 each match one sentence of the intent, and rule 5 is the standard default deny. Rule 4 allows the internet to reach the application tier directly, bypassing the presentation tier — a classic tiering violation introduced during migration. At principal level, the systemic lesson is worth noting too: migrations need automated policy-intent checking, not line-by-line eyeballing. In the assessment, though, the task is simply rapid, accurate matching of rules to stated intent.
...
The candidate is quality-assuring a mapping of controls to requirements before it goes to a senior risk owner. The requirements say:
...
**Explanation:** Compliance requires actual retention of 24 months, so the service configuration must change. Leaving the organisational standard at 12 months would leave internal doctrine contradicting national policy and would mislead every future service built to it, so the standard must change too. Doubling retention has storage cost implications, so the approved budget is "likely to need revisiting" — the question's careful wording matters, because the budget is a consequence rather than a compliance artefact. A fixes one service but leaves the standard wrong; B fixes doctrine but not reality; D is outside the architect's gift. This item rehearses cascade analysis: tracing a single policy change through standards, configurations and resources — the daily mechanics of leading change regarding factors that feed into analysis.

### Notes for the assessor

- **What a well-prepared candidate does well.** A well-prepared candidate has spent time reading firewall rule sets, control mappings and architecture decision records asking what each one forces, forbids and contradicts — that daily habit converts directly into timed-test fluency, and is worth listening for in how the candidate talks through their reasoning.
- **Restating the rule before applying it.** In each item above, the winning move is restating the given rule precisely, then testing options against it. Under time pressure, a candidate who reads carefully once beats one who skims anxiously three times — a pattern worth noting if a candidate revises an answer repeatedly.
- **Verification habits on paper.** For sequence, scheduling and capacity items (Questions 6, 8, 11), a strong candidate writes the state down after each step rather than holding it all mentally; candidates at this level lose marks not to difficulty but to mental juggling.
- **Timing.** With 15 to 30 minutes for the whole test, budget roughly a minute per item and tell the candidate this before they start; banking easy items first and flagging anything that stalls them is a legitimate strategy, not a shortcut, so let the candidate know flagging is available if your platform supports it.
- **Interface familiarity.** Most platforms offer ungraded practice items; let the candidate use them to absorb the interface first, so no working memory is wasted on navigation during the scored section.

### Common candidate pitfalls

- **Answering from expertise instead of from the stimulus.** A candidate's deep technical knowledge will sometimes suggest a "better" answer than the facts given support. The assessment scores deduction from the material provided — as Question 9's option D shows, capability is not action.
...
- **Time-sink items.** One stubborn multi-stage item can cost a candidate three easier ones; if your platform allows flagging and returning, make sure the candidate knows this before the session starts.
- **Ignoring scale boundaries and adjustments.** Methodology items often hide a capped scale or a mandated uplift (Question 5); every stated rule needs to be applied before the candidate computes an answer.

## Workplace job-specific numeric reasoning assessment

### About this assessment

A workplace job-specific numeric reasoning assessment evaluates a candidate's ability to analyse, interpret and make sound decisions using numerical data drawn from the target role — business mathematics rather than abstract formulae. Question inputs are the tables, budgets, dashboards and schedules the role genuinely encounters: for a principal security architect, that means security programme budgets, vulnerability and patching metrics, incident statistics, licensing and capacity figures, and cost-benefit comparisons of controls. The mathematics itself stays deliberately practical — percentages, ratios, weighted averages, rates of change and cost estimations — because the test prioritises interpretation over mental gymnastics. Most platforms allow or provide an on-screen calculator, and nearly all impose a strict time limit, so quick, accurate data extraction matters as much as computation.

At principal level, numeric fluency underpins credibility. When a security architect recommends design up to an organisational or inter-organisational level, the case usually turns on numbers: what a control costs against the loss expectancy it reduces, whether a remediation trajectory will meet a compliance deadline, how a budget should be apportioned across competing programmes. Senior risk owners trust architects who can move between a technical judgement and its quantified consequence without hesitation. The assessment simulates exactly that movement, repeatedly, under time pressure, and lets you see how the candidate performs it.
...
- **Research and innovation.** Technology-adoption items compare costs, capacity and benefit across candidate technologies, echoing how the role validates and adopts new technologies for the business context.
...
Let the candidate use a calculator freely if the platform provides one — the skill being measured is choosing the right numbers and operations, not mental arithmetic. Every explanation below shows the arithmetic in full, so you can check a candidate's working against it during scoring or debrief.

**Question 1 (easy) — Percentage of estate patched**

An organisation runs 4,800 servers. A monthly report shows 4,272 have the latest critical security patches applied. What percentage of the estate remains unpatched?
...
**Explanation:** Unpatched servers: 4,800 − 4,272 = 528. As a percentage: 528 ÷ 4,800 = 0.11 = 11%. Option D (89%) is the patched proportion — the classic misread of which quantity was asked for. On dashboard questions, check first whether the question wants the compliant or non-compliant share before calculating.
...
**Explanation:** 20% of £3.6 million = 0.20 × 3,600,000 = £720,000. A quick sanity check: 10% is £360,000, so 20% must be double that. Options A and D are the 10% and 25% shares — distractors built from adjacent rows of the same table, a standard trick in budget items. Extracting the right row before touching the calculator is the discipline this item rewards.
...
**Explanation:** Divide each figure by the greatest common divisor, 9: 9 ÷ 9 = 1, 18 ÷ 9 = 2, 18 ÷ 9 = 2, giving 1:2:2. Option A is correct but not in simplest form — a candidate needs to read the question's exact requirement. Ratio items are quick marks for a candidate who resists overthinking: find the common divisor and reduce.

**Question 4 (moderate) — Annualised loss expectancy**

The candidate is comparing two controls for a payments service. Without controls, a data-breach event is estimated at £2.4 million per occurrence with an annual rate of occurrence of 0.25. Control A costs £180,000 per year and reduces the annual rate of occurrence to 0.05. What is the net annual benefit of Control A?
...
**Explanation:** Annualised loss expectancy (ALE) before: £2,400,000 × 0.25 = £600,000. ALE after: £2,400,000 × 0.05 = £120,000. Risk reduction: £600,000 − £120,000 = £480,000. Net benefit after control cost: £480,000 − £180,000 = £300,000. Option D is the gross reduction (forgetting the control's cost); option A is the residual ALE. Loss-expectancy arithmetic is the quantitative backbone of enabling risk-based decisions — a candidate who has this before/after/net sequence automatic will move through this item quickly.
...
**Explanation:** Alpha: 272 ÷ 320 = 0.85 = 85%. Beta: 152 ÷ 190 = 0.80 = 80%. Alpha leads by 85 − 80 = 5 percentage points. Note the language: a difference between two percentages is expressed in percentage points, not per cent. Comparing rates rather than raw counts is exactly how performance across several projects is assessed fairly — Beta closed fewer findings but also had fewer to close.
...
A risk methodology weights services by criticality: gold services carry weight 3, silver weight 2, bronze weight 1. The estate has 5 gold services with an average risk score of 12, 10 silver services averaging 9, and 25 bronze services averaging 6. What is the criticality-weighted average risk score across the estate?
...
**Explanation:** Option X over three years: licences 3 × £220,000 = £660,000, plus £150,000 implementation = £810,000; minus savings 3 × £90,000 = £270,000; net £540,000. Option Y: licences 3 × £140,000 = £420,000, plus £310,000 implementation = £730,000; minus savings 3 × £70,000 = £210,000; net £520,000. Option Y is cheaper by £540,000 − £520,000 = £20,000. The structure to internalise is (recurring × years) + one-off − (savings × years), applied identically to both options before comparing. Common slips are omitting one option's implementation cost, or applying the savings to only one side. Also notice how sensitive the result is to the appraisal period: over five years, X's lower running cost pulls it back to £1,100,000 + £150,000 − £450,000 = £800,000 against Y's £700,000 + £310,000 − £350,000 = £660,000 — Y still wins, but the appraisal window should always be stated when presenting a technology recommendation, because the honest answer can flip with the horizon.
...
**Explanation:** The absolute change is 75 − 60 = 15 percentage points. The relative change is 15 ÷ 60 = 0.25 = 25%. Options B and D blur the two measures — the most common numerical-communication error in management reporting. As the person who presents adoption metrics for the patterns they set, a principal security architect protects their credibility by being crisp about this distinction: "up 25%" and "up 15 percentage points" describe the same improvement, and a careless mixture of the two invites challenge in exactly the forums where it can least be afforded.

### Notes for the assessor

- **Core operations dominate.** Percentages, percentage change, ratios, weighted averages and rate × time problems cover the vast majority of items — a candidate who is fluent in these will move through the paper quickly regardless of seniority.
- **The before/after/net pattern recurs.** A candidate who is used to recomputing headline figures from a real vulnerability or budget report, rather than taking them on trust, will find the loss-expectancy and cost-benefit items (Questions 4 and 9) straightforward.
- **Extraction, not calculation, is usually where errors happen.** Watch whether a candidate notes the figures they extract, with units, before computing — this single habit prevents most of the errors listed below.
- **Units and scale qualifiers matter.** "In thousands", "per month" versus "per year", GB versus TB, per cent versus percentage points: one missed qualifier invalidates otherwise perfect arithmetic, so check qualifiers specifically when marking a near-miss answer.
- **Estimation is itself a skill being tested.** A candidate who sanity-checks with a rough order-of-magnitude guess before calculating in detail is demonstrating a professional credibility skill they will need in live meetings, not just avoiding decimal slips.
- **Interface familiarity.** If possible, let the candidate try the platform's on-screen calculator on an ungraded item first; fumbling the interface costs more time than the mathematics does.
...
- **Answering the complement.** A candidate who computes the patched share when the question asks for the unpatched share (Question 1) has fallen for this; re-reading the final sentence of the stem before answering avoids it.
...
- **Over-calculating.** When options are far apart, a candidate using estimation and rounding will reach the answer faster than one doing exact long division; precision is worth the extra time only when options are close.
- **Time-sink questions.** A single multi-stage item is never worth three unanswered easy ones; if your platform allows flagging and returning, make sure the candidate knows this before the session starts.

## Workplace job-specific verbal reasoning assessment

### About this assessment

A workplace job-specific verbal reasoning assessment evaluates how well a candidate understands, analyses and draws correct conclusions from written business information, mirroring the exact reading demands of the role. Passages use the industry's real terminology — for a principal security architect, that means security policies, national guidance, assurance and audit reports, supplier correspondence and board papers. The defining discipline is fact-based logic: the candidate must answer strictly from the text provided, ignoring outside knowledge, however expert. Most items require a verdict of True (the passage supports the statement), False (the passage contradicts it) or Cannot Say (the passage provides insufficient information), supplemented by multiple-choice comprehension. Timing is tight — typically under a minute per question — to simulate workplace deadlines.

For this role, the assessment matters because language is the architect's primary instrument of influence. The role translates and accurately communicates security and risk implications at the most senior levels, across technical and non-technical stakeholders; it challenges and leads changes to policy, which means reading policy with forensic precision; it responds to challenges, which requires knowing exactly what a document does and does not claim. A principal architect who conflates "should" with "must", or a stated intention with a delivered fact, will misadvise a senior risk owner sooner or later. The Cannot Say category deserves special respect in your scoring: it rewards the professional humility of recognising when a text underdetermines a conclusion — the same judgement a security architect must exercise when an assurance report is silent on the one point that matters.

In format terms, expect a series of short passages, each followed by two to five statements or questions, delivered online with a countdown timer. Scores are typically normed against a professional comparison group, and many platforms report accuracy and speed separately. The practice set below follows that structure: six passages spanning the document types this role reads weekly, with difficulty labels so you can gauge where a candidate's time goes.

### What it measures for this role level

- **Communication (security architect).** Every item tests precise reading of the kind of material this role translates for senior stakeholders; several passages are written at board-paper register, and the questions probe whether nuance survives the candidate's reading.
...
- **Enabling and informing risk-based decisions.** Assurance-report passages test whether a candidate can state exactly what evidence establishes, a precondition for being trusted by senior risk owners as an expert.
...
Present each passage to the candidate with its questions, under a time budget of roughly a minute per question. The explanations below form your answer key, including why Cannot Say — not just True or False — is sometimes the only defensible verdict.
...
- **Reading strategy.** A strong candidate reads the statement before the passage and scans for its key terms, rather than reading every passage exhaustively first — with under a minute per item, this is what fast, accurate candidates tend to do differently.
- **Evidence over plausibility.** The reliable order is: does the text assert this (True)? Does the text contradict it (False)? Otherwise Cannot Say — regardless of how professionally confident a candidate feels about the underlying subject.
- **Qualifiers and attributions carry the answer.** Words like may, must, where, so far, states, considers, could eventually decide most items; a candidate who underlines them on the first pass is using a reliable technique — every one of the hard items above turns on such a word.
- **How this skill is usually built.** Candidates who have practised writing True / False / Cannot Say statements themselves — from a policy paragraph, an assurance report or NCSC-style guidance — tend to read items faster, because composing statements is the fastest way to internalise how they are built.
- **Pacing under time pressure.** Verbal items are deliberately dense; brief the candidate that if a verdict is not emerging within their time budget, choosing the best-supported option, flagging it and moving on is the right strategy, not a concession.

### Common candidate pitfalls

- **Importing expert knowledge.** The most dangerous pitfall for a principal-level candidate: they know how desk-based reviews, breach clocks and enclaves really work, and the test scores only what the passage says (Questions 4, 10, 15).
...
A workplace job-specific situational judgement assessment evaluates decision-making, interpersonal skill, and professional conduct through realistic workplace dilemmas. Unlike logic or numeric tests, there is rarely a single objectively right answer: several responses may seem reasonable, and the scoring reflects which behaviours best fit the organisation's values and the demands of the role. Response formats typically ask the candidate to select the most effective and least effective actions from a set, or to rank or rate every option. Timing is usually generous or absent, because the assessment targets a candidate's natural judgement rather than their speed.

For a principal security architect, the situational judgement assessment is arguably the most role-authentic of the four. Effectiveness at this level depends less on producing artefacts personally and more on how the architect acts as a point of escalation, manages senior stakeholder expectations under constrained timescales, responds to challenge, influences important business and architectural decisions, and furthers the profession by setting an example. The dilemmas below are calibrated to UK public sector values — user needs, transparency, collaboration, accessibility and inclusion — alongside the specific trust relationship a principal security architect holds with senior risk owners. When scoring, favour options that address the root issue, keep risk owners genuinely informed, preserve relationships while telling the truth, and strengthen the organisation's capability rather than just resolving today's incident. Watch for options that are passive, that pass a problem elsewhere without ownership, or that are idealistic in a way that would not survive contact with a real programme.
...
- **Enabling and informing risk-based decisions.** Several dilemmas turn on the boundary between the architect's advisory role and the risk owner's decision rights — the trust that makes a principal security architect the recognised escalation point.
- **Designing secure systems and Security technology.** Scenarios about patterns, review findings and exploitability judgements test how the candidate sets direction and creates precedents while keeping delivery moving.
...
Present each scenario to the candidate and ask them to identify the most effective and least effective options (Question 12 asks for a full ranking instead). The explanations below weigh every option and form your answer key — in situational judgement, the reasoning about the near-misses is often the most useful material for a debrief.

**Question 1 (moderate) — The risk owner who wants a shortcut**

A director (the senior risk owner) tells the candidate privately that a ministerial commitment requires a citizen-facing service to launch in three weeks. The candidate's architecture review found that the service's fraud-detection controls are incomplete, and they judge the residual risk to be well above the organisation's stated appetite. The director asks them to "soften the language" in the review so the launch decision is easier to defend.

A) Refuse and immediately email the accounting officer describing the director's request.
B) Decline to change the professional assessment, explain that accurate wording protects the director as decision-maker, and offer to add a clear statement of options — launch with specified interim controls and a dated remediation plan, or delay — so the risk decision is explicit and well-supported.
...
**Explanation:** Option B holds the professional line — the architect's assessment is their own — while doing the constructive work of a trusted adviser: reframing the situation so the risk owner can make a lawful, explicit, defensible decision, potentially including a risk-accepted launch with interim controls. It respects the decision rights of the risk owner without corrupting the evidence base, which is the heart of enabling risk-based decisions and of being trusted by senior risk owners. Option A escalates before attempting resolution, damaging the relationship and skipping the director's opportunity to step back; escalation is a tool a principal architect holds, not their first move. Option C corrupts the record and would erode the very trust that makes the role work. Option D is the least effective: it is passive, deceptive in effect, leaves the decision uninformed, and abandons both the service's users and the director. Transparency delayed past the decision point is transparency denied.

**Question 2 (moderate) — Challenged in front of the board**

At an architecture and investment board, a well-regarded chief technology officer from another directorate publicly challenges the candidate's recommended security pattern for inter-departmental data exchange, calling it "gold-plating that will delay every programme in the portfolio". Several board members visibly agree. The candidate is confident in the pattern, which reflects lessons from a serious incident elsewhere in government.

A) Restate the pattern's requirements more firmly and remind the board of the candidate's authority over security architecture.
...
**Explanation:** Option B models exactly what "successfully respond to challenges" means at principal level: it treats challenge as information, not attack; it grounds the pattern in threat and evidence rather than authority; and it converts a public standoff into a collaborative piece of work with a report-back, preserving both the pattern's integrity and the relationship. Option A leans on positional authority — the weakest form of influence and one that invites quiet non-compliance across the portfolio. Option C rewards volume over evidence: withdrawing a well-founded pattern because the room frowned teaches the organisation that security requirements are negotiable by mood. Option D is least effective because it combines silence at the accountable forum with behind-the-scenes manoeuvring — the opposite of transparency, and corrosive to board governance. Influence at this level is built on evidence offered openly and follow-through delivered visibly.

**Question 3 (moderate) — The unprecedented vulnerability**

A researcher responsibly discloses a novel vulnerability class affecting an authentication component used by the candidate's organisation and, they suspect, by several other departments. No public exploit exists yet. Their team can design a mitigation within days, but confirming the full cross-government exposure would take longer. The researcher plans to publish in 30 days.

A) Design and deploy the organisation's mitigation, and keep the details internal until the researcher publishes.
B) Ask the researcher to delay publication indefinitely while a full cross-government assessment is completed.
C) Deploy the mitigation, share the vulnerability details and mitigation design through the appropriate national and cross-government security channels, and coordinate timelines so other departments can act before publication.
D) Escalate to the candidate's director and await instructions before taking any technical action.

**Most effective: C. Least effective: A.**

**Explanation:** Option C does everything the role demands: it protects the organisation promptly, treats the 30-day window as a coordination clock, and uses recognised channels so other departments benefit from the team's mitigation design — furthering the profession and demonstrating best practice beyond the organisation, exactly as the role profile describes. Option B is unrealistic and unfair to a researcher already acting responsibly; indefinite delay requests damage the cooperative disclosure ecosystem government relies on. Option D confuses escalation with abdication: leadership should certainly be informed, but a principal architect is the expert whose recommendation leadership needs, and technical mitigation should not idle in a queue. Option A is least effective despite containing real action, because it knowingly leaves peer departments exposed to a vulnerability the team could help them fix — a failure of the collaboration and transparency values that cross-government security depends on, and a decision that would be difficult to defend after publication day.

**Question 4 (hard) — The strategy nobody asked for**

The candidate joined six months ago as the organisation's first principal security architect. Security decisions are made programme-by-programme; there is no organisation-wide security architecture strategy, and each programme's architects defend their local choices. Their remit letter mentions "setting direction", but no one has asked them for a strategy, and programme directors are focused on delivery dates.

A) Write a comprehensive strategy personally and issue it through the design authority as mandatory.
...
D) Ask the candidate's director to instruct programme directors to comply with a strategy the candidate will write.

**Most effective: B. Least effective: C.**

**Explanation:** Setting "a strategy that can be used in the long term and across the breadth of the organisation" — the role's own words — requires legitimacy as much as content. Option B builds the strategy out of the organisation's real problems, gives every future adopter a stake in it, and uses the design authority for endorsement rather than imposition; strategies adopted this way outlive their authors. Option A produces a document with the candidate's name on it and little else: mandates without engagement generate exemption requests, not alignment. Option D borrows hierarchical force instead of building influence, and positions the strategy as the director's instruction rather than the profession's direction — brittle the moment attention moves elsewhere. Option C is least effective: waiting for an incident to create a mandate is passive, gambles users' data on organisational learning-by-failure, and abdicates precisely the leadership the role was created to provide.

**Question 5 (hard) — The legacy decision that no longer holds**

Two years ago, before the candidate's tenure, the organisation accepted the risk of running a legacy payments component without modern authentication, based on an assessment that exploitation required physical network access. The candidate learns that a planned office relocation will move that network segment onto shared building infrastructure, and separately that exploit tooling for this component's weakness is now freely available. The original risk acceptance has 18 months left before its scheduled review.
...
**Explanation:** This role makes the architect an exemplar: how this moment is handled teaches every architect present what review culture is. Option A keeps the forum honest — flaws are found where finding them is the point — while modelling that critique targets designs, not people; crediting strengths and offering to pair converts a correction into development. Option B undermines the forum's purpose and, worse, routes the criticism through the person's boss rather than to them — a trust-damaging move disguised as tact. Option D has a socratic charm but is theatre at a junior colleague's expense in front of their director; used briefly it can work, but "however long that takes" makes it an ordeal. Option C is least effective: it puts citizen data at risk to spare feelings, falsifies the review's outcome, and denies the architect the feedback they need to grow. Kindness that suppresses safety-relevant truth is not kindness — and an approval a reviewer privately intends to unpick is a precedent no one should want to set.

**Question 7 (hard) — Constrained timescales, senior expectations**

A permanent secretary's office asks the candidate, at 16:00, for "a one-page view of whether the new data-sharing platform is safe to announce tomorrow morning". The platform's architecture review is 80% complete: the finished portions are sound, but the review of its consent-management integration — the politically sensitive part — is unfinished. Their team could work overnight but would still only reach partial coverage.
...
**Explanation:** The role explicitly includes furthering the profession and "demonstrating and sharing best practice within and outside the organisation", and engaging with the broader security community. Option B does this while taking the director's concern seriously in the one way it is legitimate: genuinely sensitive operational specifics (exact configurations, known residual weaknesses, incident details) should be sanitised before sharing. It also reframes scrutiny correctly — peer challenge hardens patterns, and a pattern that cannot survive community review should not be a pattern. Option A is a defensible half-measure but hoards the useful substance; departments do not compete on security, and "advantage" is the wrong frame for public service. Option D shares recklessly, ignoring that some content may truly need redaction, and steamrolls a stakeholder whose future cooperation on sharing will still be needed. Option C is least effective: it treats a misplaced concern as a veto, fails the profession-building duty of the role, and models exactly the insularity that keeps government departments solving the same security problems separately.

**Question 10 (hard) — The policy collision**

A new government-wide digital policy requires all citizen-facing services to offer a fully offline, paper-based channel. The candidate's fraud-risk analysis shows the paper channel, as specified, would bypass the digital identity verification that currently prevents an estimated £4 million of annual fraud, and the policy's implementation guidance is silent on compensating controls. The organisation's policy team plans to implement the guidance as written.

A) Implement the policy as written, since government-wide policy overrides local risk concerns.
B) Advise the organisation to refuse to implement the paper channel until the policy is amended.
C) Prepare a quantified analysis of the fraud exposure and design possible compensating controls for the paper channel; work with the policy team to implement the channel safely; and feed the gap and the proposed controls back to the policy's owners through formal channels so the guidance improves for every department.
D) Implement the policy but document the objection in the risk register for the record.

**Most effective: C. Least effective: B.**

**Explanation:** The paper channel serves genuine user needs — accessibility and inclusion for citizens who cannot or will not transact digitally — so the goal is to deliver it safely, not to defeat it. Option C is the "understanding security implications of transformation" skill operating exactly as specified: challenging and leading changes to policy and processes to support business outcomes, with legal and political awareness. It secures the local implementation with compensating controls, and — the distinctly principal-level move — exports the fix upstream so every department inherits better guidance. Option A abdicates the analysis only the architect can provide; policies are implemented by people who notice their gaps. Option D is compliance theatre: a documented objection mitigates nothing and protects only its author. Option B is least effective because refusing a lawful, user-serving government-wide policy positions security as an obstacle to inclusion, is politically untenable, and forfeits the influence the architect would otherwise have over how the channel is secured. The strongest security voices in government are the ones that make required things safe, not the ones that say no to requirements.

**Question 11 (moderate) — The exploitability disagreement**

A team's penetration testers rate a finding in a citizen-service API as critical, recommending an emergency change freeze. The delivery team's lead engineer argues the finding is theoretical because exploitation requires an authenticated session with elevated scopes, which external attackers cannot obtain. The candidate reviews the technical detail and concludes exploitation is harder than the testers assess but easier than the engineer believes: a mid-complexity chain via a partner integration could reach it.
...
**Explanation:** Action 1 comes first because every subsequent decision — communications, legal, regulatory — depends on establishing the technical facts, and the organisation's own network exposure is time-critical. Action 3 follows immediately: senior stakeholders and communicators need the known/unknown/next-update structure early, and providing it is exactly the translation duty of this role; note that 1 and 3 in practice run nearly in parallel, but a briefing with zero facts serves no one, so a first pass at 1 precedes 3. Action 4 is important but less time-critical by hours: invoking contractual duties formalises cooperation and preserves evidence, and benefits from the initial facts gathered in 1. Action 2 as written should never be executed: asserting "no citizen data affected" before the facts are established is precisely the overclaim that converts a supplier's breach into the organisation's own credibility crisis. The defensible version — "we are investigating, here is what we know and when we will update" — belongs to the communications team, informed by the action-3 briefing. Situational judgement scoring rewards spotting that one option is not merely last, but wrong in its stated form.

### Notes for the assessor

- **Score against the seniority of the role.** The candidate is being tested against the organisation's most senior security architecture voice, an escalation point, an exemplar — options that would suit a hands-off consultant or a compliance clerk are rarely the keyed answer at this level.
- **A recurring shape in the winning options.** Across the questions above, winning options share a shape: do the expert analysis, tell the truth with precision, give the decision-maker real options, and strengthen the system for next time — useful shorthand when scoring an answer that falls between the keyed most/least effective options.
- **Name the specific failure in debrief.** The discipline of naming why a weaker option is weak — passivity, overclaim, borrowed authority, hidden information — gives a candidate more useful feedback than a bare most/least effective verdict, and is worth doing for every question in the debrief.
- **Calibrate against public sector values.** User needs, transparency, collaboration, accessibility and inclusion decide several items above (notably Questions 3, 9 and 10); if two options seem close when scoring, the one that serves users and openness usually scores higher.
- **Do not reward idealism over effectiveness.** The scoring favours practically effective behaviour: option C in Question 10 beats a principled refusal precisely because it delivers the policy safely rather than dying on a procedural hill — judge the candidate's answers the same way.
...
- **Passing the problem along.** Routing feedback through someone's boss, awaiting instructions the candidate should be shaping themselves, or outsourcing judgements the role exists to make.
...
- **Ignoring the culture the candidate is being scored against.** UK public sector scenarios reward collaboration across departments, openness to scrutiny and service of user needs — behaviours a purely commercial framing ("give away our advantage") will misjudge.

## Conclusion

You have now worked through, with the candidate, the four assessment areas most relevant to principal security architect: cognitive ability, numeric reasoning, verbal reasoning, and situational judgement — built from the genuine materials of the role's working life: firewall rule sets and threat models, security programme budgets and loss-expectancy arithmetic, policies and assurance reports and board papers, and the judgement calls that come with being an escalation point, an exemplar, and a strategy-setter.

When scoring, mark each section against the worked answers above, and note not just whether the candidate reached the correct answer but whether their reasoning matches the explanation — a right answer reached for the wrong reason, and a wrong answer that reveals sound partial reasoning, both deserve a note in the record. A theme runs through all four sections and is worth watching for as you score: precision (the difference between a conditional obligation and outright non-compliance, between a percentage of the original figure and of the new one, between what a passage states and what it merely makes plausible); transparency, which runs through every judgement scenario, because in public service how a decision is made is inseparable from what is decided; and, in every domain, whether the strongest responses addressed the root issue and kept the decision-maker genuinely informed, rather than reaching for escalation, silence, or a shortcut.

Debrief the candidate promptly, while the session is fresh for both of you. Share which sections went well and which were weaker, using the explanations above to show what a strong answer looked like and why — this is valuable to the candidate regardless of outcome, and it is good practice, not a concession on rigour. Keep your scoring notes and the candidate's raw answers on file, and administer this document — the same items, the same time limits, the same order — in the same way for every candidate for this role level, so the process stays fair and defensible if it is ever reviewed.
