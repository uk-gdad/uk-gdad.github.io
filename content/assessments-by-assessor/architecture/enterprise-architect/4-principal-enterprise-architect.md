# Enterprise Architect (Principal Enterprise Architect) - Psychometric Assessments

## Introduction

This document is written for you: an assessor evaluating a candidate for principal enterprise architect within the UK Government Digital and Data profession. The candidate you are assessing is expected to lead at the highest level of the architecture practice, responsible for ensuring that enterprise strategy is developed, agreed and followed across business, technology and data domains. Whether you are running an internal assessment, chairing a promotion board, assessing a move from another department, or building the mental disciplines this level demands into your own selection process, this resource is designed to help you administer a fair, job-relevant assessment with confidence.

Psychometric assessments are structured, standardised exercises that measure the mental capabilities and behavioural judgement that predict performance in a specific role. They are widely used across the Civil Service and the wider public sector because they are fair, evidence-based and job-relevant. For a principal enterprise architect, they are not abstract puzzles: a well-designed assessment for this role asks the candidate to interpret portfolio investment data, reason precisely about architectural principles and policy wording, spot inconsistencies in roadmaps and reference architectures, and exercise judgement in politically sensitive, high-stakes stakeholder situations. In other words, they test what the role actually does.

This document contains four assessment guides, each aligned to a recognised assessment type:

1. **Workplace job-specific cognitive assessment** - pattern recognition, logical deduction, error checking and prioritisation using the artefacts a principal enterprise architect genuinely handles: roadmaps, capability models, transition states, governance decision logs, and Digital Twin architectures.
...
Each guide explains what the assessment measures, maps it explicitly to the skills in the role profile, and then gives a substantial set of items with full worked answers, held here as your answer key. Each guide closes with administration tips and common pitfalls to watch for at this level.

Administer this fairly. Use the same items and the same time limit for every candidate you assess for this role level - do not tailor the questions or the time allowed between candidates, and do not let one candidate see the items before another sits them. Keep the worked answers and explanations back as your answer key: they are for scoring, not for the candidate to read beforehand or during the assessment. After scoring, debrief the candidate - talk them through where their reasoning matched or diverged from the answer key, since that conversation is often more valuable to them than the score itself. And treat the result as one input among several: a psychometric assessment result should sit alongside the candidate's application, interview, portfolio and any other evidence you gather, never stand alone as the basis for a hiring or promotion decision.

How should you use this resource? Three suggestions. First, run each section under the stated time pressure: give the candidate the questions without the answers, and only consult the explanations once they have answered. Second, use the mapping to the role profile to explain, in the debrief, why each section is relevant to the job the candidate is being assessed for. Third, if you are preparing for a real sitting, work through one section at a time rather than the whole document at once, and revisit the pitfalls beforehand so you can recognise them in a live response.

This role operates at a level where clarity of thought under complexity is the core of the job. Treat this document as a toolkit for assessing exactly that, fairly and consistently.
...
A workplace job-specific cognitive assessment measures the exact mental processes this role demands, using practical workplace scenarios rather than abstract IQ-style items. For a principal enterprise architect, that means questions built around the artefacts the role handles every week: enterprise roadmaps with 'as is', transitional and 'to be' states; capability models; architectural principles and standards; governance decision logs; dependency maps across portfolios and programmes; and the data structures that feed a Digital Twin of the organisation.

Present it in the typical format: online, timed, and multiple choice. Allow 15 to 30 minutes, with somewhere between 15 and 25 questions, so roughly a minute per question - though harder items reward the candidate two or three minutes and easier items should take thirty seconds. Score objectively and, where you can, against a benchmark group of comparable professionals; where your platform reports speed and accuracy separately, look at both rather than a single figure. If your platform adapts difficulty as the candidate progresses, treat harder follow-up questions as a good sign rather than a concern.

Why does this matter for the role? Because at principal level, the candidate's value lies in seeing structure inside complexity: recognising that three programmes are unknowingly building the same capability, spotting the logical flaw in a transition plan before it costs a year, deducing the downstream consequences of a policy change on the organisation's reference architectures, and prioritising ruthlessly when everything is labelled critical. Cognitive assessments sample exactly these operations - pattern recognition, logical deduction, error checking, prioritisation and structured problem solving - under mild time pressure that simulates the pace of senior decision-making.
...
- **Architecture communication** and **Community collaboration**: error checking - finding the inconsistency in a roadmap, decision log or Digital Twin dataset before it misleads senior stakeholders or the architecture community the role leads.
...
An enterprise roadmap shows four capabilities progressing through maturity states: Initial → Repeatable → Defined → Managed → Optimising. The quarterly assessments read:
...
An organisation has established these architectural principles:
...
Applying the principle of maximising enterprise-wide flow of change while developing the community, which pair should be led personally?
...
**Explanation:** W and Z are the two items that are (i) blocking multiple teams now, and (ii) genuinely require principal-level authority - W is a cross-cutting dependency affecting three portfolios, and Z is an inter-departmental conflict that the role profile explicitly assigns to a principal enterprise architect: solving and unblocking issues between teams or departments at the highest level. X is already governed with conditions - delegable follow-up, not principal-level work. Y matters strategically but has a five-month runway and a capable deputy: delegating the draft develops the architecture community while direction is retained, exactly how a principal should multiply capacity. The trap options bundle one high-value item with one delegable item; prioritisation questions at this level test whether the candidate distinguishes urgency-plus-uniqueness from mere importance.

**Question 5 (moderate) - Dependency logic in transition states**

A transition plan contains these constraints:
...
3. The organisation's largest hosting contract expires in 20 months.
...
**Explanation:** The heart of horizon scanning at principal level is synthesis: individually, each signal is mildly interesting; together they define a decision window. Sovereign regions become available (signal 1) shortly before the contract expires (signal 3); policy direction (signal 2) suggests residency requirements will harden; the Digital Twin already needs residency guarantees (signal 4). Re-procurement is the natural lever to satisfy all of them at once. A ignores the convergence, B underweights policy risk, and D jumps from inference to commitment without any options analysis, breaching the discipline of making decisions characterised by high risk and complexity only after shaping the problem. Anticipating changes to policy and building resilience through the role's architectural work is exactly this move.
...
A Digital Twin ingests four feeds. The lineage register states:
...
Using expected exposure (impact x likelihood) adjusted for control status, which two risks should lead the intervention plan?
...
**Explanation:** The trap here is treating this as a pure critical-path question. If two modules could be taken in parallel, the longest prerequisite chain (Foundations → Domain architecture → Governance practice → Strategic advising) would give four quarters, and Architecture communication would slot alongside - hence the tempting answer B. But one architect can complete only one module per quarter, so every module occupies its own quarter: five modules stand between a new starter and completing Strategic advising, and all five are mandatory (Strategic advising requires both branches). The best any ordering can do is: Q1 Foundations, Q2 Domain architecture, Q3 Architecture communication (or Governance practice's prerequisite order swapped), Q4 Governance practice, Q5 Strategic advising - five quarters. The lesson for a strong candidate's answer is recognising whether the binding constraint is the dependency graph or the resource limit. Here the resource limit (one module per quarter) dominates, exactly as a single shared platform team, not the plan's logic, often dominates a transition timeline. Developing the architecture community includes designing pathways whose durations are honest about such constraints.

### Administration tips

- **Build items from real artefacts if you adapt this section.** A real roadmap, decision log or dependency map you interrogate for its critical path and contradictions produces questions the candidate will recognise as authentic to the role, rather than as abstract puzzles.
- **Expect the answer to hide in a comparison, not a raw figure.** Freshness relative to cadence, exposure relative to controls, progress relative to pattern - strong candidates at principal level look for a ratio or comparison rather than reading off a headline number, and that is worth noting when you score a response.
- **Hold a consistent time limit.** Around a minute per question on average is a reasonable allowance; tell every candidate for this role level the same limit and give them the same guidance on flagging a question and moving on if it resists them after two and a half minutes.
- **Watch how the candidate reads quantifiers.** "All new services", "unless granted an exception", "necessarily follows" - at this level the discriminator is rarely the arithmetic; it is whether the candidate notices which words constrain the answer, and that is worth listening for if you debrief them on their reasoning.
- **Offer a short warm-up before a live sitting.** Ten minutes of logic puzzles or one practice set immediately before the assessment measurably reduces slow starts for candidates, and a candidate who is rested performs more consistently than one who is not - senior candidates most often underperform through fatigue and overconfidence, not ability.

### Common pitfalls to watch for

- **Importing outside knowledge.** An experienced candidate knows how government architecture "really works" - but the question's rules are the whole universe, and a strong answer works from the material given, exactly as the role holds a programme to the written standard rather than to folklore. Watch for a candidate reasoning from what they know rather than what the item states.
- **Anchoring on dramatic details.** Impact-5 risks, "urgent" programmes and large numbers can draw a candidate's attention away from the option the logic actually supports. A strong answer computes before it commits.
- **Serialising parallel work.** In dependency questions, the most common wrong answer comes from adding durations that the constraints allow to overlap. A strong candidate sketches the chains rather than assuming everything runs in sequence.
- **The time-sink question.** One fiendish multi-stage item can consume a quarter of a candidate's window. Whether they skip it and return, or work it through, is itself informative - it shows how they triage an overloaded agenda under time pressure.
- **First-instinct lock-in.** As Question 13 demonstrates, a fast plausible answer can survive right up until it is checked. A candidate who builds a brief verification pass into their answers - does this choice satisfy every stated constraint? - tends to catch errors that a confident first instinct would miss.

## Workplace job-specific numeric reasoning assessment

### About this assessment

A workplace job-specific numeric reasoning assessment evaluates how well a candidate analyses, interprets and makes sound decisions using numerical data drawn from the role's actual working context. It is business mathematics, not abstract mathematics: percentages, ratios, weighted comparisons, cost projections and capacity figures, presented in the tables, dashboards, and financial summaries a principal enterprise architect genuinely encounters. Allow or provide an on-screen calculator, because the skill being measured is interpretation and decision quality, not mental arithmetic.

Set a strict time limit - commonly 20 to 30 minutes for 15 to 20 questions - and use inputs such as portfolio budget tables, supplier pricing schedules, cloud consumption reports, benefits-realisation dashboards, and Digital Twin data-quality metrics. Questions typically require two or three extraction-and-calculation steps: find the right figures, combine them correctly, and compare the result against options or a threshold.

For this role, numeric fluency is not optional polish; it is how architectural argument becomes executive decision. When a principal enterprise architect recommends reuse over build, the recommendation stands or falls on a defensible cost comparison. When they guide the organisation towards sustainability and scalability, they quantify consumption trajectories. When they advise on supplier selection, they evaluate weighted commercial scores. When they present the case for retiring legacy estate to a spending review, the numbers must survive scrutiny by finance directors. An assessment of this kind samples exactly that capability: can the candidate move from a dense table to a correct, decision-ready figure, quickly and without being misled?
...
- **Architecture communication**: translating calculations into figures senior stakeholders can trust - which requires getting them right first.

### Items

**Question 1 (easy) - Percentage change in portfolio spend**

A technology portfolio spent £48.0m last financial year. The spending plan for this year is £42.0m. What percentage reduction does the plan represent?
...
**Explanation:** Build: £850,000 + (4 x £120,000) = £850,000 + £480,000 = £1,330,000. Reuse: £150,000 + (4 x £95,000) = £150,000 + £380,000 = £530,000. Saving = £1,330,000 - £530,000 = £800,000. Option A is the difference in one-off costs only (£850k - £150k); C is the reuse total minus a misread; D is the build total mistaken for the saving. This is the core value-for-money arithmetic behind "recommend reuse... to achieve value for money and reduce risk" - the discipline it tests is comparing totals over a stated horizon, never headline build costs alone.

**Question 3 (moderate) - Weighted supplier evaluation**

The candidate is moderating a supplier evaluation for an integration platform. Scores (out of 10) and weightings:
...
A Digital Twin's analytics workload consumes 180,000 compute-hours per quarter at £0.42 per hour. Right-sizing is projected to cut consumption by 25%, and moving to a greener region adds 5% to the unit rate but reduces the workload's carbon footprint by 40%. If both changes are made, what is the new quarterly compute cost?
...
**Explanation:** New consumption: 180,000 x 0.75 = 135,000 hours. New rate: £0.42 x 1.05 = £0.441 per hour. New cost: 135,000 x £0.441 = £59,535. Option A is the original cost (180,000 x £0.42 = £75,600); C applies the consumption cut but not the rate rise; D applies the rate rise but not the consumption cut. The carbon figure is context, not calculation - assessments often include a number the candidate does not need, to test extraction discipline. Recommending sustainability alongside value for money means being able to show that the green option here costs £16,065 less per quarter than the status quo, not more.
...
**Explanation:** Total cost = £12m + £3m = £15m. Required total benefits for a ratio of 1.4 = 1.4 x £15m = £21m. Benefits already realised = £11m, so year 3 must deliver £21m - £11m = £10m. Option B works towards the original £24m claim; C and D result from ratio errors (using 1.4 x remaining cost, or halving). Note the strategic insight the arithmetic reveals: year 3 must deliver nearly as much benefit as the first two years combined - a signal a principal enterprise architect would challenge hard when assuring that decisions align with enterprise strategy, and exactly the kind of quantified scepticism senior stakeholders expect at this level.

**Question 7 (moderate) - Capacity headroom for a shared platform**

A shared integration platform handles a peak of 5,400 transactions per second (tps) against a tested capacity of 9,000 tps. Two new consuming services are due: one adds a projected peak of 1,800 tps, the other 1,350 tps. The organisation's standard requires 25% headroom above projected peak at all times. After onboarding both services, is the standard met?
...
**Explanation:** Projected peak: 5,400 + 1,800 + 1,350 = 8,550 tps. The standard requires capacity of at least 8,550 x 1.25 = 10,687.5 tps, but tested capacity is 9,000 tps - a shortfall. Equivalently, actual headroom is (9,000 - 8,550) / 8,550 = 5.3%, far below 25%. Option A confuses "within capacity" with "meets the headroom standard" - the precise distinction the standards exist to enforce. C invents a different measurement basis; D is factually wrong (5,400 < 9,000 today). Scalability guidance is only as good as the arithmetic behind it: this is how the role converts a standard into a concrete onboarding decision - onboard one service now, or expand capacity first.

**Question 8 (moderate) - Digital Twin data coverage**

A Digital Twin aims to represent 100% of the organisation's 2,400 technology assets. Current coverage: infrastructure assets 92% of 1,500; application assets 75% of 640; data assets 60% of 260. What is the overall coverage percentage, to the nearest whole number?
...
**Explanation:** Covered assets: infrastructure 0.92 x 1,500 = 1,380; applications 0.75 x 640 = 480; data 0.60 x 260 = 156. Total covered = 1,380 + 480 + 156 = 2,016. Coverage = 2,016 / 2,400 = 0.84 = 84%. The tempting shortcut - averaging the three percentages, (92 + 75 + 60) / 3 = 75.7% ≈ 76%, option A - is wrong because the categories differ enormously in size: infrastructure's 1,500 assets must weigh more than data's 260. This is the weighted-average trap, and it appears constantly in real coverage, compliance and adoption reporting. As the owner of the Digital Twin's creation, maintenance and consumption, a principal enterprise architect presents coverage figures to boards; the difference between a naive average and a weighted one is the difference between a defensible figure and an embarrassing correction.

**Question 9 (hard) - Ratio reasoning on architecture community growth**

An architecture community has 45 members: enterprise, solution, and technical architects in the ratio 2:4:9. The strategy requires the enterprise-to-solution ratio to reach 1:1.5 within two years while the technical architect headcount stays fixed. If solution architect numbers stay fixed too, how many additional enterprise architects must be developed or recruited?
...
**Explanation:** Ratio 2:4:9 across 45 people means each ratio unit is 45 / 15 = 3 people: 6 enterprise, 12 solution, 27 technical architects. Target enterprise:solution = 1:1.5 with solution fixed at 12 means enterprise must reach 12 / 1.5 = 8. Additional needed = 8 - 6 = 2. Option C comes from misreading the target as 1.5:1 (which would need 18 enterprise architects, +12 - not an option, prompting a second look); B and D come from unit errors. Multi-step ratio questions reward writing down the intermediate headcounts rather than manipulating ratios mentally - and this particular calculation is one the role genuinely performs when growing the community with a workforce plan.
...
A strategy commits to reducing annual technology spend from £60m to £48m over three years, with reductions phased 20% of the total in year 1, 30% in year 2, and 50% in year 3. At the end of year 2, actual annual spend is £56.5m. How far behind the planned trajectory is the organisation, in £m of annual spend?
...
**Explanation:** Now: 5,000 x £200 = £1,000,000 + £350,000 = £1,350,000. In two years: unit cost = £200 x 0.85 x 0.85 = £200 x 0.7225 = £144.50; units cost 5,000 x £144.50 = £722,500; plus £150,000 integration = £872,500. Difference = £1,350,000 - £872,500 = £477,500. Option B compounds the price decline only one year (£170 per unit); D looks only at the integration-cost difference in the wrong direction. Two disciplines matter: compound the percentage decline (not 30% off), and include both cost components. The strategic judgement - whether two years of foregone benefit outweighs £477,500 - is the candidate's to argue; the assessment tests whether the number they argue from is right.

**Question 14 (easy) - Meeting the community training budget**

The candidate has a £90,000 development budget for a 45-strong architecture community. A certification pathway costs £2,400 per person. They want to certify as many people as possible while retaining at least £15,000 for community events. How many architects can be certified?
...
**Explanation:** Available for certification = £90,000 - £15,000 = £75,000. £75,000 / £2,400 = 31.25, and a quarter of a person cannot be certified, so 31. Option A ignores the events reserve (90,000 / 2,400 = 37.5 → 37); C rounds 31.25 up, which would breach the budget by £1,800; D over-rounds down. Two habits: subtract reservations before dividing, and always round down when the unit is a whole person or licence. Growing the community within a real budget makes this the least hypothetical question in this section.

**Question 15 (moderate) - Carbon and cost trade-off for sustainability reporting**

Two data centre options for a workload: Option P consumes 480 MWh per year at £185 per MWh with a carbon intensity of 0.18 tonnes CO2e per MWh. Option Q consumes 520 MWh per year at £160 per MWh with a carbon intensity of 0.05 tonnes CO2e per MWh. The organisation's sustainability standard prices carbon internally at £80 per tonne for decision-making. Including the internal carbon price, which option has the lower total annual decision cost, and by how much?
...
**Explanation:** Option P: energy cost = 480 x £185 = £88,800; carbon = 480 x 0.18 = 86.4 tonnes, priced at 86.4 x £80 = £6,912; total decision cost = £95,712. Option Q: energy cost = 520 x £160 = £83,200; carbon = 520 x 0.05 = 26 tonnes, priced at 26 x £80 = £2,080; total decision cost = £85,280. Q is lower by £95,712 - £85,280 = £10,432. Option C is the energy-cost difference alone (£88,800 - £83,200 = £5,600), the answer reached if the carbon price is forgotten entirely; A and D reverse the direction by mishandling Q's higher consumption. Two durable lessons: apply the internal carbon price to both options symmetrically, and remember that higher energy consumption does not automatically mean higher total cost once unit rates and carbon intensity differ. Guiding the organisation on sustainability to achieve value for money depends on exactly this combined cost-and-carbon calculation, which a principal enterprise architect presents in investment cases and sustainability reports.

### Administration tips

- **Watch for extraction errors, not just calculation errors.** Most errors at this level are extraction errors: the wrong row, the wrong year, "per quarter" read as "per year", or a percentage of the wrong base. A strong candidate states which figures they need and where they are before calculating, and that habit is worth listening for.
- **Build further items from artefacts you already hold, if you extend this section.** Spending review submissions, supplier pricing schedules, cloud billing reports and benefits dashboards are ideal source material - convert real figures into questions of the forms above.
- **Five operations recur.** Percentage change from the correct base; compound growth and decline; weighted averages; expected values across probabilistic branches; and totals over a stated horizon. Nearly every question in a role-specific numeric test for a senior architect is one of these wearing different clothes, so a candidate's fluency across all five is worth checking deliberately.
- **Expect the calculator to be used for arithmetic, not for structure.** A strong candidate decides the calculation shape before touching the calculator; a candidate who dives straight into keying without stating the structure first is more likely to produce a wrongly shaped answer, however precisely calculated.
- **Look for sanity-checking of magnitudes.** A downtime figure of 4.5 minutes versus 45 minutes, a saving of £0.4m versus £4m - a candidate who checks the order of magnitude of their answer catches decimal slips that plausibility checks alone will not, and that is a good sign in a live response.

### Common pitfalls to watch for

- **Misreading scale qualifiers.** "In thousands", "per quarter", "FTE-months": a single missed qualifier invalidates an otherwise perfect calculation. A careful candidate marks every unit before beginning; its absence in a candidate's working is worth noting.
- **Percentage-base errors.** Dividing the change by the new value instead of the original, or applying two successive percentages additively rather than compounding. The wrong options in these items are engineered so those errors land exactly on a distractor, which makes the error easy to spot if the candidate shows their working.
- **Over-calculating.** Some questions yield to estimation: if three options are of the wrong magnitude, a rough calculation identifies the answer in seconds. A candidate who reaches for full precision under time pressure, when an estimate would do, is spending time they may need later.
- **The time-sink question.** Multi-branch expected-value items (like Question 10) can absorb five minutes. Whether a candidate banks the simpler questions first or works through in order is itself informative about how they navigate a timed test.
- **Forcing a match.** When a computed value is not among the options, a weaker candidate silently bends their arithmetic towards the nearest choice; a stronger one rechecks extraction first, and if their figure still stands, picks the nearest option and moves on, flagging the discrepancy rather than hiding it. That same discipline - reporting a discrepancy rather than reconciling it away - is exactly what protects the integrity of every dashboard the role owns, and is worth probing for in debrief.

## Workplace job-specific verbal reasoning assessment

### About this assessment

A workplace job-specific verbal reasoning assessment evaluates how accurately a candidate understands, analyses and draws conclusions from written material of the kind this role actually generates and consumes. The passages are not literary; they are the dense, consequential prose of the working life of a principal enterprise architect: architectural principles and standards documents, strategy papers, governance terms of reference, commercial framework guidance, cross-government policy circulars, and emails from senior stakeholders whose exact wording matters.

Present the dominant format: a passage followed by statements the candidate must classify as **True** (the passage says or necessarily implies it), **False** (the passage contradicts it), or **Cannot Say** (the passage provides insufficient information either way) - answered strictly from the text, ignoring anything the candidate happens to know from experience. Some assessments add multiple-choice comprehension items: identifying the best summary, the author's main claim, or the precise obligation a clause creates. Set brisk timing - often under a minute per question - to simulate the reality of absorbing a forty-page strategy paper the evening before a board.

Why does this matter at principal level? Because words are the governance instruments of the role. The difference between "services must use the enterprise identity platform" and "services should normally use the enterprise identity platform" is the difference between a mandate and a preference - and a principal enterprise architect is the person who writes, interprets and defends such clauses. They mediate difficult architectural discussions in which each side quotes documents selectively; they assure that decisions align with strategy by testing them against the strategy's actual words; they represent architectural governance in legal and commercial forums where imprecise reading has contractual consequences. This assessment measures the precision of reading on which all of that rests.

### What it measures for this role

Mapped against the skills in the role profile:

- **Architecture communication**: extracting exactly what a document claims, so it can be communicated faithfully to technical and non-technical stakeholders at all levels, and mediating accurately when parties dispute what a text means.
...
**Explanation:** The passage states that organisations "reporting the greatest value... were those that had first invested in data governance" - C restates this association precisely, without upgrading it to causation. A inverts the stated ranking: falling costs were primary, regulation secondary. B strips a conditional finding of its condition - only twins "built on poorly governed data" frequently misled. D relocates the doubling: adoption doubled across European public sector organisations overall, while transport and utilities describe where adoption is *concentrated*, not where it doubled. Each wrong option is a subtle misassignment of a true fragment - the exact failure mode of hurried executive summaries, and the reason the role profile pairs horizon scanning with the judgement to report it accurately.

### Administration tips

- **Expect the three-way verdict to be the real discrimination.** True/False/Cannot Say is a discipline, not a reading style every senior professional already has. The key move a strong candidate makes is asking "does the text *establish* this?" rather than "is this *believable*?". Experienced candidates most often get Cannot Say wrong, because they know too much and want to answer from expertise rather than from the text.
- **Note whether the candidate reads the statement first, then hunts.** Under time pressure, a strong approach reads the question statement, identifies its load-bearing terms, and scans the passage for exactly those terms and their qualifiers, rather than reading every passage in full first.
- **Watch for attention to modality and quantifiers.** "Must", "should", "may", "expected to", "all", "only", "unless", "at this stage" - these small words decide most answers. This mirrors how the role already reads standards and contracts; the assessment simply measures whether the candidate does it reliably at speed.
- **Build further items from your own corpus, if you extend this section.** Take a real principles document, framework guidance note or strategy paper and write five True/False/Cannot Say statements about it - writing items surfaces the traps a setter would use, which sharpens the assessment.
- **Hold a consistent pace.** Around 45-60 seconds per item is a reasonable allowance; tell every candidate the same guidance - choose a best answer and move on if a verdict will not settle within it, rather than spending three minutes on one question.
- **Score against the two overreach checks.** Before crediting a True answer, check whether the passage establishes *every* element of the statement, including its conditions and its population. Before crediting a False answer, check whether the passage actually *contradicts* the statement, or merely fails to support it. Most wrong answers at senior level are overreaches in one of these two directions, and each check takes five seconds. Statements built from true fragments wrongly combined - the right fact attached to the wrong population, condition or timeframe - are the setter's favourite construction, and only element-by-element checking catches a candidate who has fallen for one.

### Common pitfalls to watch for

- **Importing expert knowledge.** An experienced candidate knows how design authorities, procurement frameworks and spending reviews actually behave. The test's universe is the passage alone; every time a candidate's experience supplies a missing fact, they drift from True or False towards an unearned verdict. Cannot Say exists precisely for statements a knowledgeable candidate's experience wants to confirm.
- **Rounding off modality.** Upgrading "is expected to" into "will", "not yet persuaded" into "rejects", or "should normally" into "must". The passage's hedges are data, not noise, and a candidate who smooths them over is worth noting.
- **Letting the reassuring clause eclipse the obligating clause.** Compound sentences often grant with one hand and oblige with the other; weaker readers remember the grant. A strong candidate reads to the full stop.
...
- **Verdict drift across a question set.** After three consecutive Cannot Say answers, some candidates start avoiding it, suspecting a pattern. Item writers know this, which is why the set is built the way it is. Judge each of a candidate's answers on its own text - the same independence the role would demand of an assurance review.

## Workplace job-specific situational judgement assessment

### About this assessment

A workplace job-specific situational judgement assessment evaluates decision-making, interpersonal skill and professional conduct by presenting realistic workplace dilemmas and asking which responses are most and least effective. Unlike the cognitive, numeric and verbal assessments, there is rarely a single objectively right answer; instead, score responses against the values and behaviours the organisation needs from someone in this role. Formats vary: choose the most effective and least effective actions from four or five options; rank all options from best to worst; or rate each option's effectiveness on a scale. Timing is usually generous or absent, because the assessment targets natural judgement rather than speed.

For a principal enterprise architect, the scenarios are political, strategic and human: a director-general who wants to bypass governance, two departments at war over a shared platform, a supplier relationship souring at the worst moment, an architecture community losing faith in its leadership, a horizon-scanning insight nobody wants to hear. The role profile is explicit that this role networks and communicates with senior stakeholders, mediates difficult architectural discussions, unblocks issues between departments at the highest level, and inspires other architects. Situational judgement assessments sample precisely those behaviours.

Score in UK public sector contexts by typically rewarding options that serve user needs and the long-term public interest, act transparently, collaborate rather than command, uphold standards without rigidity, address problems at their root, and develop others. Options that are passive, evasive, political in the self-serving sense, or that sacrifice long-term integrity for short-term comfort should score poorly - as should idealistic options that ignore practical reality.
...
A director-general sponsoring a politically high-profile programme tells the candidate the programme will not submit its solution architecture to the design authority: "We don't have time for your committee - the minister has announced the date." The architecture, from what the candidate has glimpsed, duplicates an existing cross-government platform.

A) Agree to waive the review, given the ministerial commitment, and note the candidate's concerns in an email for the record.
B) Insist the programme joins the next scheduled design authority meeting and warn that the candidate will escalate non-attendance to the permanent secretary.
C) Offer an expedited, focused review within the week - out of committee, under the candidate's discretion as chair - targeting only the highest-risk decisions, including the apparent duplication, and explain that this protects the minister's date better than discovering the duplication in eighteen months.
D) Ask the candidate's team to review the architecture quietly from the documents they can obtain, without telling the DG, and raise issues only if they find something serious.

**Most effective: C. Least effective: A.**

**Explanation:** C demonstrates the judgement this role exists to provide: governance that flexes its process while holding its purpose. It uses a legitimate mechanism (urgent out-of-committee consideration), concentrates scrutiny where risk lives, and - critically - reframes governance as protecting the DG's own objective, which is how a principal enterprise architect gains support from senior stakeholders for high-risk topics. A is least effective: it converts the role's accountability into a defensive paper trail, abandons the value-for-money duty to challenge duplication, and teaches every future programme that ministerial announcements dissolve assurance. B holds the line but rigidly: leading with escalation threats damages a senior relationship the role needs, and treats process as the point rather than the risk. D is covert and passive - reviewing without access or candour produces weak assurance and, if discovered, corrodes the transparency on which the role's influence depends.

**Question 2 (moderate) - Two departments, one platform, open warfare**

Two departments co-fund a shared integration platform. Department X accuses Department Y of consuming disproportionate capacity; Y accuses X of blocking its releases. Both have escalated to the candidate, each demanding a ruling in their favour, and both have hinted they will withdraw funding if the ruling goes against them. The candidate's analysis suggests the platform's real problem is an outdated capacity model that neither department has seen.
...
**Explanation:** B sequences integrity correctly. First, verify - the programme director's challenge that the data "must be wrong" deserves a genuine test, and the role's accountability for the Twin's maintenance means its credibility rests on exactly such verification. Second, engage the peer with evidence and a collaborative offer - mediation before escalation. Third, state the boundary transparently: a gateway review proceeding on data known to be misleading implicates the architect's professional integrity, and saying so plainly is respectful, not hostile. D is least effective: suppressing inconvenient data from the organisation's decision-support asset is the one act that destroys the Twin's entire purpose - worse than any single wrong number, because it makes the instrument itself untrustworthy. A converts a three-week delay into complicity with a potentially misleading review. C is transparent but skips verification and peer engagement, maximising relationship damage while risking that an error gets escalated.

**Question 10 (moderate) - Coaching a struggling enterprise architect**

An enterprise architect the candidate manages has excellent technical judgement but is losing stakeholder support: their design authority interventions are experienced as lectures, and a programme director has asked for them to be replaced on a key programme. The architect believes the stakeholders are the problem.
...
**Explanation:** Work by marginal risk. Option 1 is the most defensible cut: a stable, well-understood domain's documentation refresh has the lowest cost of delay - the knowledge exists; the artefact is merely ageing. Option 3 is painful but time-bounded: two quarters of reduced horizon scanning raises the risk of missing an external signal, yet the capability restarts intact - though at principal level a strong candidate would keep a minimal personal watch regardless. Option 2 cuts deeper: cancelling development for a year sends a lasting signal to a community whose morale and retention the role stewards, and capability lost to resignations does not restart on budget recovery - which is why it ranks as less defensible than the time-bounded scanning pause. Option 4 is the least defensible: suspending assurance for in-flight programmes removes risk control exactly where money is being spent fastest, and undetected architectural failures compound - the cut that costs most precisely when budgets are tightest. Rankings like this test whether a candidate can articulate *why* one harm exceeds another: cost of delay, reversibility, signal effects and risk exposure, not sentiment.

**Question 12 (moderate) - The minister's advisor wants a name**

At a cross-government event, a special advisor asks the candidate informally which of two departments' architectural approaches is "better", making clear the answer may inform a ministerial view on where a new flagship service should be built. The candidate knows both architectures well; one is genuinely stronger for this service's needs, but the comparison is nuanced and the weaker department is six months into promising improvements.
...
**Explanation:** C threads the needle that senior architects must thread constantly: being genuinely useful to decision-makers while keeping consequential advice within channels that allow nuance, evidence and fairness. An informal verdict at an event (A) strips out exactly the context - service-specific fit, improvement trajectory - that makes the candidate's judgement worth having, and a hallway sentence may travel further and harder than intended; candour is right, but candour includes the caveats. D is least effective because it is a small dishonesty: a principal enterprise architect does have a professional view, and flattening it into diplomatic mush deprives the decision of the expertise the role exists to provide - while still leaving the advisor to decide on worse information. B is honest but unhelpfully closed; declining to answer *and* declining to offer a proper route abandons influence the organisation needs the role to exercise. Networking and communicating with senior stakeholders across enterprises means exactly this: substance over verdicts, channels over corridors, usefulness over evasion.

### Administration tips

- **Score against the role, not against the assessor's own style.** A strong response answers as the principal enterprise architect the organisation needs, exercising the accountabilities in the role profile - community development, strategy stewardship, governance integrity - even where a candidate's personal style might differ from the assessor's own.
- **Look for the option that fixes the root cause and preserves the relationship.** Strong options usually combine candour, evidence, an appropriate channel and a constructive path forward. An option that is honest but relationship-destroying, or smooth but evasive, is rarely the strongest choice.
- **Check the sequencing in a candidate's reasoning.** Many scenarios turn on order: verify before challenging, engage the peer before escalating, take soundings before the committee, respond directly before reporting. The right actions in the wrong order score as poorly as wrong actions, so probe for the sequence in debrief, not just the destination.
- **Watch for the seductive extremes.** The rigid enforcement option and the total capitulation option are both usually wrong; so are "quietly work around it" options - covert action almost always scores worst in public sector scoring, because it defeats transparency.
- **Build further items from real dilemmas, if you extend this section.** Take three difficult situations from the last year and write four response options for each, then rank them and articulate why. The articulation - cost of delay, reversibility, trust effects - is the skill being scored, and it is worth asking a candidate to articulate the same reasoning in debrief.

### Common pitfalls to watch for

- **Choosing the idealistic answer over the effective one.** "Refuse all compromise on standards" sounds principled but ignores the statutory deadline, the user need or the political reality. Score for wise trade-offs, not purity.
...
- **Ignoring the development dimension.** Scenarios involving the candidate's community or their architects almost always contain a coaching opportunity; options that discard people or skip the feedback conversation miss behaviours the role profile names explicitly.
...
Well done for working through this resource. It covers the four assessment types most relevant to this role: cognitive reasoning with the roadmaps, decision logs, dependency chains and Digital Twin data a principal enterprise architect genuinely owns; numeric reasoning across supplier evaluations, legacy cost curves, benefits realisation, and sustainability trade-offs; verbal reasoning against the principles documents, strategy papers, commercial guidance and senior correspondence whose exact wording is the role's working material; and situational judgement in the political, human and strategic dilemmas that define leadership at principal level.

When you score responses, work from the answer key held back in each section: for cognitive, numeric and verbal items, compare the candidate's answer against the stated correct answer and use the explanation to judge partial credit where their working shows sound reasoning that reached the wrong option through a small slip. For situational judgement items, score against the most-effective and least-effective answers given, and use the explanation to weigh a candidate's own reasoning if you ask them to talk through their choice. Keep a record of each candidate's answers and your scoring rationale - it supports a fair debrief and gives you something concrete to compare across candidates for the same role level.

After scoring, debrief the candidate. Walk through where their reasoning matched the answer key and where it diverged, and be candid about both strengths and gaps - this is often the most valuable part of the process for the candidate, whatever the outcome of the wider selection decision. Debriefing well also protects the credibility of the assessment: a candidate who understands why an answer was marked as it was is more likely to trust the process, win or lose.

As next steps, consider three things. First, administer this assessment the same way for every candidate you assess for this role level: the same items, the same time limits, and the same scoring approach, so that comparisons between candidates are fair. Second, if you extend or adapt any section, build new items from real artefacts and dilemmas in the way this document does, and keep the same rigour - a worked answer and an explanation for every item, and no item that depends on knowledge outside what is given. Third, remember that this assessment is one input among several: combine its result with the candidate's application, interview and any other evidence before reaching a hiring or promotion decision, and consult your organisation's human resources, people development, occupational psychology or legal compliance colleagues if you are unsure how much weight to give it.
