# Infrastructure Engineer - Competency Assessment

## Introduction

This assessment is for an assessor — a line manager, an interview panel
member, or anyone else evaluating a candidate or an employee — who needs to
judge whether a person has the baseline abilities the IT operations
profession's infrastructure engineer role level requires. It measures
demonstrated skill against the UK GDAD Profession Capability Framework's own
description of this level, for a hiring decision, a promotion or progression
board, onboarding a new starter, or a periodic capability check. It is one
input into that decision, never the whole of it — see `## Advisory` before
you begin.

## Advisory

This competency assessment is created by AI and is a work in progress for
educational purposes. It is not a validated assessment instrument — it has
not been piloted, normed or checked for adverse impact — and a rating on this
form must never be the sole basis for a hiring, promotion, pay, performance or
disciplinary decision. Before using it, consult your organisation's human
resources, people development, occupational psychology, or legal compliance
colleagues.

## How to use this assessment

**Preparing.** Read `## About this role level` first, so you know what the
framework actually expects before you look at any evidence. Then gather what
exists for this person: a CV or portfolio, a structured interview, a work
sample or task, direct observation of them at work, or references from people
who have worked with them. At this level the framework expects infrastructure
solutions and services to be built, configured, administered and supported as
directed and according to departmental policy, working with third parties, so
look for evidence of delivering against a defined policy rather than only
following instructions step by step.

**Score on evidence, not impression.** Every rating must trace to something
the person did, said, produced, or was observed doing. If you cannot point to
evidence for a rating, record it as unknown rather than guess — an honest gap
teaches you more than a guess dressed up as a score.

**Stay consistent.** Score every candidate for this role level the same way,
using the same evidence sources wherever you can. For a hiring or promotion
decision, have more than one assessor score independently, then compare notes,
rather than settling on a single view first.

**Fairness.** An unvalidated, AI-drafted instrument like this one, used
inconsistently, can produce unlawful adverse impact even when nobody intends
it. Consistent, evidence-based administration by more than one assessor
materially reduces that risk. Never let a rating be influenced by anything
other than demonstrated, job-related evidence.

## Rating scale

Every skill in the competency matrix is rated on the same four-point
proficiency scale — how well the person does the thing, on the evidence
available, not how often they do it.

| Value | Rating | Meaning |
| ---: | --- | --- |
| 1 | Awareness | You can describe skill fundamentals and show basic knowledge of related tools |
| 2 | Working | You can apply the skill with some support and use appropriate tools |
| 3 | Practitioner | You can apply the skill independently, determine the best tools, and share your experience |
| 4 | Expert | You can lead organisational best practice and teach advanced techniques |

**Worked example.** Take Skill: Systems design. At 1 — Awareness, a candidate
assists with a design component under close direction. At 2 — Working,
the baseline for this level, they translate a logical design into a physical
one, produce a detailed design and document it to the required standards,
working with well understood technology and managed levels of risk and
complexity — the framework's own wording for this level. At 3 — Practitioner,
they take on systems of medium risk and complexity unassisted, and review
others' work. Calibrate against evidence like this before you start scoring.

## About this role level

IT operations role: Infrastructure engineer. An infrastructure engineer
designs, builds, manages and supports the infrastructure services that
underpin all internal user services and services to the public.

The role has five levels, in order: associate infrastructure engineer,
infrastructure engineer, senior infrastructure engineer, lead infrastructure
engineer, and principal infrastructure engineer. This assessment covers
infrastructure engineer, the second of those.

Role level: Infrastructure engineer. Working in a team of infrastructure
specialists and engineers, an infrastructure engineer supports and maintains
infrastructure solutions and services as directed and according to
departmental policy.

At this role level, the framework expects someone to:

- ensure services are delivered and used as required
- work with and support third parties to provide infrastructure services

The framework names eleven skills for this level: Coding and scripting, IT
infrastructure, Information security, Modern development standards, Ownership
and initiative, Problem management, Service focus, Systems design, Systems
integration, Testing, and Troubleshooting and problem resolution.

## Competency matrix

### Skill: Coding and scripting

**What the framework expects at this level**

An infrastructure engineer can describe different scripting tools and
software that are available and currently in use.

**Baseline for this role level**

Baseline: 1 — Awareness. Describing available tools, rather than writing or
maintaining scripts, is knowledge-level work suited to guided practice at this
stage.

**Behavioural indicators**

- 1 — Awareness: describes the scripting tools and software currently in use
  when asked.
- 2 — Working: explains why a particular scripting tool is used for a
  given task.
- 3 — Practitioner: writes or amends a simple script using one of the described
  tools.
- 4 — Expert: recommends which scripting tool to use for a new task.

**Evidence to gather**

- Ask the candidate to describe the scripting tools and software the team
  currently uses, and what each is for.
- Ask for an example of a task where a particular scripting tool was the
  right choice.
- Ask a referee whether the candidate's knowledge of the team's scripting
  tools is accurate and current.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: IT infrastructure

**What the framework expects at this level**

An infrastructure engineer builds, configures, administers, and supports
infrastructure solutions and services — for example, networking and physical
infrastructure, commercial-off-the-shelf (COTS) software, open source
packages and solutions, or virtual and cloud computing.

**Baseline for this role level**

Baseline: 2 — Working. Building and configuring solutions, not only
supporting them as at the associate level, is applied, guided delivery — a
genuine step up, though not yet independent design work.

**Behavioural indicators**

- 1 — Awareness: builds or configures an infrastructure component with a
  colleague checking the work.
- 2 — Working: builds, configures, administers, and supports
  infrastructure solutions and services as directed, without needing a check
  at every step.
- 3 — Practitioner: builds and administers infrastructure across several areas
  unassisted.
- 4 — Expert: sets how infrastructure in their specialism is built and
  administered.

**Evidence to gather**

- Ask the candidate to walk through an infrastructure solution they built or
  configured end to end.
- Observe or ask them to describe administering a virtual or cloud computing
  service.
- Ask a referee how much direction the candidate still needs on
  infrastructure build and configuration tasks.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Information security

**What the framework expects at this level**

An infrastructure engineer can explain information security and the security
controls available to protect solutions and services.

**Baseline for this role level**

Baseline: 1 — Awareness. The framework's wording for this skill is unchanged
from the associate level, so the baseline holds; this level's growth is
expressed through IT infrastructure and Systems design instead.

**Behavioural indicators**

- 1 — Awareness: explains the basic purpose of information security and names
  a security control when asked.
- 2 — Working: explains several security controls accurately and links
  them to a real solution they have worked on.
- 3 — Practitioner: applies security control knowledge to spot a gap in a
  solution's protection.
- 4 — Expert: advises colleagues on which security controls to apply.

**Evidence to gather**

- Ask the candidate to explain, in their own words, what a named security
  control protects against.
- Ask for an example of a security control they have applied to an
  infrastructure solution.
- Ask a referee whether the candidate's explanations of security concepts are
  accurate.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Modern development standards

**What the framework expects at this level**

An infrastructure engineer explains the most important principles of modern
development standards and how they apply to their work, and applies modern
development standards principles under guidance.

**Baseline for this role level**

Baseline: 2 — Working. Applying the principles, even under guidance,
rather than only explaining why they matter, is a real step up from the
associate level.

**Behavioural indicators**

- 1 — Awareness: explains why modern development standards matter, but has
  not yet applied one.
- 2 — Working: explains the most important principles and applies them
  to their own work with guidance.
- 3 — Practitioner: applies modern development standards to their work
  unassisted.
- 4 — Expert: supports others in applying modern development standards.

**Evidence to gather**

- Ask the candidate to explain a modern development standard's principle and
  how they applied it, with guidance, to a piece of work.
- Ask for an example of guidance they received while applying a standard, and
  what changed as a result.
- Ask a referee whether the candidate applies standards correctly once shown.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Ownership and initiative

**What the framework expects at this level**

An infrastructure engineer owns an issue until a new owner has been found or
the problem has been mitigated or resolved.

**Baseline for this role level**

Baseline: 2 — Working. Holding onto an issue through a handover rather
than dropping it once escalated is basic, guided-but-independent
professionalism expected of every working-level engineer.

**Behavioural indicators**

- 1 — Awareness: keeps an issue open on their own list but needs reminding to
  chase it.
- 2 — Working: tracks an issue through to a confirmed handover or
  resolution without being chased.
- 3 — Practitioner: proactively checks that a handed-over issue actually landed
  with its new owner and was picked up.
- 4 — Expert: is the person others rely on to never let an issue slip, across
  a whole shift or team.

**Evidence to gather**

- Ask for an example of an issue the candidate followed through to resolution
  or a confirmed handover.
- Ask a referee whether issues assigned to the candidate have gone missing or
  stalled.
- Review a ticket history showing the candidate's ownership of an issue over
  time.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Problem management

**What the framework expects at this level**

An infrastructure engineer investigates problems in systems, processes and
services, with an understanding of the level of a problem — for example,
strategic, tactical or operational — and contributes to the implementation of
remedies and preventative measures.

**Baseline for this role level**

Baseline: 2 — Working. The framework's wording for this skill is
unchanged from the associate level, so the baseline holds; this level's
growth is expressed through IT infrastructure and Systems design instead.

**Behavioural indicators**

- 1 — Awareness: investigates a problem when told exactly what to look at.
- 2 — Working: investigates a problem unassisted, correctly judges
  whether it is strategic, tactical or operational, and contributes ideas to
  the remedy.
- 3 — Practitioner: leads the investigation of a problem and proposes the
  preventative measure that is adopted.
- 4 — Expert: spots patterns across multiple problems that others have
  missed.

**Evidence to gather**

- Ask the candidate to describe a problem they investigated and how they
  judged its level.
- Review a problem record or root-cause note they contributed to.
- Ask a referee about the candidate's contribution to a remedy that was
  implemented.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Service focus

**What the framework expects at this level**

An infrastructure engineer takes inputs and establishes coherent frameworks
that work.

**Baseline for this role level**

Baseline: 2 — Working. Turning a set of inputs into a working framework
for their own area of work is applied, guided practice, short of the
bigger-picture view expected at senior levels.

**Behavioural indicators**

- 1 — Awareness: follows a service framework someone else has set up.
- 2 — Working: takes several inputs and organises their own work into a
  coherent, working routine.
- 3 — Practitioner: improves a service framework so it works better for the wider
  team.
- 4 — Expert: designs a service framework that a whole team adopts.

**Evidence to gather**

- Ask the candidate to describe how they organise a mix of build,
  configuration and support tasks into a coherent routine.
- Ask a referee whether the candidate's way of working has helped or hindered
  the wider team.
- Review any process note or checklist the candidate has produced for their
  own work.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Systems design

**What the framework expects at this level**

An infrastructure engineer translates logical designs into physical designs,
produces detailed designs, effectively documents all work using required
standards, methods and tools, including prototyping tools where appropriate,
designs systems characterised by managed levels of risk, manageable business
and technical complexity, and meaningful impact, and works with well
understood technology, identifying appropriate patterns.

**Baseline for this role level**

Baseline: 2 — Working. Producing detailed, documented designs for
systems of managed risk and complexity is real design work, but bounded to
well understood technology and managed risk — a clear step above assisting
with components, but not yet the medium-risk, review-others'-work
independence of the level above.

**Behavioural indicators**

- 1 — Awareness: translates part of a logical design into a physical one with
  close review.
- 2 — Working: translates logical designs into physical designs,
  produces detailed, documented designs for systems of managed risk and
  complexity, and works with well understood technology unassisted.
- 3 — Practitioner: designs systems of medium risk and complexity independently,
  and reviews a colleague's design.
- 4 — Expert: sets the design standards others follow.

**Evidence to gather**

- Ask the candidate to walk through a physical design they produced from a
  logical design, and how they documented it.
- Review a detailed design document they produced.
- Ask a referee about the risk and complexity of systems the candidate has
  designed unassisted.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Systems integration

**What the framework expects at this level**

An infrastructure engineer builds and tests simple interfaces between
systems, and works on more complex integration as part of a wider team.

**Baseline for this role level**

Baseline: 2 — Working. Building and testing simple interfaces
independently, while still contributing to complex integration as part of a
team, is a real step up from explaining the concepts at the associate level.

**Behavioural indicators**

- 1 — Awareness: builds a simple interface with a colleague checking the
  result.
- 2 — Working: builds and tests simple interfaces unassisted, and
  contributes to more complex integration as part of a wider team.
- 3 — Practitioner: leads the build of a moderately complex interface.
- 4 — Expert: designs the integration approach for a complex, multi-system
  programme.

**Evidence to gather**

- Ask the candidate to describe a simple interface they built and tested.
- Ask for an example of their contribution to a more complex integration
  effort.
- Ask a referee about the quality of interfaces the candidate has built.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Testing

**What the framework expects at this level**

An infrastructure engineer correctly executes test scripts under supervision
and understands the role of testing and how it works.

**Baseline for this role level**

Baseline: 1 — Awareness. The framework's own wording — "under supervision" —
sets the ceiling for this skill at this level.

**Behavioural indicators**

- 1 — Awareness: executes a test script correctly with a colleague checking
  the outcome, and can explain what testing is for.
- 2 — Working: executes test scripts correctly without needing a check
  at every step.
- 3 — Practitioner: identifies gaps in a test script and suggests improvements to
  it.
- 4 — Expert: writes test scripts that others execute.

**Evidence to gather**

- Observe the candidate executing a test script, or ask them to walk through
  one they have run.
- Ask them to explain, in their own words, what testing achieves for the
  infrastructure they support.
- Ask a referee how much supervision the candidate's test execution still
  needs.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

### Skill: Troubleshooting and problem resolution

**What the framework expects at this level**

An infrastructure engineer troubleshoots and identifies problems across
different technology capabilities.

**Baseline for this role level**

Baseline: 2 — Working. Troubleshooting across different technology
capabilities unassisted is a genuine step up from the associate level's basic,
single-area troubleshooting.

**Behavioural indicators**

- 1 — Awareness: troubleshoots a familiar fault in one technology area, with a
  colleague checking the approach.
- 2 — Working: troubleshoots and identifies problems across different
  technology capabilities unassisted.
- 3 — Practitioner: breaks a problem spanning several technology areas down into
  its component parts to find the root cause.
- 4 — Expert: is the escalation point for faults that have resisted diagnosis
  by others.

**Evidence to gather**

- Observe the candidate troubleshooting a fault that spans more than one
  technology area, or ask them to walk through one they resolved.
- Ask for an example of a fault they diagnosed across different technology
  capabilities.
- Ask a referee how much supervision the candidate still needs when
  troubleshooting.

**Rate this skill**

- [ ] 1 — Awareness
- [ ] 2 — Working
- [ ] 3 — Practitioner
- [ ] 4 — Expert

*Assessor notes:*

## Overall determination

| Skill | Baseline target | Rating given | Met baseline? |
| --- | ---: | --- | --- |
| Coding and scripting | 1 — Awareness | | |
| IT infrastructure | 2 — Working | | |
| Information security | 1 — Awareness | | |
| Modern development standards | 2 — Working | | |
| Ownership and initiative | 2 — Working | | |
| Problem management | 2 — Working | | |
| Service focus | 2 — Working | | |
| Systems design | 2 — Working | | |
| Systems integration | 2 — Working | | |
| Testing | 1 — Awareness | | |
| Troubleshooting and problem resolution | 2 — Working | | |

The baseline for this role level is normally met when every skill above is
rated at or above its baseline target.

One or two skills below target is not automatically a fail. Weigh how central
the skill is to this level's accountabilities — IT infrastructure and Systems
design sit at the centre of the job, while a Coding and scripting or
Information security gap may be more coachable quickly, since the framework
only asks this level to describe or explain those, rather than apply them
independently. Record your reasoning, not just the numbers: a panel or
manager reading this later needs to know why a below-target rating was, or
was not, treated as a concern.

- [ ] Meets baseline for this role level
- [ ] Does not yet meet baseline
- [ ] Meets baseline, with development areas noted below

This determination is an input to your organisation's own hiring, promotion
or development process. It is never the decision itself.

## Assessor notes and evidence log

**Candidate or employee name:**

**Assessor name:**

**Date:**

**Evidence sources used** (tick all that apply):

- [ ] CV or portfolio
- [ ] Structured interview
- [ ] Work sample or task
- [ ] References
- [ ] Direct observation
- [ ] Other

**Overall notes:**

**Follow-up actions:**

For a hiring or promotion decision, a second assessor or a panel calibration
session is recommended before the result is acted on.

## Conclusion

Use this result alongside your organisation's own process — it is a
structured record of what was observed, not a verdict on its own. Where
development areas were noted, the sibling documents for this role level offer
a starting point:

- [Continuing professional development checklist](../../../../uk-gdad-pcf-continuing-professional-development-checklists/roles/it-operations/infrastructure-engineer/2-infrastructure-engineer.md)
- [Upskilling resources](../../../../uk-gdad-pcf-upskilling-resources/roles/it-operations/infrastructure-engineer/2-infrastructure-engineer.md)

Whatever the outcome, treat this assessment as one input among several — a
CV, an interview, a reference and this competency matrix together give a
fuller picture than any one of them alone. Thank you for taking the time to
assess carefully; it matters to the person on the other end of the process.
